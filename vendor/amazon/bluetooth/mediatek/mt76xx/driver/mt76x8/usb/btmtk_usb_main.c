...
static int btmtk_usb_send_assert_cmd(void)
{
	...
	if (ret < 0) {
		...
		if (g_data->interrupt_urb_submitted == 0) {
			...
			if (ret < 0) {
				BTUSB_ERR("%s: Submit intr-in fail, tigger hw reset", __func__);
				btmtk_usb_toggle_rst_pin();
			}
		}
	}
...
#define HCE_SYNC_CONN_COMPLETE 0x2C
		...
		BTUSB_DBG("%s: urb->status:%d, urb->length:%d", __func__, urb->status, urb->actual_length);
	}
intr_resub:
	usb_mark_last_busy(g_data->udev);
	usb_anchor_urb(urb, &g_data->intr_in_anchor);
...
#ifndef CONFIG_SECURITY_SELINUX_KERN_PERMISSIVE
			else if (print_dump_data_counter == PRINT_DUMP_PACKET_COUNT) {
				BTUSB_INFO("btmtk_usb FW dump data (%d): user build, call rst pin",
						print_dump_data_counter);
				btmtk_usb_toggle_rst_pin();
					...
					size_t count, loff_t *f_pos)
{
	...
	if (is_mt7668(g_data) && g_data->is_mt7668_dongle_state != BTMTK_USB_7668_DONGLE_STATE_POWER_ON) {
		BTUSB_ERR("%s: 7668 is not opening", __func__);
		return -EBADFD;
	}
