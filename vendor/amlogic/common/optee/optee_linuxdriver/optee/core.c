			...
			phys_addr_t *tee_log_shm_pa)
{
	...
	if (!dev || !invoke_fn || !ioremaped_shm || !tee_log_shm_pa) {
...
static int optee_probe(struct platform_device *pdev)
{
	...
	pool = optee_config_shm_ioremap(&pdev->dev, invoke_fn, &ioremaped_shm, &tee_log_shm_pa);
	if (IS_ERR(pool))
		return PTR_ERR(pool);
	...
	rc = optee_log_init(optee->teedev->dev.class, tee_log_shm_pa);
	if (rc)
		goto err;
...
static int optee_remove(struct platform_device *pdev)
{
	...
	optee_log_exit(optee->teedev->dev.class);
	tee_device_unregister(optee->teedev);
	tee_device_unregister(optee->supp_teedev);
