...
struct module_oemid module_list[20];
...
static void tvconfig_mmc_flash_write(const char *cmd, void *download_buffer,
                        unsigned int download_bytes)
{
...
static int get_oem_index(const char *hwid)
{
	...
	while(strcmp(module_list[i].config, "null")!=0)
	{
		debugf("multiconfig = %s, hwid = %s \n", module_list[i].config, model_hwid);
		if((strcmp(model_hwid, hwid)==0)&&(strstr(buf,module_list[i].config)!=NULL))
		return i;
		i++;
	}
	return -1;
...
static int check_hash(unsigned char hash[])
{
	unsigned int i;
	unsigned char output[24]="",output_str[24]="";
	unsigned char *str_ptr = output_str;
	int current_ret = run_command("store read tvconfig 0x20000000 0 0x4000000",0);
	if(current_ret)
	{
		printf("store read tvconfig partition failed\n");
		return 1;
	}
	md5(0x20000000, 0x4000000, output);
	for(i=0; i<16; i++)
	{
	...
	debugf("package  hash =%s, current hash=%s\n", hash, output_str);
	return strncmp(hash,output_str,16);
}
...
static int parse_package_hash(const char *file_name, const char *brand_name, const char *hwid, unsigned char hash[])
{
        const char *ini_value = NULL;
        unsigned int i;
        char  hwid_buf[64];
        ...
        ini_value = IniGetString(brand_name, "HASH", "null");
        if (strcmp(ini_value, "null") != 0)
        {
                //*version = strtoul(ini_value, NULL, 10);
                strcpy(hash, ini_value);
                debugf("package  hash =%s, cpy hash=%s\n", ini_value, hash);
                IniParserUninit();
                return 0;
                /*
                //check hwid
...
void update_tvconfig(const char *hwid)
{
	...
	unsigned char current_version[128] = "", package_version[128] = "", package_hash[24]="";
...
#endif
	//get shmem from bl2
        memcpy((void *)module_list,(void *)0x10000000,20*sizeof(struct module_oemid));
	...
	if(current_ret == 0)
	{
		package_ret = parse_package_hash(OEM_INI_PATH,oem_name, hwid, package_hash);
		if(package_ret!=0)
			return;
		if(check_hash(package_hash)==0)
		{
