/*
 * Copyright 2016 - 2020, Amlogic Inc
 * yonghui.yu
 *
...
/*
  ...
  MMC_RESERVED_SIZE is 8MBytes for now.
...
static int do_aept(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int ret = 0;

	if (argc < 2)
		return CMD_RET_USAGE;
	if (!strcmp("n2i", argv[1]))
	{
		int idx;
		/* Important! */
		idx = get_partition_num_by_name(argv[2]);
		if (idx >= 0)
			printf("part %s idx is %d\n", argv[2], idx);
		else
...
U_BOOT_CMD(
	aept, 3, 0, do_aept,
	"Amlogic emmc partition table sub system",
	" n2i part-name - display index number of partition\n"
);
