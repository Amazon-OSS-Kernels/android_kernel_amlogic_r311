...
int saradc_probe(void)
{
	...
	if (saradc_fdt_ready) {
		...
		*/
		...
		{
			nodeoffset=fdt_path_offset(fdt_addr, "/");
			if (parent_offset < 0) {
				printf("saradc: not find the node /arch  it should be arm64\n");
			}else{
				prop = fdt_getprop(fdt_addr, nodeoffset, prop_name, &len);
				if(prop){
					if(!strncmp((char *)prop,"arm32",len)){
						printf("saradc: arm32 set default value\n");
						saradc->saradc_clk_addr =  (u32 __iomem *)AO_SAR_CLK;
						saradc->saradc_base_addr = (u32 __iomem *)AO_SAR_ADC_REG0;
					}
				}else {
					printf("error! saradc: not find get property arch \n");
					return -1;
				}
			}
		}
