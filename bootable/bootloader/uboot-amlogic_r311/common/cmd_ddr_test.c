...
//bit 0 trigger effect reset.
	if ((magic_chipid) != (DDR_STICKY_MAGIC_NUMBER+DDR_CHIP_ID)) {
...
if (argc >6)
{
	...
	writel(magic_chipid, PREG_STICKY_REG0);
 ...
 if (argc == 1)
	...
	if (argc >= 2)
		...
		wr_reg_and_mask_1 = simple_strtoull_ddr(argv[3], &endp, 10);
			...
			//	 		 	   	 if(argc >= 5)
		//	{
	 //	 wr_reg_or_mask_2= simple_strtoull_ddr(argv[4], &endp, 10);
...
U_BOOT_CMD(
	ddr_offset,	30,	1,	do_ddr_offset_ddr_lcdlr,
	"DDR delay offset function",
	"follow ddrtest [LOOP] [ADDR].Default address is 0x8d000000\n"
);
...
#ifdef CONFIG_IDME
	char str[64]={0};
	char idme_freq_str[64]={0};
	uint32_t idme_clock = 1008, idme_read_freq = 0;
	if(idme_get_var_external("bt_mfg", idme_freq_str, sizeof(idme_freq_str)) == 0) {
		idme_read_freq = simple_strtoull_ddr(idme_freq_str, NULL, 16);
		if (idme_read_freq & 0xfff)
			idme_clock = idme_read_freq & 0xfff;
		printf("idme_read=0x%x, ddrtest_set_ddr_freq=%dMHz\n",idme_read_freq, idme_clock);	
	}
	sprintf(str,"ddr2clk %d",idme_clock);
	printf("\nstr=%s\n",str);
	if(global_ddr_clk!=(idme_clock)){
		run_command(str,0);
		while(1);
	}
#endif
...
int do_ddr_change_frequency(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	char *endp;
	unsigned long clk;
	...
	clk = simple_strtoul(argv[1], &endp,0);
	if (*argv[1] == 0 || *endp != 0) {
		printf ("Error: Wrong format parament!\n");
		return 1;
	}
	writel(clk | (0xf12 << 20), P_PREG_STICKY_REG9);
	printf("Set P_PREG_STICKY_REG9 done [0x%08x]\n", readl(P_PREG_STICKY_REG9));
	reset_system();
...
U_BOOT_CMD(
	ddr2clk,	10,	1,	do_ddr_change_frequency,
	"DDR set clk  function",
	"DDR clk set:ddr2clk e...g... 1104.\n"
);
int do_ddr_test_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
#ifdef CONFIG_IDME
	char idme_ddrtest_str[64]={0};
	int idme_read_uboot_flag = 0;
	if(idme_get_var_external("bt_mfg", idme_ddrtest_str, sizeof(idme_ddrtest_str)) == 0) {
		idme_read_uboot_flag = simple_strtoull_ddr(idme_ddrtest_str, NULL, 16);
		if(!(idme_read_uboot_flag & 0x10000))
			return 0;
		printf("idme_read_ddr_uboot_flag is 1\n");
		printf("enable ddr_window\n");
	}
#endif
...
#if ( CONFIG_DDR_PHY_NEW_TAG1 <   P_DDR_PHY_NEW_TAG_G12)
