...
static int bootm_find_fdt(int flag, int argc, char * const argv[])
{
...
#ifndef CONFIG_USE_BOOTIMAGE_DTB
	...
	printf("load bootimage dtb from 0x%lx ......\n",
			(unsigned long)(images.ft_addr));
#ifdef CONFIG_MULTI_DTB
	extern unsigned long get_multi_dt_entry(unsigned long fdt_addr);
	/* update dtb address, compatible with single dtb and multi dtbs */
			 ...
			 int boot_progress)
{
	...
	if (*(uint32_t *)((images->ep) + 0x38) == le32_to_cpu(LINUX_ARM64_IMAGE_MAGIC))
		...
		if ((bootargs = getenv("bootargs")) != NULL){
			len = strlen(bootargs) + strlen(" vmalloc=300M") + 1;
			newbootargs = malloc(len);
			if (newbootargs != NULL) {
				memset(newbootargs, 0, len);
				strcpy(newbootargs, bootargs);
				strcat(newbootargs, " vmalloc=300M");
