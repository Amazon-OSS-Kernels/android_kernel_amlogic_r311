...
static int _aml_get_secure_boot_kernel_size(const void* pLoadaddr, unsigned* pTotalEncKernelSz)
{
    ...
    *pTotalEncKernelSz = 0;
    ...
    /* Return since we are using UFBL secure boot */
    return 0;
#else
...
static int do_image_read_kernel(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if (secureKernelImgSz) {
        ...
        actualBootImgSz= (char*)android_image_get_end(hdr_addr) - (char*)hdr_addr;
#if defined(UFBL_FEATURE_SECURE_BOOT)
	actualBootImgSz += SIGNATURE_SZ;
#endif
    }
