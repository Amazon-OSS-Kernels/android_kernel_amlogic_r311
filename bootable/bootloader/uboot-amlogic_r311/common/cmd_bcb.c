...
#define RECOVERY_SILENT_OTA    "--silent"
    ...
    char * const argv[])
{
    ...
    /* This command seems to cause issue if the partition is corrupted. */
    /* printf("[commannd:%s]\n[status:%s]\n[recovery:%s]\n",
            command, status, recovery); */

#if CONFIG_AMZN_SILENT_OTA
    if (strstr(recovery, RECOVERY_SILENT_OTA)) {
        //set_silent_ota_flag();
        printf("recovery: found Silent OTA flag.\n");

        if(run_command("get_rebootmode", 0) < 0){
              printf("run_command for cmd:run get_rebootmode failed.\n");
	}

        env = getenv("reboot_mode");

        if(!strcmp(env,"cold_boot")){

              printf("STR mode because cold mode and Silent OTA waiting for waking up to recovery.\n");
              if(run_command("run system_factory_off", 0) < 0){
                     printf("run_command for cmd:run system_factory_off failed.\n");
	      }

        }else{
              set_silent_ota_flag();
              printf("go to check recovery mode.\n");
	}
    }
    ...
    if (!memcmp(command_mark, command, strlen(command_mark))) {
        ...
        if (clear_misc_partition(clearbuf, sizeof(clearbuf)) < 0) {
            ...
            if (strlen(command) > COMMANDBUF_SIZE) {
                    printf("command size is greater than allowable buffer, clearing partition\n");
                    if (clear_misc_partition(clearbuf, sizeof(clearbuf)) < 0) {
                            printf("clear misc partition failed.\n");
                            goto ERR;
                    } else {
