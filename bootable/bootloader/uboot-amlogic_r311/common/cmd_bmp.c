			...
			void **alloc_addr)
{
	...
	/*debugP("srcAddr[%x, %x]\n", srcAddr[0], srcAddr[1]);*/
	if (memcmp(addr, gzip_magic, sizeof(gzip_magic)) != 0)
	{
		puts("Error: this is not zip file!\n");
		return NULL;
	}
	 ...
	 * Decompress bmp image
	 */
	len = CONFIG_MAX_PIC_LEN;
	/* allocate extra 3 bytes for 32-bit-aligned-address + 2 alignment */
	dst = malloc(CONFIG_MAX_PIC_LEN + 3);
	if (dst == NULL) {
		puts("Error: malloc in gunzip failed!\n");
		return NULL;
	}
	...
	if (gunzip(bmp, CONFIG_MAX_PIC_LEN, (uchar *)addr, &len) != 0) {
		free(dst);
		return NULL;
	}
	if (len == CONFIG_MAX_PIC_LEN)
		puts("Image could be truncated"
			" (increase CONFIG_MAX_PIC_LEN)!\n");
