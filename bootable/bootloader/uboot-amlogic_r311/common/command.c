			       ...
			       int *repeatable, ulong *ticks)
{
	...
	cmd_tbl_t *cmdtp;
	static int sec_flashing_enabled = 0;
	...
	if (amzn_is_command_blocked(argv[0]) == true) {
		if (sec_flashing_enabled || amzn_device_is_sec_flashing_enabled()) {
			sec_flashing_enabled = 1;
		} else {
			printf("I'm sorry, but command %s is blocked\n", argv[0]);
			return 1;
		}
	}
