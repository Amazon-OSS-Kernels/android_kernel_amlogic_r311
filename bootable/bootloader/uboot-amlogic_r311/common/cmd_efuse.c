...
#define CMD_EFUSE_ANTIROLLBACK_SET	9
...
int cmd_efuse(int argc, char * const argv[], char *buf)
{
	...
	if (strncmp(argv[1], "read", 4) == 0) {
		...
		goto efuse_action;
	} else if (strncmp(argv[1], "antirollback_set", 16) == 0) {
		action = CMD_EFUSE_ANTIROLLBACK_SET;
		goto efuse_action;
	} else{
		printf("%s arg error\n", argv[1]);
...
efuse_action:
	...
	/* efuse write */
		...
		return ret;
	} else if (CMD_EFUSE_ANTIROLLBACK_SET == action) {
		ret = bl31_smc(EFUSE_ENABLE_ANTIROLLBACK, (('a' << 16) | ('r' << 8) | ('b' << 0)), 0);

		if (ret)
			printf("aml log : EFUSE antirollback enable fail [%d]!\n", ret);
		else
			printf("aml log : EFUSE antirollback enable success!\n");
...
int query_efuse_status(const char *item)
{
	...
	static const struct {
		char *szQuery;
		unsigned long lAddrConfig;
		unsigned int nMask;
		unsigned int nNegFlag;
	} query[] = {
		...
		{"ARB", AO_SEC_SD_CFG10, 1<<25, 0},      /* Anti-rollback: 1: enabled; 0:disabled */
		{ NULL, 0, 0, 0 }
	};
	...
	for (i = 0; i < ARRAY_SIZE(query) && query[i].szQuery; i++)
		if (!strcmp(query[i].szQuery, item)) {
			rc  = (readl(query[i].lAddrConfig) & query[i].nMask)
					? 1 : 0;
			rc ^= (query[i].nNegFlag ? 1 : 0);
			break;
		}
done:
	return rc;
...
static int do_query(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	nReturn = query_efuse_status(argv[1]);
exit:
