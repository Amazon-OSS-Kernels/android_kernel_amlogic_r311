...
int is_phy_addr_ready(cec_msg_t *msg)
{
	...
	if (((hdmi_cec_func_config >> CEC_FUNC_MASK) & 0x1) &&
	    ...
	    msg->cec_power) {
		if (get_phy_addr(source_addr) &&
		    get_logic_addr(source_addr) &&
			(get_phy_addr(source_addr) != 0xFFFF)) {
				uart_puts("is_phy_addr_ready 0x");
				uart_put_hex(get_phy_addr(source_addr), 16);
				uart_puts("\n");
				return 1;
		}
	}
...
void cec_save_addr(void)
{
	unsigned int phy_addr;
	int i, port_id;

	phy_addr = get_phy_addr(readl(AO_RTI_STATUS_REG1));

	if ((phy_addr == 0xFFFF) || ((phy_addr & 0xF000) == 0)) {
		writel((0xFFFF << 16) | ((~WAKE_UP_PORT_ID_MASK) &
			readl(AO_RTI_STATUS_REG0)), AO_RTI_STATUS_REG0);
		return;
	}
	...
	writel((port_id << 16) |
			((~WAKE_UP_PORT_ID_MASK) & readl(AO_RTI_STATUS_REG0)) ,
			AO_RTI_STATUS_REG0);
}
