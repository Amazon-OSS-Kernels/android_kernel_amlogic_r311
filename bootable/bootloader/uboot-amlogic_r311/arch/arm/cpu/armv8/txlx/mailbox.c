...
void send_to_led_pattern(unsigned int val)
{
	scpi_send32(SCPI_CMD_SET_LED_PATTERN, val, LOW_PRIORITY);
}
...
void set_WOL_power(unsigned int power)
{
	send_usr_data(SCPI_CL_WOL_POWER, &power, sizeof(unsigned int));
}

int get_WOL_power(unsigned int *value)
{
#if 0
	unsigned int *response;
	unsigned int size;
	...
	mb_message_start(LOW_PRIORITY);
	mb_message_send(SCPI_CMD_GET_WOL_POWER, LOW_PRIORITY);
	mb_message_receive((void *)&response, &size, LOW_PRIORITY);
	mb_message_end(LOW_PRIORITY);
