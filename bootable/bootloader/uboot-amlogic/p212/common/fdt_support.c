...
#if defined(CONFIG_IDME)
	str = getenv("bootargs");
	char paramname[] = "androidboot.serialno=";
	char buf[24] = {0};
	u8   serial_length, i;
	if (!idme_get_var_external("serial", buf, sizeof(buf)-1)) {
		for (i = 0; i < SERIAL_LEN; i++) {
			/* suppose the serial numbers are consist  of numbers and upper case chars
			 * If the lower case chars are also supported, add the lower case checks */
			if (!(((buf[i] >= '0') && (buf[i] <='9'))
				|| ((buf[i] >='A') && (buf[i] <='Z')))) {
				err =-1;
				printf("WARNING: serial_no coantains invalid chars %s.\n");
				return err;
				}
			}
		char *tmp = strstr(str, paramname);
		serial_length = strlen(buf);
		if (serial_length > SERIAL_LEN)
			serial_length = SERIAL_LEN;
		if(tmp)
			memcpy(tmp + strlen(paramname), buf, serial_length);
	}
#endif
	str = getenv("bootargs");
	if (str) {
