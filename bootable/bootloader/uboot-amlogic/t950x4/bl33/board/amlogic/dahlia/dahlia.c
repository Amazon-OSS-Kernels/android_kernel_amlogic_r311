...
static int get_amp_type(char *amp_type, int size)
{
    int ret = -1;
    char model_name[256] = "";
    ...
    ini_value = IniGetString("DEFAULT", "AMP_TYPE", "null");
    if (strcmp(ini_value, "null") == 0) {
        printf("%s, get \"AMP_TYPE\" item failed!\n", __func__);
        goto exit;
    }

    memset(amp_type, 0 , size);
    strncpy(amp_type, ini_value, size - 1);
...
void reset_mt7668(void)
{
	...
	run_command("gpio clear GPIOB_12",0);
	mdelay(200);
...
*/
		...
		case 0x60000000:
			...
			else if (strcmp("1.5G_dahlia", config_name) == 0) {
                        	if (strcmp(amp_type, "tas5805") == 0) {
				strcpy(dtb_name, "t5d_t950d4_proto-am301-1.5g-tas5895\0");
			...
			else if (strcmp(amp_type, "tas5805") == 0) {
				strcpy(dtb_name, "t5d_t950d4_proto-am301-1g-tas5805\0");
                                setenv("cpu_version", "rev_b");
                        }
                        else {
