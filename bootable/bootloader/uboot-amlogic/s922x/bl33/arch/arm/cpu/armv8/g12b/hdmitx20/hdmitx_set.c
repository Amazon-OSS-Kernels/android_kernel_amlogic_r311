...
void hdmi_tx_set(struct hdmitx_dev *hdev)
{
	...
	hdmitx_set_hw(hdev);
	hdmitx_debug();
	update_logoparam_value("logoparam.var.hdmi_mode", getenv("outputmode"));
	setenv("hdmimode", getenv("outputmode"));
	update_logoparam_value("logoparam.var.hdmi_colorattr", getenv("colorattribute"));
	printf("save mode is : %s  attr: %s\n",
