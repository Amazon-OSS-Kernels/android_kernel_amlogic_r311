...
int amzn_device_unlock_status = 0;
#endif
...
static int image_verify(unsigned char *addr)
{
	...
	imgsize = (char*)android_image_get_end(hdr) - (char*)hdr;
	ret = amzn_image_verify(addr, addr + imgsize, imgsize, NULL);

	/*Amlogic use 0 as success */
	return (ret == 1) ? 0 : -1;
...
int do_bootm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	int nRet = 0;
#if defined(UFBL_FEATURE_UNLOCK)
	amzn_device_unlock_status = amzn_target_is_unlocked();
...
#else
	(void)nLoadAddr;
	int nRet = 0; //aml_sec_boot_check(AML_D_P_IMG_DECRYPT,nLoadAddr,GXB_IMG_SIZE,GXB_IMG_DEC_ALL);
#endif
	if (nRet)
	{
		printf("\naml log : Sig Check %d\n",nRet);
