		...
		int migratetype)
{
	...
	/* migrate type of page must be under protect of zone->lock */
...
continue_merging:
	while (order < max_order - 1) {
		...
		#ifdef CONFIG_AMLOGIC_MEMORY_EXTEND
			buddy_t = get_freepage_migratetype(buddy);
			__mod_zone_migrate_state(zone, -(1 << order), buddy_t);
...
#ifdef CONFIG_AMLOGIC_MEMORY_EXTEND	/* make real mt under zone lock*/
...
#ifdef CONFIG_AMLOGIC_CMA
static inline bool cma_fallback(int migratetype)
{
	int i, mt;

	for (i = 0; ; i++) {
		mt = fallbacks[migratetype][i];
		if (mt == MIGRATE_RESERVE)
			break;
		if (mt == MIGRATE_CMA)
			return true;
	}
			  ...
			  int migratetype)
{
	...
	for (page = start_page; page <= end_page;) {
		...
		list_move(&page->lru,
			  &zone->free_area[order].free_list[migratetype]);
	#ifdef CONFIG_AMLOGIC_MEMORY_EXTEND
		list_type = get_freepage_migratetype(page);
...
static int try_to_steal_freepages(struct zone *zone, struct page *page,
				  int start_type, int fallback_type, int *list)
#else
static int try_to_steal_freepages(struct zone *zone, struct page *page,
...
static inline struct page *get_migrate_page(struct zone *zone,
					    unsigned int order,
					    unsigned int current_order,
					    int start_migratetype, bool cma)
#else
/* get free migrate page from fallbacks */
static inline struct page *get_migrate_page(struct zone *zone,
					    unsigned int order,
...
#endif
{
	...
	#ifdef CONFIG_AMLOGIC_CMA
		if (migratetype == MIGRATE_CMA && !cma)
			continue;
	#endif /* CONFIG_AMLOGIC_CMA */
...
#endif /* CONFIG_AMLOGIC_CMA */
{
	...
	if (unlikely(start_migratetype == MIGRATE_RECLAIMABLE ||
		...
		end_order = max(order, (int)(pageblock_order / 2));
	}
	...
	for (current_order = MAX_ORDER - 1;
	     ...
	     --current_order) {
	#ifdef CONFIG_AMLOGIC_CMA
		page = get_migrate_page(zone, order, current_order, start_migratetype, cma);
	#else
...
#endif /* CONFIG_AMLOGIC_CMA */
{
	...
	if (cma_fallback(migratetype) && cma) {
...
retry_reserve:
	...
	#ifdef CONFIG_AMLOGIC_CMA
		page = __rmqueue_fallback(zone, order, migratetype, cma);
	#else
...
static int rmqueue_bulk(struct zone *zone, unsigned int order,
			...
			int migratetype, int cold, bool cma)
#else
static int rmqueue_bulk(struct zone *zone, unsigned int order,
			unsigned long count, struct list_head *list,
...
again:
	...
	#ifdef CONFIG_AMLOGIC_CMA
		/*
		 * CMA pages may get for can't use cam conditions from pcp
		 * so we should allocate again
		 */
		if (!cma && cma_page(page)) {
			spin_lock(&zone->lock);
			page = __rmqueue(zone, order, migratetype, false);
			if (!page) {
				spin_unlock(&zone->lock);
				goto failed;
			}
			...
			spin_unlock(&zone->lock);
			goto alloc_sucess;
		} else if (migratetype == MIGRATE_MOVABLE && cma) {
...
use_pcp:
		...
		spin_lock_irqsave(&zone->lock, flags);
	#ifdef CONFIG_AMLOGIC_CMA
		page = __rmqueue(zone, order, migratetype, cma);
	#else
...
#ifdef CONFIG_AMLOGIC_MEM_PRE_WAKE_KSWAP
static bool need_wakeup_kswap(gfp_t gfp_mask, struct zone *zone)
{
	...
	/* free pages is blow threshold */
	free_pages = global_page_state(NR_FREE_PAGES);
#ifdef CONFIG_CMA
	free_cma = global_page_state(NR_FREE_CMA_PAGES);
#endif
	if ((free_pages - free_cma) < high_wmark_pages(zone))
		return true;
	return false;
}
#endif /* CONFIG_AMLOGIC_MEM_PRE_WAKE_KSWAP */
...
retry:
#ifdef CONFIG_AMLOGIC_MEM_PRE_WAKE_KSWAP
	if (need_wakeup_kswap(gfp_mask, preferred_zone))
		wake_all_kswapds(order, zonelist, high_zoneidx, preferred_zone);
#endif /* CONFIG_AMLOGIC_MEM_PRE_WAKE_KSWAP */
...
extern void show_swap_cache_info_lmk(void);
...
{
#ifdef CONFIG_AMLOGIC_MEMORY_EXTEND
	lmk_add_to_buffer("active_anon:%lu inactive_anon:%lu isolated_anon:%lu"
		...
		" free_cma:%lu\n"
	#ifdef CONFIG_AMLOGIC_CMA
		" total_cma:%lu driver_cma:%lu"
	#endif /* CONFIG_AMLOGIC_CMA */
		" anon_cma:%lu file_cma:%lu isolate_cma:%lu isolate:%lu"
		" unmovable:%ld reclaimable:%ld movable:%ld reserve:%ld",
#else
	lmk_add_to_buffer("active_anon:%lu inactive_anon:%lu isolated_anon:%lu"
		" active_file:%lu inactive_file:%lu isolated_file:%lu"
		" unevictable:%lu"
		...
		global_page_state(NR_FREE_ISOLATE),
		global_page_state(NR_FREE_UNMOVABLE),
		global_page_state(NR_FREE_RECLAIMABLE),
		global_page_state(NR_FREE_MOVABLE),
		global_page_state(NR_FREE_RESERVE));
	#else
		global_page_state(NR_FREE_CMA_PAGES));
	#endif /* CONFIG_AMLOGIC_MEMORY_EXTEND */
