...
static int dvb_atsc_timeout = 4000;
...
int dvb_detect_strength(struct dvb_frontend *fe)
{
	fe_status_t s;
	u16 strength;
	int strength_threshold;
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
	if (dvb_tuner_delay <= 10)
		return 1;
	msleep(dvb_tuner_delay);
	fe->ops.read_signal_strength(fe, &strength);
	dprintk("strength is %d\n", strength);
	/*signal strength %, 13%=-87,50% = -50*/
	/*j.83b sen -72dbm, 8vsb sen -85dbm*/
	if (fe->dtv_property_cache.modulation > QAM_AUTO)
		strength_threshold = 13;
	else if (fe->dtv_property_cache.modulation <= QAM_AUTO)
		strength_threshold = 24;
	else
		strength_threshold = 13;
