 ...
 * a) A log indicating original corrupted block number will be printed
 * to kmsg buffer.
 * b) A uevent message will also be sent to userland via kobject_uevent_env.
 * c) If FEC reaches maximum corruption threshold, return -EIO.
 * Otherwise return 0.
 */
			...
			sector_t block)
{
	...
	if (atomic_read(&f->corrected) >= FEC_MAX_ERROR) {
		DMWARN_LIMIT("%s: reached maximum fec error correction",
				v->data_dev->name);
		return -EIO;
	}
