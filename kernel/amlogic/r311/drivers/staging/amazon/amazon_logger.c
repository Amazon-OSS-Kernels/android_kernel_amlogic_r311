...
#define METRICS_ENTRY_MAX_PAYLOAD 1024
#define META_DATA_MAX_PAYLOAD     256
...
#define AMAZON_MINERVA_MAX_PAYLOAD 1024
#define AMAZON_MINERVA_META_SIZE 256
#endif
 ...
 * @priority:   the Android priority of the message
 * @groupID :   the group id for kernel metrics
...
{
        static char str[METRICS_ENTRY_MAX_PAYLOAD];
...
{
	static char str[AMAZON_MINERVA_MAX_PAYLOAD];
			...
			const char *dimensions, const char *annotations)
{
	...
	if (key) {
		snprintf(str, AMAZON_MINERVA_MAX_PAYLOAD,
			"%s:%s:type=%d;DV;1,key=%s;DV;1,timer=%ld;TI;1,unit=%s;DV;1%s:HI",
			program, source, type,
			key, timer_value, unit,
			metadata_msg);
	} else {
		snprintf(str, AMAZON_MINERVA_MAX_PAYLOAD,
			"%s:%s:type=%d;DV;1,timer=%ld;TI;1,unit=%s;DV;1%s:HI",
			program, source, type,
			timer_value, unit,
			metadata_msg);
	}
...
again:
	while (1) {
		 ...
		 */
		ret = ring_buffer_wait(logger->buf, RING_BUFFER_ALL_CPUS);
		if (ret)
			return ret;
		/*
