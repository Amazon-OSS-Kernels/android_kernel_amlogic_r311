...
#define AML_ION_FLAG_BUFFER_NEEDS_CLEAR (1 << 30)
				      ...
				      unsigned long align)
{
	...
	if (!offset) {
		pr_err("ion_codec_mm_allocate failed out size %lu\n", size);
		mutex_unlock(&codec_heap->size_mutex);
		return ION_CARVEOUT_ALLOCATE_FAIL;
	}
				      ...
				      unsigned long flags)
{
	...
	/*
	 * Ensure that the gfx buffers are zeroed
	 */
	ptr = page_address(pfn_to_page(PFN_DOWN(paddr)));
	if ((flags && AML_ION_FLAG_BUFFER_NEEDS_CLEAR) == 1)
		memset(ptr, 0, size);
