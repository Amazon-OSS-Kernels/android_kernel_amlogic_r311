...
/* ACOS_MOD_BEGIN {fwk_crash_log_collection} */
/* Declarations */
static DEFINE_MUTEX(lowmem_mutex);
...
extern void ion_mm_heap_memory_detail_lmk(void);

extern void show_free_areas_lmk(unsigned int filter);
...
{
	struct lmk_track_node *tracker_node;

	if (mutex_lock_interruptible(&lowmem_mutex) < 0) {
		pr_err("%s: lock lowmem_mutex failed\n", __FUNCTION__);
		return;
	}

	tracker_node = kzalloc(sizeof(struct lmk_track_node), GFP_KERNEL);
	pr_info("add fun %p of module %s\n", fun, module);
	if (tracker_node) {
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (min_score_adj == OOM_SCORE_ADJ_MAX + 1) {
		...
		mutex_unlock(&lowmem_mutex); /* ACOS_MOD_ONELINE {fwk_crash_log_collection} */
		return 0;
	}
	...
	if (selected) {
		...
		/* ACOS_MOD_BEGIN {fwk_crash_log_collection} */
		if (foreground_kill) {
			show_free_areas_lmk(SHOW_MEM_FILTER_NODES);
			lowmem_print(1, "======ion heap=====\n");
