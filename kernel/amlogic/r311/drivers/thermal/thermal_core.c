...
/* Portions of this file are copyright 2017 by Amazon.com, Inc.
				...
				int trip, enum thermal_trip_type trip_type)
{
	...
	/* let governor know it is critical */
	if (tz->governor && tz->governor->critical)
		tz->governor->critical(tz, trip);

	if ((trip_type == THERMAL_TRIP_CRITICAL) &&
		(tz->temperature >= trip_temp)) {

#ifdef CONFIG_AMAZON_SIGN_OF_LIFE
		if (!strncmp(tz->type, "soc_thermal",
				sizeof("soc_thermal") - 1))
...
static void handle_thermal_trip(struct thermal_zone_device *tz, int trip)
{
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	char buf[THERMO_METRICS_STR_LEN + 1];
	static int tripdone[THERMAL_MAX_TRIPS];
	long trip_temperature;
	long trip_hyster;
#endif
	...
	if (tz->temperature <= (trip_temperature  - trip_hyster))
		tripdone[trip] = 0;
#endif
	/*
...
static DEVICE_ATTR(trips, 0444, trips_show, NULL);
