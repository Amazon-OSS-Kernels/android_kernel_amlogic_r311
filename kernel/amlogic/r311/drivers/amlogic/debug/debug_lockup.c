...
void notrace isr_in_hook(unsigned int cpu, unsigned long long *tin, unsigned int irq)
{
...
void notrace isr_out_hook(unsigned int cpu, unsigned long long tin, unsigned int irq)
{
	...
	if (tout > isr_thr + tin) {
		t_diff = tout - tin;
		do_div(t_diff, ns2ms);
		pr_err("ISR_Long___ERR. irq:%u  tout-tin:%llu ms\n",
			irq, t_diff);
	}
	...
	if (((cnt_total[irq] > CCCNT_WARN) && (irq != 30) &&
	     (t_isr[irq] * 100 >= 30 * t_total[irq])) ||
	    (t_isr[irq] * 100 >= 50 * t_total[irq])) {
		t_isr_tmp = t_isr[irq];
...
void pr_lockup_info(int c)
{
	...
	for_each_online_cpu(cpu)
	{
		...
		pr_err("\ndump_cpu[%d] ISR:%3d <%x h:%x, s:%x> %s <%p>\n",
			...
			(int)(preempt & SOFTIRQ_MASK), p->comm,
			cpu_sirq[cpu]);
		if (t_i_d[cpu]) {
...
static int __init debug_lockup_init(void)
{
	...
	debugfs_create_file("isr_check_en", S_IFREG | S_IRUGO,
			    debug_lockup, NULL, &isr_check_en_debug_ops);
	debugfs_create_file("irq_check_en", S_IFREG | S_IRUGO,
