...
static char *demod_version = "V0.05";
...
static CLASS_ATTR(dvbc_reg, 0660, dvbc_reg_show, dvbc_reg_store);
static CLASS_ATTR(atsc_para, 0664, atsc_para_show, atsc_para_store);
...
static int amdemod_stat_islock(struct aml_fe_dev *dev, int mode)
{
	...
	/*	pr_dbg("DTMB lock status is %u\n",
		       ((dtmb_read_reg(DTMB_TOP_FEC_LOCK_SNR) >> 14) &
			0x1));*/
	...
	(struct dvb_frontend *fe, fe_status_t *status)
{
	...
	if ((c->modulation <= QAM_AUTO) && (c->modulation != QPSK)
		...
		if ((s == 0) && (last_lock == 1)
			...
			s = 1;
			pr_dbg("[rsj] unlock,but fsm >= 0x76\n");
		}
	}
	dvbfe = get_si2177_tuner();
	if (dvbfe != NULL)
...
static int gxtv_demod_fe_enter_mode(struct aml_fe *fe, int mode)
{
	...
	if (fe->dtv_demod->cma_flag == 1) {
		...
		if (fe->dtv_demod->cma_flag == 1) {
			pr_dbg("CMA MODE, cma flag is %d,mem size is %d",
				fe->dtv_demod->cma_flag,
				fe->dtv_demod->cma_mem_size);
			dtmb_cma_alloc(dev);
			memstart_dtmb = dev->mem_start;
		} else {
			memstart_dtmb = fe->dtv_demod->mem_start;
...
static int gxtv_demod_fe_leave_mode(struct aml_fe *fe, int mode)
{
	...
	if (mode == AM_FE_DTMB) {
		/* close arbit */
		if (is_meson_txhd_cpu()) {
			/* demod_set_demod_reg(0x0, TXLX_DEMOD_REG4); */
			demod_set_reg_rlt(TXLX_DEMOD_REG4, 0x0);
		} else {
			/* demod_set_demod_reg(0x0, DEMOD_REG4); */
			demod_set_reg_rlt(GXBB_DEMOD_REG4, 0x0);
		}
...
static int __init gxtvdemodfrontend_init(void)
{
	...
	ret = class_create_file(gxtv_clsp, &class_attr_atsc_para);
	if (ret)
		pr_error("[gxtv demod]%s create class error.\n", __func__);
