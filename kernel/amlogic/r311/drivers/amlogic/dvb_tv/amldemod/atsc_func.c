...
MODULE_PARM_DESC(cci_threshold, "\n\t\t cci_threshold");
static int cci_threshold = 800;
module_param(cci_threshold, int, 0644);

MODULE_PARM_DESC(atsc_fast, "\n\t\t atsc fast test");
static int atsc_fast;
module_param(atsc_fast, int, 0644);
...
void atsc_fast_sw(void)
{
	if (atsc_fast) {
		atsc_write_reg(0x072a, 0x02);
		atsc_write_reg(0x072c, 0x02);
	} else {
		/*default setting*/
		atsc_write_reg(0x072a, 0x04);
...
if (!field_test_version) {
	...
	atsc_write_reg(0x718, 0x15);
	atsc_write_reg(0x71b, 0x0d);
	ar_flag = 0;
}
...
int cci_run(void)
{
	...
	int time[10];
	int i;
	threshold = avg_len*cci_threshold;
	...
	for (ck0 = 0; ck0 < avg_len; ck0++) {
		...
		buf_result[ck0][0] = result[0];
		buf_result[ck0][1] = result[1];
		buf_result[ck0][2] = result[2];
		buf_result[ck0][3] = result[3];
		buf_result[ck0][4] = read_atsc_fsm();

		peak[bin[0]] = peak[bin[0]] + power[0];
		peak[bin[1]] = peak[bin[1]] + power[1];
		peak[bin[2]] = peak[bin[2]] + power[2];
		peak[bin[3]] = peak[bin[3]] + power[3];
		/*pr_dbg("[%d]%d,[%d]%d,[%d]%d,[%d]%d\n",
	...
	for (ck0 = 0; ck0 < 2; ck0++) {
		pr_dbg(" %d - %d CCI: pos %x  power %x .\n",
			cci_cnt, ck0, max_b[ck0], max_p[ck0]);
	}
	time[4] = jiffies_to_msecs(jiffies);
/*	for (ck0 = 0;ck0 < avg_len; ck0++)
	{
	    pr_dbg("FSM[%x] CCI[%d]: %d %d %d %d\n",
	    buf_result[ck0][4], ck0, buf_result[ck0][0],
	    buf_result[ck0][1], buf_result[ck0][2], buf_result[ck0][3]);
	}*/
...
int cfo_run(void)
{
	...
	for (i = -(scan_range-1); i <= scan_range+1; i++) {
		...
		pr_dbg("[autoscan]crRate is %x, Offset is %dkhz\n ",
			crRate, Offset);
		/*detec cfo signal*/
		for (j = 0; j < detec_cfo_times; j++) {
			...
			if (sys_state >= CR_Lock) {
				cfo_sta = Lock;
				pr_dbg("fsm[%x][autoscan]cfo lock\n",
					read_atsc_fsm());
				break;
			}
			msleep(20);
		}
		pr_dbg("fsm[%x]cfo_sta is %d\n",
		...
		if (cfo_sta == Lock) {
			for (j = 0; j < cfo_times; j++) {
				...
				if (sys_state >= CR_Peak_Lock) {
					cr_peak_sta = Lock;
					pr_dbg("fsm[%x][autoscan]cr peak lock\n",
						read_atsc_fsm());
					break;
				} else if (sys_state <= 20) {
					pr_dbg("no signal,break\n");
					break;
				}
				msleep(20);
			}
		...
		if (cr_peak_sta == Lock) {
			/*atsc_reset();*/
			pr_dbg("fsm[%x][autoscan]atsc_reset\n",
			read_atsc_fsm());
			return Cfo_Ok;
		} else {
			if (table_count >= (max_count-1)) {
				pr_dbg("cfo not lock,will try again\n");
				return Cfo_Fail;
			}
			table_count++;
			Offset = freq_table[table_count];
...
/* ber per */
	...
	tmp[0] = atsc_read_reg(0x0f72);
	tmp[1] = atsc_read_reg(0x0f71);
	tmp[2] = atsc_read_reg(0x0f70);
	tap_abs = (tmp[0] << 16) + (tmp[1] << 8) + tmp[2];

	tmp[0] = atsc_read_reg(0x0f76);
	tmp[1] = atsc_read_reg(0x0f75);
	tmp[2] = atsc_read_reg(0x0f74);
	tmp[3] = atsc_read_reg(0x0f73);
	tap_pwr = (tmp[0] << 24) + (tmp[1] << 16) + (tmp[2] << 8) + tmp[3];
	pr_dbg
	    ("SNR %4x SNRdB %2d.%2d FSM %x cr %6x ck %6x",
	     ...
	     , SM, cr, ck);
	pr_dbg
		(" ber is %8x, ser is %8x, pwr is %x, abs is %x\n",
		ber, ser, tap_pwr, tap_abs);
