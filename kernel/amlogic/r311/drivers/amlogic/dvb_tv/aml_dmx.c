...
static void section_buffer_watchdog_func(unsigned long arg)
{
	...
	u32 error_clear_inval_buf = 0;
	u32 error_clear_inval_buf_data1;
		...
		if (LARGE_SEC_BUFF_MASK ==
			...
			if (section_busy32 & (dmx->smallsec.enable ?
				...
				DMX_WRITE_REG(device_no,
						SEC_BUFF_READY,
						section_busy32);
				error_clear_inval_buf = 1;
				error_clear_inval_buf_data1 = section_busy32;
			}
#if 0
	...
	if (error_clear_inval_buf)
		pr_error("clear invalid buffer 0x%x\n",
			error_clear_inval_buf_data1);
	return;
}
...
static struct dvb_demux_feed fake_feed;
/*return val: 0 - fail, 1 - success*/
static int section_crc2(
		u32 (*crc32)(struct dvb_demux_feed *f,
			const u8 *src,
			size_t len),
		u8 *p, int len) {
	fake_feed.feed.sec.crc_val = ~0;
	if (crc32)
		return (crc32(&fake_feed, p, len)) ? 0 : 1;
	return 1;
}
...
static void hardware_match_section(struct aml_dmx *dmx,
		u16 sec_num, u16 buf_num, unsigned long irq_flags)
{
	...
	int check_crc = 0;
	int crc_ok = 0;
	u32 (*crc32_f)(struct dvb_demux_feed *f, const u8 *src, size_t len);
	...
	chid = f->chan_id;
	feed = dmx->channel[chid].feed;
	check_crc = feed->feed.sec.check_crc;
	crc32_f = feed->demux->check_crc32;

	/*IRQ OPEN {{*/
	spin_unlock_irqrestore(&dmx->dvb->slock, irq_flags);

	dma_sync_single_for_cpu(dmx_get_dev(dmx),
			dmx->sec_pages_map + (buf_num << 0x0c),
			(1 << 0x0c), DMA_FROM_DEVICE);

	crc_ok = section_crc2(check_crc ? crc32_f : NULL, p, sec_len);

	spin_lock_irqsave(&dmx->dvb->slock, irq_flags);
	/*}} IRQ OPEN*/
...
static void process_section(struct aml_dmx *dmx, unsigned long irq_flags)
{
	...
	if (ready) {
		...
		for (i = 0; i < SEC_BUF_COUNT; i++) {
			...
			hardware_match_section(dmx, sec_num, i, irq_flags);
...
static irqreturn_t dmx_irq_handler(int irq_number, void *para)
{
	...
	unsigned long flags;
	u32 error_ts_pin = 0;
	u32 error_many_int = 0;
	u32 error_many_int_data1;
	u32 error_many_int_data2;
...

	{
		...
		if (jiffies_to_msecs(jiffies - dmx->int_check_time) >= 100
			...
			if (dmx->int_check_count > 1000) {
				...
				error_many_int = 1;
				error_many_int_data1 = dmx->int_check_count;
				error_many_int_data2 =
...
irq_handled:
	...
	if (error_many_int)
		pr_error("Too many interrupts (%d interrupts in %d ms)!\n",
			error_many_int_data1, error_many_int_data2);
	return IRQ_HANDLED;
}
...
static void dvr_sync_data(struct aml_asyncfifo *afifo,
				u8 **buf1, u32 *buf1_len,
				u8 **buf2, u32 *buf2_len)
{
	int total = afifo->buf_len / afifo->flush_size;
	int factor = dmx_get_order(total);
	int size = afifo->buf_len >> factor;
	int cnt;
	int offset;
	u32 reg_val;
	u32 start_addr;
	...
	afifo->buf_toggle = (reg_val - start_addr) / afifo->flush_size;

	if (buf1 && buf2 && buf1_len && buf2_len) {
		*buf1 = *buf2 = NULL;
		*buf1_len = *buf2_len = 0;
	}

	if (afifo->buf_read > afifo->buf_toggle) {
		cnt = (total - afifo->buf_read) * size;
		offset = afifo->buf_read * size;
		dma_sync_single_for_cpu(asyncfifo_get_dev(afifo),
				afifo->pages_map + offset,
				cnt,
				DMA_FROM_DEVICE);
		*buf1 = (u8 *)afifo->pages + offset;
		*buf1_len = cnt;
		afifo->buf_read = 0;
	}
	if (afifo->buf_toggle > afifo->buf_read) {
		cnt = (afifo->buf_toggle - afifo->buf_read) * size;
		offset = afifo->buf_read * size;
		dma_sync_single_for_cpu(asyncfifo_get_dev(afifo),
				afifo->pages_map + offset,
				cnt,
				DMA_FROM_DEVICE);
		*buf2 = (u8 *)afifo->pages + offset;
		*buf2_len = cnt;
				...
				struct aml_swfilter *sf)
{
	...
	if (sf) {
		if (buf1_len)
			ret = _rbuf_write(&sf->rbuf, buf1, buf1_len);
		if (buf2_len && (ret >= 0))
			ret = _rbuf_write(&sf->rbuf, buf2, buf2_len);
	} else {
...
static void dvr_irq_bh_handler(unsigned long arg)
{
	...
	if (has_data && dvb
		&& afifo->source >= AM_DMX_0 && afifo->source < AM_DMX_MAX) {
		dmx = &dvb->dmx[afifo->source];
		if (dmx->init && dmx->record) {
...
*/
	...
	buf = afifo->pages ? afifo->pages : asyncfifo_alloc_buffer(len);
	if (!buf)
		return -1;
...
int aml_stb_hw_set_source(struct aml_dvb *dvb, enum dmx_source_t src)
{
	...
	if (dvb->stb_source != hw_src) {
		...
		pr_info("illegal demux source new:%d old:%d\n",
			hw_src, old_source);
