...
/********************************CODE CHANGE LIST*****************************/
/* Date --- Version --- Note *************************************************/
/* 2019/11/05 --- V1.15 --- Add dynamic monitoring line frequency deviation. */
/* 2019/12/31 --- V1.16 --- Fix atv vif setting and offset error. */
#define AMLATVDEMOD_VER "Ref.2019/12/31a_V1.16"
#else
...
static void aml_atvdemod_get_pll_status(struct dvb_frontend *fe, void *stat)
{
	...
	if ((vpll_lock&0x1) == 0 && line_lock == 0) {
		*status = FE_HAS_LOCK;
		pr_info("visual carrier lock:locked[vpll:0x%x, line:0x%x]\n",
				vpll_lock, line_lock);
	} else {
		pr_info("visual carrier lock:unlocked[vpll:0x%x, line:0x%x]\n",
				vpll_lock, line_lock);
		*status = FE_TIMEDOUT;
		/*  *status = FE_HAS_LOCK;*/
	}
