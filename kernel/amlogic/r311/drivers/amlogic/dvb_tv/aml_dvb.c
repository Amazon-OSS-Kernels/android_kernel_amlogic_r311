...
long notrace aml_stb_get_base(int id)
{
	int newbase = 0;
				    ...
				    const char *buf, size_t count)
{
	...
	r = sscanf(buf, "%d %31s %x %x", &id, mname, &ctrl, &invert);
	if (r != 4)
		return -EINVAL;
...
static int aml_tsdemux_set_vid(int vpid)
{
	...
	dmx = get_stb_dmx();
	pr_info("am_dvb: set vid:0x%x\n", vpid);
	if (dmx) {
		if (dmx->vid_chan != -1) {
			pr_info("am_dvb: set vid:0x%x free chan\n", vpid);
			dmx_free_chan(dmx, dmx->vid_chan);
			dmx->vid_chan = -1;
		} else {
			pr_info("am_dvb: set vid:0x%x not free chan[%d][%d]\n",
				vpid, dmx->id, dvb->stb_source);
		}
		...
		pr_error("am_dvb: set vid:0x%x error dmx is null\n", vpid);
	}
...
static int aml_tsdemux_set_aid(int apid)
{
	...
	spin_lock_irqsave(&dvb->slock, flags);
	pr_info("am_dvb: set aid:0x%x\n", apid);
	dmx = get_stb_dmx();
	if (dmx) {
		if (dmx->aud_chan != -1) {
			pr_info("am_dvb: set aid:0x%x free chan\n", apid);
			dmx_free_chan(dmx, dmx->aud_chan);
			dmx->aud_chan = -1;
		} else {
			pr_info("am_dvb: set aid:0x%x not free chan[%d][%d]\n",
				apid, dmx->id, dvb->stb_source);
		}
		...
		pr_error("am_dvb: set aid:0x%x error dmx is null\n", apid);
	}
