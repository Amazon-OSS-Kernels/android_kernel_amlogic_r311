...
static int dvb_ca_en50221_link_size = 4096;
module_param_named(link_size, dvb_ca_en50221_link_size, int, 0644);
MODULE_PARM_DESC(link_size, "");
		...
		int slot, u8 *ebuf, int ecount)
{
	...
	status = ca->pub->read_lpdu(ca->pub, slot, buf, HOST_LINK_BUF_SIZE);
	if (status < 0) {
			...
			int flags, int slot_count)
{
	...
	ret = dvb_register_device(dvb_adapter,
			&ca->dvbdev, &dvbdev_ca, ca, DVB_DEVICE_CA);
	if (ret)
		goto error;
