...
{
	return (unsigned long)nsecs_to_jiffies64(sched_clock());
}
...
*/
#ifdef USE_INT_PIO
	{
		...
		int gpio;
		gpiod = of_get_named_gpiod_flags(child, "irq-gpios", 0, NULL);
...
static int __init aml_cimax_spi_mod_init(void)
{
	int ret;
...
static void __exit aml_cimax_spi_mod_exit(void)
{
	pr_dbg("Amlogic CIMAX SPI Exit\n");
	class_unregister(&cimax_spi_class);
}

module_init(aml_cimax_spi_mod_init);
module_exit(aml_cimax_spi_mod_exit);
