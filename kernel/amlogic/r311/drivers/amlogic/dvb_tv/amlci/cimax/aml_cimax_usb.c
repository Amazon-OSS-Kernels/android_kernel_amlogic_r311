...
{
	if (usb->cam_inserted[slot] == 1) {
		usb->cam_inserted[slot] = 2;
		aml_cimax_slot_state_changed(usb->cimax,
...
static int cimax_usb_poll(struct cimax_usb *usb)
{
	...
	case CIMAX_CAM_DET_OK: {
		...
		if ((!!usb->cam_inserted[slot]) != insert) {
			usb->cam_inserted[slot] = insert;
			cimax_usb_cam_plugin(usb, slot, insert);
...
*/
	{
		...
		int gpio;
		gpiod = of_get_named_gpiod_flags(child, "rst-gpios", 0, NULL);
	...
	struct class_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%i", &loop) == 1) {
		int a = g_usb->cam_inserted[0];
		int b = g_usb->cam_inserted[1];
		u8 cm[2];
		cm[0] = loop ? (b ? 0x85 : 0x80) : 0x81;/*CH*/
		cm[1] = loop ? (a ? 0x51 : 0x11) : 0x00;/*MOD*/
		cimax = g_usb->cimax;
		err = aml_cimax_usb_write_reg(cimax, ROUTER_CAM_CH, cm, 2);
	}
	return size;
}
...
static int __init aml_cimax_usb_mod_init(void)
{
	int ret;
...
static void __exit aml_cimax_usb_mod_exit(void)
{
	pr_dbg("Amlogic CIMAX USB Exit\n");
	class_unregister(&cimax_usb_class);
}

module_init(aml_cimax_usb_mod_init);
module_exit(aml_cimax_usb_mod_exit);
