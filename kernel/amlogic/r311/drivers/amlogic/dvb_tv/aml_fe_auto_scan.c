...
#include "aml_fe.h"
#ifdef CONFIG_AM_ATV_AUTO_SCAN
#include "amlatvdemod_auto_scan/atvdemod_func.h"
#else
...
#define pr_inf(fmt, args ...) pr_dbg("FE: " fmt, ## args)
#define pr_audio(fmt, args ...) \
	do { \
		if (debug_fe & 0x20) \
			pr_info("FE: " fmt, ## args); \
	} while (0)
...
static unsigned int analog_search_flag;
static unsigned int aml_timer_en = 1;
module_param(aml_timer_en, uint, 0444);
...
static void afc_timer_disable(void);
...
int amlogic_gpio_request(unsigned int pin, const char *label)
{
	...
	if (ret < 0)
		pr_error("gpio[%s] request fail.\n", label);
	return 0;
}
EXPORT_SYMBOL(amlogic_gpio_request);

unsigned int aml_get_analog_search(void)
{
	return analog_search_flag;
}

void aml_set_analog_search(unsigned int val)
{
	analog_search_flag = val;
}
...
struct dvb_frontend *get_si2177_tuner(void)
{
	...
	for (i = 0; i < FE_DEV_COUNT; i++) {
		...
		if (!strcmp(dev->drv->name, "si2151_tuner"))
			return dev->fe->fe;
		if (!strcmp(dev->drv->name, "si2159_tuner"))
			return dev->fe->fe;
		return dev->fe->fe;
	}
...
void aml_fe_get_atvaudio_state(int *state)
{
	...
	int vpll_lock = 0;
	int line_lock = 0;
	struct dvb_frontend *fe = get_si2177_tuner();
	...
	if (fe != NULL) {
		if (fe->ops.info.type == FE_ANALOG) {
			...
			if ((vpll_lock == 0) && (line_lock == 0)) {
				...
				*state = 0;
				pr_audio("state: %d, vpll:0x%x, line:0x%x.\n",
						*state, vpll_lock, line_lock);
			}
		}
	} else {
		pr_audio("%s, fe == NULL.\n", __func__);
...
#if 0 /* Instead of checking power of audio. */
	if (power >= 150)
		*state = 1;
	else
		*state = 0;
...
void set_aft_thread_enable(int enable, u32_t delay)
{
	...
	struct dtv_frontend_properties *c = &dvb->fe->dtv_property_cache;
	...
	/* c->analog.afc_range != 0 means searching, not enable afc */
	if (enable && c->analog.afc_range != 0) {
		mutex_unlock(&afc_mutex);
		return;
	}
...
int aml_get_afc_state(void)
{
	return afc_lock_status;
}
...
static void aml_fe_do_work_pre(int lock)
{
	...
	if (!lock) {
		...
		if (afc_offset == afc_range[afc_pre_step]) {
			c->frequency -= afc_range[afc_pre_step]*1000;
		...
		if (debug_fe & 0x10)
			pr_err("%s,unlock afc_offset:%d KHz, set freq:%d\n",
				__func__, afc_offset, c->frequency);
		return;
	} else {
		afc_pre_lock_cnt++;
		if (debug_fe & 0x10)
			pr_err("%s,afc_pre_lock_cnt:%d\n",
...
static void aml_fe_do_work(struct work_struct *work)
{
	...
	if (++wave_cnt <= afc_wave_cnt) {/*40ms*/
		...
		if (debug_fe & 0x4)
			pr_err("%s,wave_cnt:%d is wave,lock:%d ignore\n",
				__func__, wave_cnt, lock);
		return;
	}
...
void aml_timer_hander(unsigned long arg)
{
	...
	if ((fe == NULL) || (dvb == NULL) || (timer_init_state == 0))
		return;
...
int aml_fe_analog_set_frontend(struct dvb_frontend *fe)
{
	...
	set_aft_thread_enable(0, 0);/*afc_timer_disable();*/
	...
	else if (fee->tuner->drv->id == AM_TUNER_SI2159)
	...
	if ((FE_ANALOG == fe->ops.info.type)
			&& (analog_search_flag == 0)
			&& (c->analog.afc_range == 0)) {
		set_aft_thread_enable(1, 10);/*afc_timer_enable(fe);*/
		atvdemod_set_timer_enable(true);
	}

	if (debug_fe & 0x40)
		pr_err("%s,frequency:%d\n", __func__, p.frequency);
	return ret;
}
...
static int aml_fe_analog_get_frontend(struct dvb_frontend *fe)
{
	...
	pr_dbg("%s, p->frequency:%d, afe->params.frequency:%d.\n",
			__func__, p->frequency, afe->params.frequency);
...
static void aml_fe_get_analog_fmt(struct dvb_frontend *fe,
		int auto_search_std, v4l2_std_id *video_fmt,
		unsigned int *audio_fmt)
{
	struct dtv_frontend_properties *p = &fe->dtv_property_cache;
	struct aml_fe *fee = fe->demodulator_priv;
	int i = 0;
	...
	if (auto_search_std & 0x01) {
		for (i = 0; i < try_vfmt_cnt; i++) {
			...
			if (i == (try_vfmt_cnt / 3) ||
				 ...
				 * then, try others std.
				 */
				if (p->analog.std &	V4L2_COLOR_STD_PAL)
					p->analog.std = V4L2_COLOR_STD_NTSC
							| V4L2_STD_NTSC_M;
#if 0 /*for secam */
				else if (p->analog.std & V4L2_COLOR_STD_NTSC)
					p->analog.std = V4L2_COLOR_STD_SECAM
							| V4L2_STD_SECAM;
#endif
				else if (p->analog.std & V4L2_COLOR_STD_NTSC)
					p->analog.std = V4L2_COLOR_STD_PAL
		...
		if (cvbs_std == 0) {
			...
			p->analog.std = (V4L2_COLOR_STD_PAL | V4L2_STD_PAL_DK);
		...
		std_bk = trans_tvin_fmt_to_v4l2_std(cvbs_std);
	} else {
...
static enum dvbfe_search aml_fe_analog_search(struct dvb_frontend *fe)
{
	...
	/* int atv_cvd_format = 0, hv_lock_status = 0, snr_vale = 0; */
	v4l2_std_id std_bk = 0;
	struct aml_fe *fee = NULL;
	int audio = 0;
	int double_check_cnt = 1;
	int auto_search_std = 0;
	int search_count = 0;
#ifdef ATV_SUPPORT_SECAM_L
	...
	if (unlikely(!fe || !p ||
			...
			!fe->ops.set_frontend ||
			!fe->ops.tuner_ops.get_pll_status ||
	...
	pr_dbg("[%s] afc_range[%d], tuner type[%d], freq[%d], flag[0x%x].\n",
			__func__, p->analog.afc_range,
			fee->tuner->drv->id, p->frequency, p->analog.flag);
	...
	if (p->analog.audmode == 0) {
		if (auto_search_std)
			p->analog.audmode = p->analog.std & 0x00FFFFFF;
		else {
			if (p->analog.std & V4L2_COLOR_STD_PAL)
				p->analog.audmode = V4L2_STD_PAL_DK;
			else if (p->analog.std & V4L2_COLOR_STD_NTSC)
				p->analog.audmode = V4L2_STD_NTSC_M;
			else if (p->analog.std & V4L2_COLOR_STD_SECAM)
				p->analog.audmode = V4L2_STD_PAL_DK;

			p->analog.std = (p->analog.std & 0xFF000000)
					| p->analog.audmode;
		}
		auto_search_std |= 0x02;
		pr_dbg("[%s] user audmode is 0, so set it to %s.\n",
				__func__, v4l2_std_to_str(p->analog.audmode));
	}

	/*afc tune disable*/
	set_aft_thread_enable(0, 0);/*afc_timer_disable();*/
	analog_search_flag = 1;
	set_atvdemod_scan_mode(1);
	...
	if (slow_mode || (fee->tuner->drv->id == AM_TUNER_R840)
		...
		afc_step = p->analog.afc_range; /*ATV_AFC_2_0MHZ;*/
	} else {
		pr_dbg("[%s] slow mode to search the channel\n", __func__);
		afc_step = ATV_AFC_1_0MHZ;
	}
	...
	pr_dbg("%s Auto search user analog.std: 0x%08x\n",
			__func__, (unsigned int) p->analog.std);

#ifdef DEBUG_TIME_CUS
	time_end = jiffies_to_msecs(jiffies);
	time_delta = time_end - time_start;
	pr_dbg("[ATV_SEARCH_SET_FRONTEND]%s: time_delta_001:%d ms,afc_step:%d\n",
	...
	while (minafcfreq <= p->frequency &&
		...
		do {
			if (fee->tuner->drv->id == AM_TUNER_MXL661) {
				usleep_range(30 * 1000, 30 * 1000 + 100);
			} else if (fee->tuner->drv->id == AM_TUNER_R840) {
				usleep_range(20 * 1000, 20 * 1000 + 100);
			...
			if (tuner_status_cnt_local == 0) {
				...
				if (auto_search_std &&
					try_secam == false &&
					!(p->analog.std
						& V4L2_COLOR_STD_SECAM) &&
					!(p->analog.audmode
						& V4L2_STD_SECAM_L)) {
					/* backup std and audio mode */
					std_bk = p->analog.std;
					audio = p->analog.audmode;
		...
		if (pll_lock) {
			...
			ret = aml_fe_afc_closer(fe, minafcfreq,
					maxafcfreq + ATV_AFC_500KHZ, 1);
			if (ret == 0) {
				aml_fe_get_analog_fmt(fe, auto_search_std,
						&std_bk, &audio);
		...
		pr_dbg("[%s] freq[analog.std:0x%08x] is[%d] unlock\n",
				__func__,
				(uint32_t) p->analog.std, p->frequency);
	...
	pr_dbg("[%s] p->frequency=[%d] over of range [min=%d, max=%d], search failed.\n",
			__func__, p->frequency, minafcfreq, maxafcfreq);
	p->frequency = set_freq;
	/*afc tune enable*/
	analog_search_flag = 0;
	set_atvdemod_scan_mode(0);
	set_aft_thread_enable(1, 0);/*afc_timer_enable(fe);*/
		...
		int maxafcfreq, int isAutoSearch)
{
	...
	if (abs(c->frequency - freq_success) < 3000000
		...
		if ((0 == strcmp(hwid_val, "1000")) ||
			...
			/* beyond 5s search same frequency is ok */
			if ((time_now.tv_sec - success_time.tv_sec) < 5)
				return -1;
		} else {
	...
	if ((fe->ops.analog_ops.get_afc || fe->ops.tuner_ops.get_afc) &&
		...
		while (abs(afc) > AFC_BEST_LOCK) {
			if (fee->tuner->drv->id == AM_TUNER_SI2151 ||
				fee->tuner->drv->id == AM_TUNER_SI2159 ||
				fee->tuner->drv->id == AM_TUNER_R840)
...
/* dtv_demod0_atsc_version */
	if (type == AM_DEV_DTV_DEMOD) {
		snprintf(buf, sizeof(buf), "%s%d_atsc_version", name, id);
			     ...
			     const char *buf, size_t size)
{
	...
	if (sscanf(buf, "tuner %i driver %31s", &id, dev_name) == 2) {
...
#ifdef CONFIG_OF
	} else if (sscanf(buf, "tuner %i reset_gpio %31s", &id, gpio_name) == 2) {
		val =
	...
	} else if (sscanf(buf, "dtv_demod %i driver %31s", &id, dev_name) == 2) {
...
#ifdef CONFIG_OF
	} else if (sscanf(buf, "dtv_demod %i reset_gpio %31s", &id, gpio_name) ==
		   2) {
		val =
		    desc_to_gpio(of_get_named_gpiod_flags
				 (dvb->pdev->dev.of_node, gpio_name, 0, NULL));
	...
	} else if (strstr(buf, "audio_power")) {
		int audio_power = 0;

		retrieve_vpll_carrier_audio_power(&audio_power);
		pr_dbg("audio_power: %d.\n", audio_power);
	} else if (strstr(buf, "adc_power")) {
		int adc_power = 0;

		retrieve_adc_power(&adc_power);
		pr_dbg("adc_power:%d\n", adc_power);
	} else if (sscanf(buf, "set_frontend %d", &id) == 1) {
		struct aml_dvb *dvb = aml_get_dvb_device();
		struct dtv_frontend_properties *c = NULL;

		c = &dvb->fe->dtv_property_cache;

		pr_dbg("c->frequency       = %d.\n", c->frequency);
		pr_dbg("c->analog.soundsys = %d.\n", c->analog.soundsys);
		pr_dbg("c->analog.audmode  = %d.\n", c->analog.audmode);
		pr_dbg("c->analog.std      = 0x%x.\n",
				(unsigned int) c->analog.std);
		pr_dbg("input std          = 0x%x.\n\n", id);
		c->analog.std = (c->analog.std & 0x00FFFFFF)
				| (id & 0xFF000000);
		c->analog.audmode = id & 0x00FFFFFF;

		aml_fe_analog_set_frontend(dvb->fe);
	} else if (strstr(buf, "audio_autodet")) {
		pr_err("aml_audiomode_autodet......\n");
		amlatvdemod_set_std(AML_ATV_DEMOD_VIDEO_MODE_PROP_PAL_DK);
		aml_audiomode_autodet(dvb->fe);
	}
...
static int aml_fe_probe(struct platform_device *pdev)
{
	...
	if (ret == 0)
		pr_info("aml_fe memory resource done.\n");
	else
		pr_info("aml_fe memory resource undefined!!\n");
