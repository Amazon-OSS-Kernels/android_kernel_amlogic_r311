...
static int di_reg_unreg_cnt = 50;
...
static const char version_s[] = "2019-1-24";
...
/*change 4 to 3,for save time for provider*/
static int bypass_get_buf_threshold = 3;
...
/*
 * bit[4]: enable skip top for i input && bypass case
 ...
 */
static int prog_proc_config = (1 << 5) | (1 << 4) | (1 << 1) | 1;
/*
...
static unsigned int null_cnt = 1;
module_param_named(null_cnt, null_cnt, uint, 0644);

static void insert_null_pre(unsigned int addr)
{
	unsigned int i = 0;

	for (i = 0; i < null_cnt; i++)
		aml_write_vcbus(PRE_NULL_REG, 0);
}
...
unsigned int Rd(unsigned int adr)
{
	...
	insert_null_pre(adr);
	value = aml_read_vcbus(adr);
	insert_null_pre(adr);
	return value;
}

/*export for vpp read di regs*/
unsigned int vpp_read_di_pre_reg(unsigned int adr)
{
	return Rd(adr);
}
EXPORT_SYMBOL(vpp_read_di_pre_reg);

void Wr_reg_bits(unsigned int adr,
	unsigned int val,
	unsigned int start,
	unsigned int len)
{
	unsigned int mask, value, orig, tmp;
...
unsigned int Rd_reg_bits(unsigned int adr, unsigned int start, unsigned int len)
{
	unsigned int value = 0;

	value = (Rd(adr)&(((1<<len)-1)<<start))>>start;
	return value;
}

static void insert_null_post(unsigned int addr)
{
	unsigned int i = 0;

	for (i = 0; i < null_cnt; i++)
		VSYNC_WR_MPEG_REG(POST_NULL_REG, 0);
}
...
static unsigned int di_dbg_mask = 2;
...
void DI_VSYNC_WR_MPEG_REG(unsigned int addr, unsigned int val)
{
	if (is_need_stop_reg(addr))
		return;
	insert_null_post(addr);
	VSYNC_WR_MPEG_REG(addr, val);
	insert_null_post(addr);
	return;
}
EXPORT_SYMBOL(DI_VSYNC_WR_MPEG_REG);
	...
	unsigned int start, unsigned int len)
{
	...
	mask = ((1<<len)-1)<<start;
	value = val<<start;
	orig = DI_POST_REG_RD(addr);
	tmp = orig & ~mask;
	tmp |= (value & mask);
	DI_VSYNC_WR_MPEG_REG(addr, tmp);
	return;
}
...
int DI_POST_WR_REG_BITS(u32 adr, u32 val, u32 start, u32 len)
{
	...
	mask = ((1<<len)-1)<<start;
	value = val<<start;
	orig = DI_POST_REG_RD(adr);
	tmp = orig & ~mask;
	tmp |= (value & mask);
	DI_VSYNC_WR_MPEG_REG(adr, tmp);
	return 1;
}
...
#endif
unsigned int di_debug_flag = 0;/* enable rdma even di bypassed */
...
static DEVICE_ATTR(debug, 0220, NULL, store_dbg);
static DEVICE_ATTR(dump_pic, 0220, NULL, store_dump_mem);
...
#define MAX_LOCAL_BUF_NUM         9
#define MAX_LOCAL_PROG_BUF_NUM    4
...
static int dump_di_post_stru_seq(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "%-25s = %d\n", "run_early_proc_fun_flag",
		di_post_stru.run_early_proc_fun_flag);
	seq_printf(seq, "%-25s = %d\n", "cur_disp_index",
		di_post_stru.cur_disp_index);
	seq_printf(seq, "%-25s = %d\n", "post_de_busy",
		di_post_stru.post_de_busy);
	seq_printf(seq, "%-25s = %d\n", "de_post_process_done",
		di_post_stru.de_post_process_done);
	seq_printf(seq, "%-25s = 0x%p\n,", "cur_post_buf",
		di_post_stru.cur_post_buf);
	       ...
	       const char *buf, size_t len)
{
	...
	if ((parm[1] != NULL) && (unlikely(di_buf == NULL))) {
		kfree(buf_orig);
		pr_err("di_buf is NULL!!\n");
		return len;
	}
...
/* pr_dbg("dump path =%s\n",dump_path); */
		...
		if (de_devp->flags && DI_MAP_FLAG) {
			if (de_devp->flag_cma == 3 || de_devp->flag_cma == 4)
				buff = codec_mm_phys_to_virt(di_buf->nr_adr);
			else
...
#endif
static bool is_mpegx_source(struct vframe_s *vf)
{
	if ((vf->video_format == VFRAME_FORMAT_MPEG12) ||
		(vf->video_format == VFRAME_FORMAT_MPEG4))
		return true;
	return false;
}
...
/*disable di post hw for mirror disable*/
static void di_mirror_disable(void)
{
...
#define VFRAME_FORMAT_MASK      \
	...
	(vframe->type & VIDTYPE_VIU_FIELD))
		...
		if ((vframe->type & VIDTYPE_INTERLACE) &&
			!(di_pre_stru.cur_inp_type & VIDTYPE_INTERLACE) &&
			!is_mpegx_source(vframe)) {
			pr_dbg("DI P->I.\n");
			return 3;
		}
 ...
 */
#define VIDTYPE_DI	\
	((VIDTYPE_VIU_422 | VIDTYPE_VIU_SINGLE_PLANE) | \
	(VIDTYPE_VIU_FIELD | VIDTYPE_PRE_INTERLACE))

static unsigned int bypass_mask;
module_param_named(bypass_mask, bypass_mask, uint, 0644);
...
static unsigned char is_bypass(vframe_t *vf_in)
{
	...
	if (di_pre_stru.cur_prog_flag &&
	    ((bypass_prog) ||
	     (di_pre_stru.cur_width > 1920) || (di_pre_stru.cur_height > 1088)
	...
	if (vf_in && vf_in->error_flag
		&& vf_in->video_format == VFRAME_FORMAT_MPEG12) {
		bypass_mask |= 8;
		return 1;
	}
/*prot is conflict with di post*/
	...
	if ((di_vscale_skip_enable & 0x4) && vf_in) {
		vf_tmp.type = VIDTYPE_DI;
		vf_tmp.ratio_control = vf_in->ratio_control;
		vf_tmp.source_type = vf_in->source_type;
		if (vf_in->type & VIDTYPE_COMPRESS) {
			...
			vf_tmp.width = vf_in->width;
			vf_tmp.height = vf_in->height;
		}
		ret = get_current_vscale_skip_count(&vf_tmp);
		...
		if (di_vscale_skip_count > 0 ||
			...
			if (di_dbg_mask & 4) {
				pr_info("bypass for skip cnt %d 0x%x/%u/%u.",
					ret, vf_tmp.type,
					vf_tmp.width, vf_tmp.height);
			}
			return 1;
			}
	}
	...
	int mcinfo_canvas_idx)
{
	...
	if (!di_buf)
		return;
	/* ensure canvas width 32 align */
	mcinfo_width = roundup(di_buf->canvas_height, 32);
...
#endif
#ifdef CONFIG_CMA
unsigned int di_cma_alloc_total(struct di_dev_s *de_devp)
{
	de_devp->total_pages =
	...
	if (de_devp->total_pages) {
		...
		pr_info("%s:xxxxxx DI CMA allocate fail.\n", __func__);
		return 0;
	}

}

bool di_free_mirror_mem(void)
{
	bool ret;

	ret = (used_post_buf_index == -1);
	return ret;
}
EXPORT_SYMBOL(di_free_mirror_mem);

bool di_force_free_mem(void)
{
	unsigned int i  = 0;

	/*return for init check*/
	if (init_flag || (de_devp == NULL) || (di_sema_init_flag == 0) ||
		(used_post_buf_index == -1)) {
		pr_dbg("no free(%d,0x%p,%d,%d).\n", init_flag, de_devp,
			di_sema_init_flag, used_post_buf_index);
		return false;
	}
	...
	if (((de_devp->flag_cma == 1) ||
		...
		up(&di_sema);
		pr_info("di force free mirror frame mem.\n");
	} else {
		pr_dbg("di unreg flow undone now can't free.\n");
		return false;
	}
#endif
	return true;
}
EXPORT_SYMBOL(di_force_free_mem);
...
void di_cma_release(void)
{
	...
	for (i = 0; (i < local_buf_num); i++) {
		...
		if ((ii >= USED_LOCAL_BUF_MAX) &&
			(buf_p->pages != NULL)) {
			if ((de_devp->flag_cma == 3) ||
				(de_devp->flag_cma == 4)) {
				codec_mm_free_for_dma(DEVICE_NAME,
					buf_p->nr_adr);
				buf_p->pages = 0;
				buf_p->nr_adr = 0;
				rels_cnt++;
				if (cma_print)
					pr_info("DI codec_mm release buf[%d] ok.\n",
							i);
			} else {
			...
			if (cma_print)
				pr_dbg("DI buf[%d] page:0x%p no release.\n",
...
{
	unsigned int start_time, end_time, delta_time, ii;
	struct di_buf_s *buf_p = NULL;
	int itmp, alloc_cnt = 0;
	int flags = 0;
	bool out_of_mem_flag = false;
	bool istvp = false;
	...
	queue_for_each_entry(buf_p, ptmp, QUEUE_LOCAL_FREE, list) {
		if ((de_devp->flag_cma == 3) || (de_devp->flag_cma == 4)) {
			if (de_devp->flag_cma == 4 && !istvp)
				flags = CODEC_MM_FLAGS_CMA_FIRST |
					CODEC_MM_FLAGS_CPU;
			if (buf_p->pages == NULL) {
				buf_p->nr_adr = codec_mm_alloc_for_dma(
					DEVICE_NAME,
					de_devp->buffer_size >> PAGE_SHIFT,
					0, flags);
				if (buf_p->nr_adr == 0) {
					buf_p->pages = NULL;
					out_of_mem_flag = true;
					pr_err("xxxxxxxxx DI codec_mm allocate %d %d fail.flags %d\n",
						buf_p->index,
						de_devp->buffer_size, flags);
					break;
				} else {
					alloc_cnt++;
					if (istvp) {
						buf_p->pages =
						(struct page *) TVP_MEM_PAGES;
					} else {
						buf_p->pages =
					codec_mm_phys_to_virt(buf_p->nr_adr);
					}
					if (cma_print)
						pr_info("DI CMA  allocate buf[%d]page:0x%p addr 0x%lx\n",
								buf_p->index,
								buf_p->pages,
								buf_p->nr_adr);
				}
			...
			if (cma_print)
				pr_info("nr addr 0x%lx ok.\n", buf_p->nr_adr);
		}

		if (di_pre_stru.buf_alloc_mode == 0 &&
				di_pre_stru.mtn_size != 0) {
...
#endif
			if (cma_print) {
				pr_info("buf[%d] mtn_adr 0x%lx cnt_adr 0x%lx\n",
					buf_p->index, buf_p->mtn_adr,
					buf_p->cnt_adr);
			}
			if (mc_mem_alloc) {
				...
				buf_p->mcinfo_adr =
					...
					di_pre_stru.mv_size;
				if (cma_print) {
					pr_info("\tmv_adr 0x%lx mc_adr 0x%lx\n",
						buf_p->mcvec_adr,
						buf_p->mcinfo_adr);
				}
			}
			...
			buf_p->cnt_adr = buf_p->nr_adr;
			buf_p->mcvec_adr = buf_p->nr_adr;
			buf_p->mcinfo_adr = buf_p->nr_adr;
			if (cma_print) {
				pr_info("%s set nr_adr to ma&mc.\n",
					__func__);
			}
		}
	}
	if (out_of_mem_flag) {
		...
		end_time = jiffies_to_msecs(jiffies);
		delta_time = end_time - start_time;
		pr_info("%s: ++++ alloc %u*%d buffer use %u ms(%u~%u) +++++\n",
			__func__, alloc_cnt, de_devp->buffer_size,
			delta_time, start_time, end_time);
	}
	if (used_post_buf_index != -1)  {
		for (ii = 0; ii < USED_LOCAL_BUF_MAX; ii++) {
			if (used_local_buf_index[ii] != -1)
				alloc_cnt++;
		}
	}
	if (alloc_cnt == local_buf_num) {
		enough_mem = true;
		return 1;
	} else {
		enough_mem = false;
...
#endif
/* calculate how many memory di need according
 * to max fmt 1920x1080
 */
static unsigned int get_di_buf_size(
	...
	unsigned char prog_flag)
{
	...
	/* make sure canvas width must be divided by 256bit|32byte align */
	if (prog_flag == 1) {
		buf_size = nr_width * canvas_height * 2;
		buf_size = roundup(buf_size, PAGE_SIZE);
		buf_num = de_devp->mem_size / buf_size;
		if (buf_num > MAX_LOCAL_PROG_BUF_NUM)
			buf_num = MAX_LOCAL_PROG_BUF_NUM;
	} else {
		...
		if (prog_flag == 0) {
			...
			mc_size = roundup(canvas_height, 32);
		}
		if (mcpre_en) {
			buf_size = nr_size + mtn_size + count_size +
				mv_size + mc_size;
		} else {
#ifdef NEW_DI_V1
			buf_size = nr_size + mtn_size + count_size;
#else
			buf_size = nr_size + mtn_size;
#endif
		}
		buf_size = roundup(buf_size, PAGE_SIZE);
		buf_num = de_devp->mem_size / buf_size;
		...
		if (post_wr_en && post_wr_surpport) {
			buf_num = (de_devp->mem_size +
				(nr_width*canvas_height<<2)) /
				(buf_size + (nr_width*canvas_height<<1));
		}

		if (buf_num > MAX_LOCAL_BUF_NUM)
			buf_num = MAX_LOCAL_BUF_NUM;
	}
	for (itmp = 0; (itmp < MAX_LOCAL_BUF_NUM*2); itmp++) {
		buf_p = &(di_buf_local[itmp]);
			if (buf_p->pages)
				buf_num--;
	}
...
static int di_init_buf(int width, int height, unsigned char prog_flag)
{
	...
	if (prog_flag == 1) {
		...
		if (local_buf_num > MAX_LOCAL_PROG_BUF_NUM)
			local_buf_num = MAX_LOCAL_PROG_BUF_NUM;
	...
	for (i = 0; i < local_buf_num; i++) {
		...
		if (ii >= USED_LOCAL_BUF_MAX) {
			...
			di_buf->buffer_size = de_devp->buffer_size;
...
static void dump_vframe(vframe_t *vf)
{
	...
	pr_info("width %d, height %d, compw %u comph %u, orientation 0x%x\n",
		vf->width, vf->height, vf->compWidth,
		vf->compHeight, vf->orientation);
...
static void print_di_buf(struct di_buf_s *di_buf, int format)
{
	...
	if (format == 1) {
		...
		"\t+index %d, 0x%p, type %d, vframetype 0x%x\n",
			...
			di_buf->type,
			di_buf->vframe->type);
		pr_info("\t+trans_fmt %u,bitdepath %d pages %p\n",
		...
		pr_info("index %d, 0x%p(vframe 0x%p), type %d pages %p\n",
			di_buf->index, di_buf,
...
static void dump_state(void)
{
	...
	pr_info("buffer_size=%d, mem_flag=%d, cma_flag=%d\n",
		...
		de_devp->flag_cma);
	pr_info("new_keep_last_frame_enable %d, hold_video %d",
		new_keep_last_frame_enable, hold_video);
...
static int seq_file_di_state_show(struct seq_file *seq, void *v)
{
	...
	dump_state_flag = 1;
	seq_printf(seq, "version %s, provider vframe level %d,\n",
		version_s, provider_vframe_level);
	seq_printf(seq, "init_flag %d, is_bypass %d, receiver_is_amvideo %d\n",
		init_flag, is_bypass(NULL), receiver_is_amvideo);
...
seq_printf(seq, "recovery_log_queue_idx=%d, recovery_log_di_buf=0x%p\n",
	...
	seq_printf(seq, "buffer_size=%d, mem_flag=%d, cma_flag=%d\n",
		...
		de_devp->flag_cma);
	/*diff with trunk*/
	seq_printf(seq, "new_keep_last_frame_enable %d, hold_video %d\n",
		new_keep_last_frame_enable, hold_video);
...
"index %d, 0x%p, type %d, vframetype 0x%x pre_ref_count %d post_ref_count %d\n",
	...
	seq_puts(seq, "\n");
	seq_printf(seq, "vf_peek()=>0x%p,di_process_cnt = %d\n",
		vf_peek(VFM_NAME), di_process_cnt);
	seq_printf(seq, "video_peek_cnt = %d,force_trig_cnt = %d\n",
		video_peek_cnt, force_trig_cnt);
	seq_printf(seq, "reg_unreg_timerout = %lu\n", reg_unreg_timeout_cnt);
/*diff with trunk seq_printf(seq, "combing version: %s\n", combing_ver_s);*/
	dump_state_flag = 0;
	return 0;
}
...
static void pre_de_process(void)
{
	...
	if (di_pre_stru.input_size_change_flag) {
		di_pre_size_change(pre_width, pre_height,
			di_pre_stru.di_inp_buf->vframe->video_format);
...
/* field num for chan2. 1 bottom, 0 top. */
	...
	if (di_pre_stru.cur_prog_flag == 1 || di_pre_stru.enable_mtnwr == 0) {
		...
		RDMA_WR_BITS(DI_MTN_CTRL1, 0, 12, 2);
		RDMA_WR(DI_INTR_CTRL,
			/* mask nrwr interrupt. */
			(2 << 16) |
			/* mask diwr intrpt. */
			(((post_wr_en && post_wr_surpport)?0:1) << 18) |
			(0x1f << 19) | /* mask hist check interrupt. */
			#ifdef DET3D
		...
		RDMA_WR(DI_INTR_CTRL,
			...
			(0 << 21) | /* unmask mcdi interrupt. */
			(0 << 22) | /* unmask mcdi interrupt. */
			(0 << 23) | /* unmask mcdi interrupt. */
			#ifdef DET3D
			((det3d_en ? 0 : 1) << 24) |
			#else
			(1 << 24) | /* mask det3d interrupt. */
	...
	/*di_arb_sw(true);*/	/*2019-01-22 by VLSI feng.wang*/
	di_unlock_irqfiq_restore(irq_flag2);

	/*reinit pre busy flag*/
	t_cur = sched_clock();
	do_div(t_cur, NSEC_PER_MSEC);

	di_pre_stru.pre_de_busy_timer_count = 0;
	di_pre_stru.pre_de_busy = 1;
	di_pre_stru.irq_time[0] = t_cur;
	di_pre_stru.irq_time[1] = t_cur;
#ifdef CONFIG_AML_RDMA
...
{
	int i = 0;
	/* modify for ccitt033.ts pq issue */
	if ((codec_type == VFRAME_FORMAT_MPEG12) ||
		(codec_type == VFRAME_FORMAT_MPEG4)) {
		if (width < 720)
			combing_glbmot_radprat[3] = 39;
		else
			combing_glbmot_radprat[3] = 75;
	} else
		combing_glbmot_radprat[3] = 80;
	...
	if (is_meson_gxtvbb_cpu() && dejaggy_enable) {
		...
		if ((frame_type & VIDTYPE_TYPEMASK) == VIDTYPE_PROGRESSIVE &&
			...
			if (dejaggy_enable) {
				...
				dejaggy_flag =
				combing_dejaggy_setting[min(cur_lev, 4)];
...
module_param_named(time_debug, time_debug, bool, 0644);
...
#define WR_CANVAS_WID                                   8
		...
		if (vframe == NULL)
			...
			if (di_pre_stru.in_seq < 6 && time_debug) {
				pr_info("DI %ums %d frame coming.\n",
					jiffies_to_msecs(jiffies_64),
					di_pre_stru.in_seq);
			}
...
jiffies_to_msecs(jiffies_64 - vframe->ready_jiffies64));
		...
		di_buf->vframe->private_data = di_buf;
		/* clear mem box handle for di'mem no mem box */
		...
		if (change_type == 1 || (change_type == 2 &&
			...
			pr_info(
			"%s:%d source change: 0x%x/%d/%d/%d=>0x%x/%d/%d/%u/%u/%d/%d/%d\n",
				__func__,
				di_pre_stru.in_seq,
				di_pre_stru.cur_inp_type,
				di_pre_stru.cur_width,
				...
				di_buf->vframe->source_type,
				di_buf->vframe->video_format,
				(di_buf->vframe->bitdepth&BITDEPTH_Y10)?10:8);
			...
			/*for P -> I,force process as p @20180330 new add*/
			di_buf->vframe->type = di_pre_stru.cur_inp_type;
		...
		if (is_bypass(di_buf->vframe)) {
			...
			/*@20180205 new add policy for hdmiin bypass case*/
			if ((di_vscale_skip_enable & (1 << 4)) &&
				(di_buf->vframe->source_type ==
	...
	if (di_buf->di_wr_linked_buf)
		di_print("%s: linked %s[%d] => di_wr_buf\n", __func__,
			...
			di_buf->di_wr_linked_buf->index);
#endif
	di_pre_stru.di_inp_buf->vframe->mem_handle = NULL;
	di_pre_stru.di_inp_buf->vframe->mem_head_handle = NULL;
	...
	if (di_pre_stru.di_inp_buf->vframe->height > 1080)
		di_pre_stru.di_inp_buf->vframe->height = 1080;
	...
	if ((pulldown_win_mode & 0xfffff) != 0) {
		for (ii = 0; ii < 5; ii++) {
			...
			if (mode == 1) {
				...
				dup1_wn_fdiff =
					di_buf->di_buf_dup_p[1]->
				win_pd_info[min(ii + 1, 4)].field_diff_num;
				dup1_wc_fdiff =
					di_buf->di_buf_dup_p[1]->
					win_pd_info[ii].field_diff_num;
				dup1_wp_fdiff =
...
void drop_frame(int check_drop, int throw_flag, struct di_buf_s *di_buf)
{
	...
	if (drop_flag) {
		...
		if (disp_frame_count == 0 && time_debug) {
			pr_info("DI %ums send 1st frame to post ready.\n",
				jiffies_to_msecs(jiffies_64));
		}
...
/* nr/blend0/ei0/mtn0 clock gate */
	...
	if (mirror_disable) {
		...
		pr_info("%s disable di mirror image %u.\n", __func__,
			mirror_disable);
	} else {
		pr_info("%s enable di mirror image %u.\n", __func__,
			mirror_disable);
	}
	if ((post_wr_en && post_wr_surpport)) {
		diwr_set_power_control(0);
	...
	unsigned short codec_type)
{
	...
	combing_threshold_config(width, codec_type);
...
static bool need_bypass(struct vframe_s *vf)
{
	...
	if ((vf->flag & VFRAME_FLAG_GAME_MODE) &&
		(vf->width > 720))
		return true;
	/* true bypass for afbc 1920 || 1088 input */
	if ((vf->type & VIDTYPE_COMPRESS) &&
		((vf->compWidth > default_width) ||
		(vf->compHeight > default_height + 8)))
		return true;
...
static void di_reg_process_irq(void)
{
	...
	unsigned int buf_size, free_size, check_cnt;
	...
	if (!IS_ERR_OR_NULL(vframe)) {
		if (need_bypass(vframe) ||
			((di_debug_flag>>20) & 0x1) || !enough_mem) {
			...
			return;
		} else if (de_devp->flag_cma == 3 ||
				de_devp->flag_cma == 4) {
			prog_flag = (is_progressive(vframe) &&
						(prog_proc_config & 0x10));
			if (vframe->video_format == VFRAME_FORMAT_MPEG12) {
				prog_flag = false;
				check_cnt = (((vframe->duration)/96<<1)+9)/10;
				nr_done_check_cnt =
				(check_cnt >= 5) ? check_cnt : 10;
				pr_info("nr done check cnt %u.\n",
					nr_done_check_cnt);
			} else
				nr_done_check_cnt = 10;
			buf_size = get_di_buf_size(default_width,
					default_height, prog_flag ? 2 : 0);
			free_size = codec_mm_get_free_size();
			if (free_size < buf_size) {
				enough_mem = false;
				if (!di_pre_stru.bypass_flag) {
					pr_info("======di no mem %ux%u-0x%x, bypass all %u-%u=======",
						vframe->width, vframe->height,
					vframe->type, buf_size, free_size);
				}
				bypass_mask |= 0x40;
				di_pre_stru.bypass_flag = true;
				return;
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TXLX)) {
			...
			/* need keep mif disable before mif config ready */
			enable_di_pre_mif(false);
		...
		if (is_progressive(vframe) && is_from_vdin(vframe) &&
			(prog_proc_config & 0x10)) {
			/* pre only clkb upto 500M*/
			 ...
			 * smoothly.
			 */
			di_init_buf(default_width, default_height,
					(prog_flag ? 2 : 0));
...
static void di_pre_trigger_work(struct di_pre_stru_s *pre_stru_p)
{
	...
	if (pre_stru_p->pre_de_busy && init_flag) {
		...
		if ((pre_stru_p->pre_de_busy_timer_count >=
			nr_done_check_cnt) &&
			((get_current_time_ms() -
			di_pre_stru.irq_time[1]) > (10*nr_done_check_cnt))) {
			if ((pre_stru_p->field_count_for_cont < 10) ||
				(di_dbg_mask&0x2)) {
				pr_info("DI*****wait %d timeout 0x%x(%d ms)S*****\n",
			...
			if ((pre_stru_p->field_count_for_cont < 10) ||
				(di_dbg_mask&0x2)) {
				pr_info("DI*****wait %d timeout 0x%x(%d ms)E*****\n",
					pre_stru_p->field_count_for_cont,
					Rd(DI_INTR_CTRL),
					(unsigned int)(get_current_time_ms() -
					di_pre_stru.irq_time[1]));
	...
	if (force_recovery) {
		if (recovery_flag || (force_recovery & 0x2)) {
			...
			if (init_flag) {
				pr_err("======DI force recovery %d:%d======\n",
					recovery_flag, recovery_log_reason);
				pr_err("recovery queue %d, buf[%d]\n",
					recovery_log_queue_idx,
					recovery_log_di_buf->index);
				force_recovery &= (~0x2);
				recovery_flag = 0;
			}
		}
...
static int di_task_handle(void *data)
{
	...
	while (1) {
		...
		if (active_flag) {
			...
			#endif
			/*
			 * load pq table after mem allocated
			 * ensure pq load after di_pre_size_change config done
			 */
			if ((de_devp->flags & DI_LOAD_REG_FLAG) &&
				atomic_read(&devp->mem_flag) &&
				init_flag && field_count) {
...
static int di_receiver_event_fun(int type, void *data, void *arg)
{
	...
	if (type == VFRAME_EVENT_PROVIDER_QUREY_VDIN2NR) {
		...
		while (di_pre_stru.unreg_req_flag ||
			di_pre_stru.cma_alloc_done) {
			usleep_range(10000, 10001);
			if (di_pre_stru.unreg_req_flag_cnt++ >
				di_reg_unreg_cnt) {
				reg_unreg_timeout_cnt++;
				pr_err("%s:unreg_reg_flag timeout %d %d!!!\n",
					__func__, di_pre_stru.unreg_req_flag,
					di_pre_stru.cma_alloc_done);
				di_unreg_process();
				break;
			}
...
#define INPUT2PRE_2_BYPASS_SKIP_COUNT   4
		...
		pr_info("%s VFRAME_EVENT_PROVIDER_REG %s.\n", __func__,
			provider_name);
		if (reg_flag) {
...
static void fast_process(void)
{
	...
	if (queue_empty(QUEUE_RECYCLE))
		return;
	if (active_flag && is_bypass(NULL) &&
...
void recycle_keep_buffer(void)
{
	...
	if ((used_post_buf_index != -1) && (new_keep_last_frame_enable)) {
		if (di_buf_post[used_post_buf_index].type == VFRAME_TYPE_POST) {
			...
			di_lock_irqfiq_save(irq_flag2);
			for (i = 0; i < USED_LOCAL_BUF_MAX; i++) {
				di_buf = di_buf_post[used_post_buf_index].
					di_buf_dup_p[i];
				if (!IS_ERR_OR_NULL(di_buf)) {
					if (di_buf->buffer_size ==
						de_devp->buffer_size) {
						queue_in(di_buf, QUEUE_RECYCLE);
						pr_dbg(" %d ", di_buf->index);
					} else {
						pr_err("%s: cur(%u)!=old(%u) skip %d.\n",
							__func__,
							de_devp->buffer_size,
							di_buf->buffer_size,
							di_buf->index);
...
get_vframe:
	...
	if (disp_frame_count == 1 && time_debug) {
		pr_info("DI %ums first frame output.\n",
			jiffies_to_msecs(jiffies_64));
	}
	if (vframe_ret) {
...
/* int itmp = 0; */
	...
	if ((used_post_buf_index != -1) &&
		(used_post_buf_index != di_post_stru.cur_disp_index)) {
		pr_info("[DI]recycle buffer %d, get cnt %d.\n",
			used_post_buf_index, disp_frame_count);
		recycle_keep_buffer();
	}
...
static struct di_pq_parm_s *di_pq_parm_create(struct am_pq_parm_s *pq_parm_p)
{
	...
	if (pq_parm_p->table_len >= TABLE_LEN_MAX) {
		kfree(pq_ptr);
		pr_err("[DI] load 0x%x wrong pq table_len.\n",
			pq_parm_p->table_len);
		return NULL;
	}
	mem_size = (unsigned long)sizeof(struct am_reg_s)*
		pq_parm_p->table_len;
...
static long di_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AMDI_IOC_SET_PQ_PARM:
		cpy_size = (unsigned long)sizeof(struct am_pq_parm_s);
		if (copy_from_user((void *)&tmp_pq_s, argp, cpy_size)) {
		...
		if (tmp_pq_s.table_name & tab_flag) {
			pr_info("[DI] load 0x%x pq table len %u %s.\n",
				...
				(init_flag && field_count)?"directly":"later");
		} else {
		...
		cpy_size = (unsigned long)(tmp_pq_s.table_len *
			sizeof(struct am_reg_s));
		if (copy_from_user((void *)di_pq_ptr->regs, argp, cpy_size)) {
			di_pq_parm_destory(di_pq_ptr);
			pr_err("[DI] user copy pq table errors\n");
			return -EFAULT;
		}
		/* ensure pq load after di_pre_size_change config done */
		if (init_flag && field_count) {
			di_load_regs(di_pq_ptr);
			nr_load_regs(di_pq_ptr);
			di_pq_parm_destory(di_pq_ptr);
		} else {
...
static void set_di_flag(void)
{
	if (is_meson_gxtvbb_cpu() || is_meson_txl_cpu() ||
		...
		di_vscale_skip_enable = is_meson_txlx_cpu()?0x1c:4;
...
/* const void *name = NULL; */
	...
	if (di_devp->flag_cma >= 1) {
		...
		pr_info("DI: CMA size 0x%x type %d.\n",
			di_devp->mem_size, di_devp->flag_cma);
...
module_param_named(enough_mem, enough_mem, bool, 0644);
