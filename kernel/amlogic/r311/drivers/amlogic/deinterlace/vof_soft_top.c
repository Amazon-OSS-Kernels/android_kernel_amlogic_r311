		...
		struct sFlmSftPar *pPar, int nROW, int nCOL, bool reverse)
{
	...
	static int cmb22_nocmb_num;
	static int cmb32_blw_wnd_rel;
	/* UINT8 *PREWV = pRDat.pFlg32; or pRDat.pFlg22 */
	/* static int TCNm[HISCMBNUM]; history: the number of combing-rows */
	static int *TCNm;

	static UINT8 *pFlg32;	/* [HISDETNUM]; //history information */
	static UINT8 *pMod32;	/* [HISDETNUM]; */
	static UINT8 *pFlg22;
	static UINT8 *pMod22;
	static UINT8 *mNum32;
	static UINT32 *DIF02; /* windows */

	static int VOFWnd[2 * VOFWNDNUM]; /* VOF windows 5*(bgn/end) */
	static int nCSum;		/* Combine sum */
	static int nWCmb; /* Total Window combing */
	static int nBCmb; /* combing line of below */

	static UINT32 nCb32;

	static UINT32 nRCmbAd[ROWCMBLEN];
	static int nDif02Min;
	static int nT0;
	static int nT1;
	static int nT2;
	static int nT3;
	...
	cmb22_nocmb_num = pPar->cmb22_nocmb_num;
	cmb32_blw_wnd_rel = cmb32_blw_wnd;
	TCNm = pRDat->TCNm;

	pFlg32 = pRDat->pFlg32;	/* [HISDETNUM]; //history information */
	pMod32 = pRDat->pMod32;	/* [HISDETNUM]; */
	pFlg22 = pRDat->pFlg22;
	pMod22 = pRDat->pMod22;
	mNum32 = pRDat->mNum32;
	DIF02 = pRDat->rROFrmDif02; /* windows */

	for (nT0 = 0; nT0 < 2 * VOFWNDNUM; nT0++)
		VOFWnd[nT0] = 0;

	nCSum = 0;		/* Combine sum */
	nWCmb = 0; /* Total Window combing */
	nBCmb = 0; /* combing line of below */
	...
	nDif02Min = ((nROW * nCOL) >> cmb32_min02_sft);
	nT0 = 0;
	nT1 = 0;
	nT2 = 0;
	nT3 = 0;
...
{
	static int rCmbRwMinCt0; /* 8; //for film 3-2 */
	static int rCmbRwMinCt1; /* =7; //for film 2-2 */
	/* static int rCmbRwMaxStp=1; //fill in the hole */
	static int rCmbRwMinCt;
	static int nCSUM;	/* Combing sum (nCSUM>rCmbRwMinCt0) */
	static int nMIN;
	static int nT0;
	static int nT1;
	static int nT2;
	...
	static int nEnd;
	static int fEND;
	static int pIDx[VOFWNDNUM + 1][2];	/* Maximum-5windows */
	static int nIDx;

	rCmbRwMinCt0 = 0; /* 8; //for film 3-2 */
	rCmbRwMinCt1 = 0; /* =7; //for film 2-2 */
	rCmbRwMinCt = 0;
	nCSUM = 0;	/* Combing sum (nCSUM>rCmbRwMinCt0) */
	nMIN = 0;
	nT0 = 0;
	nT1 = 0;
	nT2 = 0;
	...
	nEnd = 0;
	fEND = 0;
	for (nIDx = 0; nIDx <= VOFWNDNUM; nIDx++)
		pIDx[nIDx][0] = pIDx[nIDx][1] = 0;
	nIDx = 0;
