...
static void pre_hold_block_mode_config(void)
{
	if (is_meson_txlx_cpu()) {
		...
		DI_Wr(DI_PRE_HOLD, (1 << 31) | (31 << 16) | 63);
	...
	unsigned char mem, unsigned char chan2, unsigned char nrwr)
{
	...
	#if 0/*move to enable_di_pre_aml control*/
...
{
	unsigned int hist_check_only = 0, chan2_en, mem_en;
	...
	chan2_en = (pd22_check_en | hist_check_only) &
				(di_chan2_mif->nocompress);
	mem_en = di_mem_mif->nocompress;
...
void enable_afbc_input(struct vframe_s *vf)
{
	...
	if (vf->type & VIDTYPE_COMPRESS) {
		...
		RDMA_WR(VD2_AFBC_MODE, r);
		RDMA_WR(VD2_AFBC_CONV_CTRL, 0x100);
		u = (vf->bitdepth >> (BITDEPTH_U_SHIFT)) & 0x3;
		v = (vf->bitdepth >> (BITDEPTH_V_SHIFT)) & 0x3;
		RDMA_WR(VD2_AFBC_DEC_DEF_COLOR,
			0x3FF00000 | /*Y,bit20+*/
			0x80 << (u + 10) |
			0x80 << v);
		/* chroma formatter */
		RDMA_WR(VD2_AFBC_VD_CFMT_CTRL,
		...
		RDMA_WR(VD2_AFBC_VD_CFMT_W,
			(w_aligned << 16) | (w_aligned/2));
		RDMA_WR(VD2_AFBC_MIF_HOR_SCOPE,
			(0 << 16) | ((w_aligned>>5) - 1));
		RDMA_WR(VD2_AFBC_MIF_VER_SCOPE,
		    (0 << 16) | ((h_aligned>>2) - 1));

		RDMA_WR(VD2_AFBC_PIXEL_HOR_SCOPE,
			(0 << 16) | (vf->width - 1));
		RDMA_WR(VD2_AFBC_VD_CFMT_H, out_height);

		RDMA_WR(VD2_AFBC_PIXEL_VER_SCOPE,
			0 << 16 | (vf->height-1));
		RDMA_WR(VD2_AFBC_SIZE_IN, h_aligned | w_aligned << 16);
		RDMA_WR(VD2_AFBC_SIZE_OUT, out_height | w_aligned << 16);
		RDMA_WR(VD2_AFBC_HEAD_BADDR, vf->compHeadAddr>>4);
		RDMA_WR(VD2_AFBC_BODY_BADDR, vf->compBodyAddr>>4);
		/* disable inp memory */
...
25); /* KDEINT */
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXBB) {
		...
		if ((DI_POST_REG_RD(VIU_MISC_CTRL0) & 0x50000) != 0x50000) {
...
void disable_post_deinterlace_2(void)
{
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXBB) {
		...
		if ((DI_POST_REG_RD(VIU_MISC_CTRL0) & 0x50000) != 0)
...
{
	/*always on*/
	DI_Wr(VIUB_GCLK_CTRL0, 0xff01);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL1, 0xaaaaaaaa);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL2, 0xaaaaaaaa);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL3, 0xaaaaaaaa);/*3:*/
}
...
void di_pre_gate_control(bool gate)
{
	if (gate) {
		...
		/* freerun VD2 AFBC */
		DI_Wr_reg_bits(VIU_MISC_CTRL1, 0x3f, 2, 6);
		DI_Wr_reg_bits(VIU_MISC_CTRL1, 3, 26, 2);
	}
	nr_gate_control(gate);
	DI_Wr(VIUB_GCLK_CTRL0, 0xff01);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL1, 0xaaaaaaaa);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL2, 0xaaaaaaaa);/*3:*/
	DI_Wr(VIUB_GCLK_CTRL3, 0xaaaaaaaa);/*3:*/
...
{
	bool ret = false;
	if ((RDMA_RD(VPU_ARB_DBG_STAT_L1C1) & PRE_ID_MASK) == PRE_ID_MASK)
		ret = true;
...
void di_arb_sw(bool on)
{
	...
	if (on) {
		...
		RDMA_WR(VPU_RDARB_REQEN_SLV_L1C1, 0xffff);
	} else {
		/*close arb:*/
		RDMA_WR(VPU_WRARB_REQEN_SLV_L1C1, 0x2b);
		RDMA_WR(VPU_RDARB_REQEN_SLV_L1C1, 0xf1f1);

		di_pre_nr_enable(false); /*by Feijun*/
		/*check status*/
		if (!di_pre_idle()) {
			pr_err("di:err1:0x[%x]\n",
				RDMA_RD(VPU_ARB_DBG_STAT_L1C1));
			for (i = 0; i < 9; i++) {
				if (di_pre_idle())
					break;
			}
...
void di_load_regs(struct di_pq_parm_s *di_pq_ptr)
{
	...
	unsigned int ctrl_regs[SKIP_CTRE_NUM] = {0x1707, 0x1718, 0x1740,
		0x2f03, 0x2f04, 0x2f70};
...
void nr_load_regs(struct di_pq_parm_s *di_pq_ptr)
{
	...
	unsigned int table_name = 0, nr_table = 0;
	bool ctrl_reg_flag = false;
	unsigned int nr_ctrl_regs[4] = {DI_CLKG_CTRL, DI_NR_CTRL0,
		MCDI_MOTINEN, MCDI_CTRL_MODE};
	struct am_reg_s *regs_p;
	...
	for (i = 0; i < len; i++) {
		...
		if (table_name & nr_table)
			ctrl_reg_flag = set_nr_skip_regs(addr, value);
		if (!ctrl_reg_flag && value != Rd(addr)) {
			if (pq_load_dbg == 2) {
				pr_info("nr [%u][0x%x] = [0x%x]&[0x%x]\n",
					i, addr, value, mask);
			}
			DI_Wr(addr, value);
		}
