		...
		unsigned short width, unsigned short height)
{
	...
	DI_Wr_reg_bits(DNR_CTRL, 1, 16, 1);
		...
		unsigned short width, unsigned short height)
{
	...
	/* enable nr4 freerun nr4 clk */
	DI_Wr_reg_bits(NR4_TOP_CTRL, 0xaaa, 20, 12);
...
static unsigned int skip_regs[3][2];
/*
 * set nr skip ctrl regs value
 */
bool set_nr_skip_regs(unsigned int addr, unsigned int value)
{
	unsigned int i = 0, skip_addr[3] = {0x2d60, 0x2dff, 0x3738};
...
static void nr_skip_regs_load(void)
{
	unsigned int i = 0, addr, value;
	unsigned int skip_addr[3] = {0x2d60, 0x2dff, 0x3738};

	for (i = 0; i < 3; i++) {
		addr = skip_addr[i];
		value = skip_regs[i][1];
		if (skip_regs[i][0] && value != Rd(addr)) {
			DI_Wr(addr, value);
		}
		skip_regs[i][0] = 0;
	}
...
{
	unsigned int i = 0;
	struct NR_CTRL_REGS_s *pnr_regs = nr_param.pnr_regs;
...
void nr_process_in_irq(void)
{
	...
	if (dnr_en)
		...
		DI_Wr_reg_bits(DNR_CTRL, 0, 16, 1);
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TXLX)) {
...
static nr4_param_t nr4_params[NR4_PARAM_SIZE];
static void nr4_params_init(struct NR4_PARM_s *nr4_parm_p)
{
	memset(nr4_params, 0, NR4_PARAM_SIZE * sizeof(struct nr_param_s));
