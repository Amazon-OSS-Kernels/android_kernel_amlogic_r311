...
int flm2224_flag = 1; /*10*/

int flm64_flag = 1; /*10*/
#ifdef DEBUG_SUPPORT
module_param(flm2224_flag, int, 0644);
MODULE_PARM_DESC(flm2224_flag, "flm2224_flag");

module_param(flm64_flag, int, 0644);
MODULE_PARM_DESC(flm64_flag, "flm64_flag");
#endif
#define flm_mod_prdbg(fmt, args...)\
	do {\
		if (pr_pd)\
			sprintf(debug_str + strlen(debug_str), fmt, ## args);\
	} while (0)
...
int	FlmModsDet(struct sFlmDatSt *pRDat, int nDif01, int nDif02)
{
	...
	if (nT1 == 2 && flm64_flag == 1 && (Min_Max02 - Min_Min02) > 10) {
		tModLvl = 0;
		flm_mod_prdbg("#Min_Max02=%d, Min_Min02=%d,cnt=%d\n",
		    Min_Max02, Min_Min02, nModCnt[nT1]);
	} else if (nT1 == 2 && flm64_flag == 2) {
		flm_mod_prdbg("#Min_Max02=%d, Min_Min02=%d,cnt=%d\n",
		    Min_Max02, Min_Min02, nModCnt[nT1]);
		tModLvl = 0;
	}
