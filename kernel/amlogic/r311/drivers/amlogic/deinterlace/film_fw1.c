...
static int flm22_flag1;
module_param(flm22_flag1,  int, 0644);
MODULE_PARM_DESC(flm22_flag1, "flm22_flag1");
		...
		struct sFlmSftPar *pPar, int nROW, int nCOL, bool reverse)
{
	...
	static int nDIF01[HISDIFNUM];
	static int nDIF02[HISDIFNUM];
	/* UINT32 nCb32=0; */
	static unsigned int ntmp;
	static unsigned int flm22_mim_numb;
	...
	static int flm22;
	static int flm32;
	static int flm22_flag;
	static int flm2224_flag;
	static int flm22_comth;
	static int comdif;
	static int dif01avg;
	...
	mDly = pPar->mPstDlyPre;
	mNDly = pPar->mNxtDlySft;
	flm22 = pPar->flm22_en;
	flm32 = pPar->flm32_en;
	flm22_flag = pPar->flm22_flag;
	flm2224_flag = pPar->flm2224_flag;
	flm22_comth = pPar->flm22_comth;
	comdif = 0;
	dif01avg = 0;
...
{
	static int sFrmDifAvgRat;	/* 16;  //0~32 */
	/*  The Large Decision should be: (large>average+LgDifThrd) */
	static int sFrmDifLgTDif; /* 4096 */
	static int sF32StpWgt01;	/* 15; */
	static int sF32StpWgt02;	/* 15; */
	static int sF32DifLgRat;	/* 16; Dif>Rat*Min-->Larger */

	/* int sF32DifSmRat = 16;  //Dif*Rat<Max  --> Smaller */

	static UINT8 *pFlm02;
	static UINT8 *pFlm02t;
	static UINT8 *pFlm01;
	static UINT8 *pFlg32;

	static UINT8 sFld32[6] = { 0, 0, 0, 0, 0, 0 };

	static const UINT8 FIX02[2][5] = { {2, 3, 4, 5, 1}, {4, 5, 1, 2, 3} };
	...
	static int nMx;
	static int nSM;
	static UINT8 nFlg01[6] = { 0, 0, 0, 0, 0, 0 };
	static UINT8 nFlg02[6] = { 0, 0, 0, 0, 0, 0 };
	static UINT8 nFlg12[6] = { 0, 0, 0, 0, 0, 0 };
	...
	static int nFlgChk1;
	static int nFlgChk2;
	static int nFlgChk3; /* for Mit32VHLine */

	sFrmDifAvgRat = pPar->sFrmDifAvgRat;        /* 16;  //0~32 */
	sFrmDifLgTDif = pPar->sFrmDifLgTDif; /* 4096 */
	sF32StpWgt01 = pPar->sF32StpWgt01;  /* 15; */
	sF32StpWgt02 = pPar->sF32StpWgt02;  /* 15; */
	sF32DifLgRat = pPar->sF32DifLgRat;  /* 16; Dif>Rat*Min-->Larger */

	pFlm02 = pRDat->pFrm32;
	pFlm02t = pRDat->pFrm32t;
	pFlm01 = pRDat->pFld32;
	pFlg32 = pRDat->pFlg32;
	...
	nMn = nDif02[HISDIFNUM - 1];
	nMx = nDif02[HISDIFNUM - 1];
	nSM = nDif02[HISDIFNUM - 1];
			...
			int *nDif01, struct sFlmSftPar *pPar)
{
	...
	static int sFlm2MinAlpha;	/* 32; // [0~63] */
	static int sFlm2MinBelta;	/* 32; // [0~63] */
	static int sFlm20ftAlpha;	/* 16; // [0~63] */
	static int sFlm2LgDifThd;	/* 4096; */
	static int sFlm2LgFlgThd;	/* 8; */
	static int flm22_flag;
	static int flm22_comlev;
	static int flm22_comlev1;
	static int flm22_comnum;

	static int cFlg;
	static const int rFlg[4] = { 2, 3, 4, 1 };
	...
	static int num22; /* original static */

	static int FdTg[6];

	static int nFlgChk1; /* chk1 */
	static int nFlgCk20; /* chk2-0 */
	static int nFlgCk21; /* chk2-1 */
	static int nFlgChk3; /* chk3 */
	static int nFlgChk4; /* chk4 */
	static int nFlgChk5; /* chk5 */
	static int nFlgChk6; /* dif02-small */
	...
	sFlm2MinAlpha = pPar->sFlm2MinAlpha;	/* 32; // [0~63] */
	sFlm2MinBelta = pPar->sFlm2MinBelta;	/* 32; // [0~63] */
	sFlm20ftAlpha = pPar->sFlm20ftAlpha;	/* 16; // [0~63] */
	sFlm2LgDifThd = pPar->sFlm2LgDifThd;	/* 4096; */
	sFlm2LgFlgThd = pPar->sFlm2LgFlgThd;	/* 8; */
	flm22_flag = pPar->flm22_flag;
	flm22_comlev = pPar->flm22_comlev;
	flm22_comlev1 = pPar->flm22_comlev1;
	flm22_comnum = pPar->flm22_comnum;

	cFlg = pFlg[HISDETNUM - 1];
	...
	nMn = nDif01[HISDIFNUM - 1];
	nMx = nDif01[HISDIFNUM - 1];

	nSM20 = nDif01[HISDIFNUM - 1];
	nSM21 = 0;
	nSM22 = 0;
	nL21 = 0;
	nL22 = 0;
	...
	nFlgChk4 = 0; /* chk4 */
	nFlgChk5 = 0; /* chk5 */
	nFlgChk6 = 0; /* dif02-small */

	iWidth  = pRDat->iWidth;
	iHeight = pRDat->iHeight;
	nFlm22Lvl = 0;
	nSIZE = iWidth * iHeight + 1;
	...
	if (flm22_flag) {
		if (pFlg[HISDETNUM-1] == 3
			...
			if (comsum > flm22_comnum &&
				((nDif01[HISDIFNUM-2] +
				nDif01[HISDIFNUM-2] / 10) > nDif02[HISDIFNUM-2]
				|| flm22_flag1)) {
				if (num22 < 30)
					num22 = num22 + 1;
				else
