...
static void work_irlearning(struct work_struct *work)
{
	struct remote_chip *chip = container_of(work, struct remote_chip, work_irlearn);
	char *envp[3] = {
		[0] = "ir_learn_done",
		[1] = NULL,
		[2] = NULL,
	};
//    dev_err(chip->dev, "generate IR learn done uevent\n");
	kobject_uevent_env(&chip->dev->kobj, KOBJ_CHANGE, envp);
}

static irqreturn_t ir_learn_interrupt(int irq, void *dev_id)
{
	struct remote_chip *chip = (struct remote_chip *)dev_id;


//    dev_err(chip->dev, "******ir_learn_interrupt\n");
	schedule_work(&(chip->work_irlearn));
	return 0;
}
#endif
...
#ifdef CONFIG_MESON_REMOTE_LEARN
    res_irq_m3 = platform_get_resource(pdev, IORESOURCE_IRQ, 1);
    if (IS_ERR_OR_NULL(res_irq_m3)) {
        dev_err(chip->dev, "get IORESOURCE_IRQ M3 error, %ld\n",
                PTR_ERR(p));
    } else {
        chip->irq_m3 = res_irq_m3->start;
        chip->ir_learn = true;
        dev_info(chip->dev, "****IR Learn is ENABLE****\n");
        dev_info(chip->dev, "platform_data irq_m3 =%d\n", chip->irq_m3);
    }
#endif
...
#ifdef CONFIG_MESON_REMOTE_LEARN
    if (chip->ir_learn) {
    	ret = request_irq(chip->irq_m3, ir_learn_interrupt, IRQF_SHARED,
    				"ir_learn", (void *)chip);
    	if (ret < 0) {
            /* can't learn interrupt in device tree. the IR learn is not provided! */
            dev_err(chip->dev, "request_irq m3 error %d\n", ret);
            free_irq(chip->irqno, chip);
            goto error_irq;
    	}
        INIT_WORK(&(chip->work_irlearn), work_irlearning);
    }
#endif
...
static int remote_probe(struct platform_device *pdev)
{
	...
	/* Support Amazon's customized IR keys. */
...
#ifdef CONFIG_MESON_REMOTE_LEARN
    if (chip->ir_learn) {
        cancel_work_sync(&(chip->work_irlearn));
        free_irq(chip->irq_m3, chip);
    }
#endif
...
static int remote_resume(struct platform_device *pdev)
{
	...
	unsigned int resume_method = get_resume_method();

	dev_info(chip->dev, "remote resume:0x%0x\n", resume_method);
	...
	if (rtc_str_wake && (get_resume_method() == AUTO_WAKEUP)) {
		input_event(chip->r_dev->input_device,
			EV_KEY, KEY_POWER, 1);
		input_sync(chip->r_dev->input_device);
		input_event(chip->r_dev->input_device,
	...
	if (resume_method == REMOTE_CUSTOM1_WAKEUP)
		key_code = KEY_CUSTOM_1;
	else if (resume_method == REMOTE_CUSTOM2_WAKEUP)
		key_code = KEY_CUSTOM_2;
	else if (resume_method == REMOTE_CUSTOM3_WAKEUP)
	...
	if (key_code) {
		...
		input_event(chip->r_dev->input_device, EV_KEY, key_code, 1);
		input_sync(chip->r_dev->input_device);
		input_event(chip->r_dev->input_device, EV_KEY, key_code, 0);
		input_sync(chip->r_dev->input_device);
		if (scpi_clr_wakeup_reason())
			pr_debug("clr wakeup reason fail.\n");
	}
