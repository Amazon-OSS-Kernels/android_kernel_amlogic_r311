...
/* backlight status in OTA stage */
static int backlight_on_req;
static unsigned int silent_ota_started; /* 1: started, 0: not started */
static char reboot_mode1[16]; /* factory and normal */

static int __init silent_ota_started_setup(char *str)
{
	int ret = 0;

	if (str != NULL)
		ret = kstrtouint(str, 10, &silent_ota_started);

	return ret;
}
__setup("silent_ota=", silent_ota_started_setup);

static int __init reboot_mode_setup(char *str)
{
	int ret = 0;

	memset(reboot_mode1, 0x0, 16);
	if (str != NULL)
		ret = sprintf(reboot_mode1, "%s", str);
...
static int key_check_for_silent_ota(unsigned int key_code)
{
	pr_warn("SILENT OTA - KEYCODE = %d\n", key_code);
	if (KEY_POWER == key_code) {
		backlight_on_req = OTA_TOGGLE_BACKLIGHT;
		pr_warn("SILENT OTA - Toggle the backlight\n");
	}
	else if (KEY_CUSTOM_1 == key_code ||
			...
			KEY_ENTER == key_code ||
			KEY_SEARCH == key_code) {
		backlight_on_req = OTA_TURN_ON_BACKLIGHT;
		pr_warn("SILENT OTA - Turn ON the backlight\n");
	}
	if (backlight_on_req) {
...
static void silent_ota_update_work(struct work_struct *work)
{
	struct remote_dev *remote = container_of(work, struct remote_dev,
			silent_ota_update);
	key_check_for_silent_ota(remote->last_keycode);
}
...
static void ir_do_keyup(struct remote_dev *dev)
{
	...
	if ((silent_ota_started || strncmp(reboot_mode1, "factory_reset", 13)) &&
			!boot_complete)
		schedule_work(&(dev->silent_ota_update));
...
int remote_register_device(struct remote_dev *dev)
{
	...
	INIT_WORK(&dev->silent_ota_update, silent_ota_update_work);
