...
static uint64_t storage_smc_ops(uint64_t func)
{
	register unsigned int r0 asm("r0") = func;
	asm volatile(
		__asmeq("%0", "r0")
		__SMC(0)
		: "+r" (r0));
...
static void *map_reserve_mem(phys_addr_t phys_addr, size_t size, void *vaddr)
{
...
#define PROT_PTE_DEVICE (L_PTE_PRESENT | L_PTE_YOUNG | L_PTE_DIRTY | L_PTE_XN)
	err = ioremap_page_range((unsigned long)vaddr,
	...
	err = ioremap_page_range((unsigned long)vaddr,
				 (unsigned long)vaddr + size,
				 phys_addr, PROT_NORMAL);
#endif
	if (err)
		return NULL;

	return (void *)((unsigned long)vaddr + offset);
}
...
static int storage_probe(struct platform_device *pdev)
{
	...
	storage_out_base = ioremap_cache(phy_storage_out_base,
					storage_block_size);
	storage_block_base = map_reserve_mem(phy_storage_block_base,
					storage_block_size,
					phys_to_virt(phy_storage_block_base));
