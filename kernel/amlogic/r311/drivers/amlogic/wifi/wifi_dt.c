...
*/
	...
	if (wifi_info.power_on_pin) {
		...
		if (wifi_info.power_pin_reset
			&& !strstr(saved_command_line, "recovery_part=")) {
			ret = set_power(wifi_info.power_on_pin_level);
			mdelay(200);
			ret = set_power(!wifi_info.power_on_pin_level);
		} else {
			WIFI_INFO("Don't reset MT7668 in recovery mode\n");
			ret = set_power(!wifi_info.power_on_pin_level);
		}
		if (ret)
	...
	if (wifi_info.power_on_pin2) {
		...
		if (wifi_info.power_pin_reset) {
			ret = set_power2(wifi_info.power_on_pin_level);
			mdelay(200);
			ret = set_power2(!wifi_info.power_on_pin_level);
		} else
			ret = set_power2(wifi_info.power_on_pin_level);
...
static int wifi_dev_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = of_property_read_u32(pdev->dev.of_node,
		"power_pin_reset", &plat->power_pin_reset);
		if (ret)
			plat->power_pin_reset = 0;
