...
#else
			temp = out[j][i] + (determinant >> 1);
			do_div(temp, determinant);
			out[j][i] = temp;
#endif
		}
	}
...
#else
	temp = norm * prmy[3][0] * 2;
	do_div(temp, prmy[3][1]);
	B[0] = (temp + 1) >> 1;
	B[1] = norm;
	temp = norm * z[3] * 2;
	do_div(temp, prmy[3][1]);
