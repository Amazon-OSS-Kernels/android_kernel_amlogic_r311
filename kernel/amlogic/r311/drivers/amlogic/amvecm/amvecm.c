	...
	loff_t *off)
{
	if (pq_config_buf == NULL) {
		pq_config_buf = vmalloc(sizeof(struct pq_config_s));
		if (pq_config_buf == NULL)
			return -ENOSPC;
	}

	if (len < sizeof(struct pq_config_s))
		return -EINVAL;

	if (copy_from_user(pq_config_buf, buf, sizeof(struct pq_config_s)))
		return -EFAULT;
		...
		unsigned int cmd, unsigned long arg)
{
	...
	**********************************************************************/
	...
	case AMVECM_IOC_SET_OVERSCAN:
		...
		vpp_pq_load_table = kmalloc(12*sizeof(struct ve_pq_table_s),
			GFP_KERNEL);
		if (vpp_pq_load_table == NULL) {
			ret = -EFAULT;
			pr_amvecm_dbg("[amvecm..] vpp_pq_load_table malloc fail!!\n");
			break;
		}
		memcpy(vpp_pq_load_table, vpp_pq_load.amve_pq_table_s,
			12 * sizeof(struct ve_pq_table_s));
		 ...
		 const char *buffer, size_t count)
{
	...
	if (n == 0) {
		pr_info("strsep null,parm[] uninitialized !\n");
		kfree(buf_orig);
		return -EINVAL;
	}
...
static struct class_attribute amvecm_class_attrs[] = {
	...
	__ATTR(dnlp_debug, S_IRUGO | S_IWUSR,
...
/* #if (MESON_CPU_TYPE >= MESON_CPU_TYPE_MESONG9TV) */
	...
	__ATTR(dnlp_insmod, S_IRUGO | S_IWUSR,
