		...
		unsigned long prate)
{
	...
	if (mpll->sdm_in == 0 && mpll->n_in == 0) {
		rate = prate;
	} else {
		rate64 = (uint64_t)prate * SDM_MAX;
		frac64 = do_div(rate64, ((SDM_MAX*mpll->n_in) + mpll->sdm_in));
		/* in fact, clk rate won't exceed 32bits */
		rate = rate64;
	}
...
{
	struct mpll_clk *mpll = to_clk_pll(hw);
	u64 divider, val64, frac64;
	unsigned long frac;
	uint val;

	divider = prate;
	frac = do_div(divider, drate);
	val64 = (u64)frac * SDM_MAX;
	frac64 = do_div(val64, drate);
	val = val64;
