...
#ifdef CONFIG_AM_NEW_PLL_EN
	FRATE(CLK_FIXED_PLL, "fixed_pll", NULL, CLK_IS_ROOT, 1999998046),
#else
...
static void __init txlx_clk_init(struct device_node *np)
{
...

	{
...
#else
		if (IS_ERR(vapb)) {
			pr_err("%s: failed to config vapb clock: %ld\n",
			__func__, PTR_ERR(vapb));
			kfree(vapb);
			return;
		}
#endif
		...
		if (!IS_ERR(fixdiv5))
			...
			pr_err("failed to config clk_mali clock\n");
			kfree(clk_mali_0);
			kfree(clk_mali);
			return;
		}
