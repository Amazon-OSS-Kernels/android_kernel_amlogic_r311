...
static u32 run_ready_min_buf_num = 3;
...
#ifdef SUPPORT_4K2K
	if (vdec_is_support_4k() && !codec_mm_is_lessmem())
...
/*! MULTI_INSTANCE_SUPPORT*/
	...
	if (codec_mm_is_lessmem()) {
		if ((buf_alloc_width + buf_alloc_width) > (1920 + 1088)) {
...
static void set_frame_info(struct VP9Decoder_s *pbi, struct vframe_s *vf)
{
	...
	vf->video_format = VFRAME_FORMAT_VP9;
	vf->flag = 0;
...
static int frame_duration_adapt(struct VP9Decoder_s *pbi,
		struct vframe_s *vf, u32 valid)
{
				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if (display_frame_count[pbi->index] == 0)
		pr_info("vp9 decoder first frame ready\n");
...
static irqreturn_t vvp9_isr_thread_fn(int irq, void *data)
{
	...
	*/
	...
	if (OVER_SIZE(vp9_param.p.width, vp9_param.p.height)) {
		pr_info("vp9 not support 4k oversize !! width %d height %d\n",
		vp9_param.p.width, vp9_param.p.height);
		debug |= (VP9_DEBUG_DIS_LOC_ERROR_PROC |
			VP9_DEBUG_DIS_SYS_ERROR_PROC);
		pbi->fatal_error |= DECODER_FATAL_ERROR_SIZE_OVERFLOW;
#ifdef MULTI_INSTANCE_SUPPORT
...
static void wait_hevc_search_done(struct VP9Decoder_s *pbi)
{
	int count = 0;
...
{
	struct VP9Decoder_s *vp9 = (struct VP9Decoder_s *)vdec->private;
#else
int vvp9_dec_status(struct vdec_info *vstatus)
{
	struct VP9Decoder_s *vp9 = &gHevc;
#endif
	vstatus->frame_width = vp9->frame_width;
	vstatus->frame_height = vp9->frame_height;
	if (vp9->frame_dur != 0)
...
static s32 vvp9_init(struct VP9Decoder_s *pbi)
{
	...
	if (vvp9_local_init(pbi) < 0)
		return -EBUSY;
	init_waitqueue_head(&pbi->wait_q);
#ifdef MULTI_INSTANCE_SUPPORT
	if (pbi->m_ins_flag) {
...
static int ammvdec_vp9_probe(struct platform_device *pdev)
{
	...
	if (pbi->index < MAX_DECODE_INSTANCE_NUM)
		display_frame_count[pbi->index] = 0;
...
static int ammvdec_vp9_remove(struct platform_device *pdev)
{
	...
	if (vdec->next_status == VDEC_STATUS_DISCONNECTED
			...
			pbi->dec_result = DEC_RESULT_FORCE_EXIT;
			vdec_schedule_work(&pbi->work);
			wait_event_interruptible_timeout(pbi->wait_q,
