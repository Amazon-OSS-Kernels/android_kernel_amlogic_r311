...
int vdec_prepare_input(struct vdec_s *vdec, struct vframe_chunk_s **p)
{
	 ...
	 */
	if (input_frame_based(input)) {
		chunk = vdec_input_next_input_chunk(&vdec->input);
...
int vdec_disconnect(struct vdec_s *vdec)
{
	...
	if (vdec->slave){
		if (!wait_for_completion_timeout(
				&vdec->slave->inactive_done,
				msecs_to_jiffies(2000)))
			goto discon_timeout;
	}
	else if (vdec->master){
			if (!wait_for_completion_timeout(
				&vdec->master->inactive_done,
				msecs_to_jiffies(2000)))
				goto discon_timeout;
	}
	return 0;
discon_timeout:
	pr_err("%s timeout !status %x\n", __func__, vdec->status);
	return -1;
}
...
static irqreturn_t vdec_thread_isr(int irq, void *dev_id)
{
	...
	if (!vdec->threaded_irq_handler)
		goto thread_isr_done;
	ret = vdec->threaded_irq_handler(vdec);
thread_isr_done:
	if (vdec)
...
void hevc_poweroff_safe(struct vdec_s *vdec)
{
	unsigned long flags;
	WRITE_VREG(HEVC_STREAM_CONTROL, 0);
	spin_lock_irqsave(&vdec_spin_lock, flags);
