...
static int tsync_video_delay = 0;
static int tsync_video_started;
#define PCR_FORCE_RESET_DURATION (3000)
static int tsync_audio_starttime;
...
*/
#define DEFAULT_MODE_SWITCH_EXPIRE_TIME 3*1000
static int tsync_mode_switch(int mode, unsigned long diff_pts, int jump_pts)
{
	...
	pr_info
	("%c-discontinue,pcr=%x,vpts=%x,apts=%x,diff_pts=%lu,jump_Pts=%d\n",
	...
	if (mode == 'T') {	/*D/A--> ... */
		if (tsync_av_mode == TSYNC_STATE_D) {
			...
			if (tsync_av_dynamic_duration_ms > DEFAULT_MODE_SWITCH_EXPIRE_TIME)
				tsync_av_dynamic_duration_ms = DEFAULT_MODE_SWITCH_EXPIRE_TIME;
			...
			diff_pts < tsync_av_threshold_min) {
				pr_info("< tsync_av_threshold_min\n");
				tsync_av_mode = TSYNC_STATE_S;
				tsync_mode = TSYNC_MODE_AMASTER;
				tsync_av_latest_switch_time_ms = jiffies_ms;
				tsync_av_dynamic_duration_ms = DEFAULT_MODE_SWITCH_EXPIRE_TIME;
			...
			diff_pts < tsync_av_threshold_max) {
				pr_info("<tsync_av_threshold_max\n");
...
void tsync_avevent_locked(enum avevent_e event, u32 param)
{
	...
	case AUDIO_START:
		...
		timestamp_pcrscr_enable(1);
		apause_flag = 0;
		tsync_audio_starttime = jiffies_ms32;
		break;
...
int tsync_get_video_delay(void)
{
    if(tsync_get_debug_vpts())
        printk("get the video delay now!\n");

    return tsync_video_delay;
}
EXPORT_SYMBOL(tsync_get_video_delay);
...
int tsync_set_apts(unsigned pts)
{
	...
	if (tsync_mode == TSYNC_MODE_AMASTER) {
		...
		if (dobly_avsync_test) {
			if (tsync_audio_starttime > 0
				&& (jiffies_ms32 - tsync_audio_starttime)
				< PCR_FORCE_RESET_DURATION)
	...
	if (tsync_mode == TSYNC_MODE_AMASTER) {
		...
		/* av diff too much, switch to vmaster directly */
		if (abs(timestamp_apts_get() -
			timestamp_vpts_get()) >=
			AV_DISCONTINUE_THREDHOLD_MAX) {

			t = timestamp_vpts_get();
			pr_info("[%d] reset pcr:0x%x-->0x%x\n",
			__LINE__, pts, t);
...
/*add the user setting video delay to
video pts for sync model to delay the video
this function only works on Blanche2020 and
it will use property to ensure*/
static ssize_t show_video_delay(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",tsync_video_delay/90);
}


static ssize_t store_video_delay(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	int pts;
...
static struct class_attribute tsync_class_attrs[] = {
	...
	__ATTR(video_delay, S_IRUGO | S_IWUSR | S_IWGRP, show_video_delay,
	store_video_delay),
...
static struct mconfig tsync_configs[] = {
	...
	MC_FUN_ID("last_checkin_vpts", tsync_show_fun, NULL, 22),
	MC_FUN_ID("last_checkout_apts", tsync_show_fun, NULL, 23),
	MC_FUN_ID("last_checkout_vpts", tsync_show_fun, NULL, 24),
};
