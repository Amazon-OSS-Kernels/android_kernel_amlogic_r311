...
#endif
static unsigned int reorder_dpb_size_margin = 4;
static unsigned int reference_buf_margin = 2;
...
static unsigned int frmbase_cont_bitlevel = 0x1aa;
...
int prepare_display_buf(struct vdec_s *vdec, struct FrameStore *frame)
{
	...
	for (i = 0; i < vf_count; i++) {
		...
		vf->video_format = VFRAME_FORMAT_H264;
			...
			int slice_type)
{
	if (pic)
		...
		info,
		(pic->structure < 0) ? "" : ((pic->structure <
		NUM_PICTURE_STRUCTURE) ? picture_structure_name[pic->structure]
		: "Unknown Pic_Structure"),
		pic->coded_frame ? "Frame" : "Field",
		(slice_type < 0) ? "" : ((slice_type < NUM_SLICE_TYPES) ?
		slice_type_name[slice_type] : "Unknown Slice"),
	...
	u32 param1, u32 param2, u32 param3, u32 param4)
{
	...
	if (mb_width > 0x78 && mb_height > 0x44) {
		dpb_print(DECODE_ID(hw), 0,
			"vmh264 not support 4K mb_height %x mb_width %d\n",
		mb_height, mb_width);
		hw->data_flag |= ERROR_FLAG;
	...
	if (hw->config_bufmgr_done == 0) {
		...
		if (max_reference_size > 10) {
			hw->max_reference_size = max_reference_size + 2;
			hw->dpb.mDPB.size = hw->dpb.reorder_pic_num
					= active_buffer_spec_num
					= hw->max_reference_size + 2;
		} else {
	...
	if (dec_dpb_status == H264_CONFIG_REQUEST) {
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if (slice_header_process_status == 1) {
				...
				if (((unsigned long)(hw->vh264_amstream_dec_info
						.param)) & 0x8) {
					fast_output_enable =
...
{
	struct vdec_h264_hw_s *hw = (struct vdec_h264_hw_s *)(vdec->private);
	struct h264_dpb_stru *p_H264_Dpb = NULL;
...
static void vmh264_dump_state(struct vdec_s *vdec)
{
	...
	dpb_print(DECODE_ID(hw), 0,
		"====== %s\n", __func__);
	dpb_print(DECODE_ID(hw), 0,
		"width/height (%d/%d), reorder_pic_num %d dpb size(bufspec count) %d max_reference_size(collocate count) %d fast_output_enable %d\n",
		hw->frame_width,
		hw->frame_height,
		hw->dpb.reorder_pic_num,
		hw->dpb.mDPB.size,
...
static int vh264_stop(struct vdec_h264_hw_s *hw)
{
	...
	fast_output_enable = H264_OUTPUT_MODE_NORMAL;
	/* amvdec_disable(); */
	dpb_print(DECODE_ID(hw), 0,
		"%s\n",
		__func__);
...
int ammvdec_h264_mmu_init(struct vdec_h264_hw_s *hw)
{
	...
	pr_info("ammvdec_h264_mmu_init tvp = 0x%x mmu_enable %d alloc_size %d\n",
			tvp_flag, hw->mmu_enable, alloc_size);
...
static int ammvdec_h264_probe(struct platform_device *pdev)
{
	...
	__func__, mmu_enable, hw->double_write_mode);
...
static int ammvdec_h264_remove(struct platform_device *pdev)
{
	...
	ammvdec_h264_mmu_release(hw);
	h264_free_hw_stru(&pdev->dev, (void *)hw);
	pr_info("ammvdec_h264_remove done\n");
	return 0;
}
