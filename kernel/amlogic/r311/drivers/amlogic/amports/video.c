...
#include "video.h"
// fosmod_graphics_profiling begin
#include <trace/events/amz_atrace.h>
// fosmod_graphics_profiling end
...
static u32 omx_current_frame_num;
static bool omx_drop_done;
#define OMX_TV_AVSYN_MAX_RESET_COUNT 20
#define OMX_TV_AVSYNC_GAP (30*90)
...
#define OMX_MAX_CHECK_DROP_COUNT 3
...
/*drop video threshold 500ms*/
#define TUNNEL_MODE_DROP_THRESHOLD 45000

static s32 amvideo_poll_major;
static s8 dolby_first_delay; /* for bug 145902 */

/*vframe_type -1: invalid; 0: PROGRESSIVE; 1: INTERLACE;*/
static int vframe_type = -1;
static int cur_sys_vframe_type = -1;
...
struct switch_dev video2_state_sdev = {
/* android video frametype switch device */
	.name = "video_frametype",
};
...
static DEFINE_SPINLOCK(omx_sync_lock);
...
static u32 frame_in_count;
static bool first_frame_peeked;
	...
	if (pattern == PTS_32_PATTERN) {
		...
		if (pts_trace == 2) {
			if ((pts_41_pattern_sink[1] == 1) &&
				...
				(pts_pattern[PTS_41_PATTERN] < PTS_41_PATTERN_DETECT_RANGE)) {
				pts_pattern[PTS_41_PATTERN]++;
				if (pts_pattern[PTS_41_PATTERN] == PTS_41_PATTERN_DETECT_RANGE) {
			...
			if ((pts_41_pattern_sink_index < PTS_41_PATTERN_SINK_MAX) && (pts_41_pattern_sink_index > 0)) {
				pts_41_pattern_sink[pts_41_pattern_sink_index] = 1;
				pts_41_pattern_sink_index++;
			} else if (pts_pattern[PTS_41_PATTERN] == PTS_41_PATTERN_DETECT_RANGE) {
				pts_pattern[PTS_41_PATTERN] = 0;
				pts_41_pattern_sink_index = 0;
				pts_pattern_exit_cnt[PTS_41_PATTERN]++;
				ATRACE_COUNTER("VIDEO: broken pattern", 1); // fosmod_graphics_profiling oneline
		...
		ATRACE_COUNTER("VIDEO: broken pattern", 0); // fosmod_graphics_profiling oneline
		return;
	}
...
static void vsync_toggle_frame(struct vframe_s *vf)
{
	...
	if (vf->early_process_fun) {
		...
		if (DI_POST_REG_RD(DI_IF1_GEN_REG) & 0x1) {
			...
			DI_VSYNC_WR_MPEG_REG(DI_IF1_GEN_REG,
					  vpp_read_di_pre_reg(DI_IF1_GEN_REG) &
					  0xfffffffe);
		}
	}
	...
	if (0 == strcmp(hwid_val, "1111") || (0 == strcmp(hwid_val, "1100"))){
	t_cur = (jiffies_64 - vf->ready_jiffies64) * 1000;
	do_div(t_cur, HZ);
	vframe_walk_delay = t_cur;
	}
	...
	cur_dispbuf = vf;
	if (cur_dispbuf)
		vframe_type = (cur_dispbuf->type_original & 0x1);
				   ...
				   struct vframe_s *next_vf, u32 dur)
{
	...
	static const u32 rpt_tab[4] = { 0x100, 0x100, 0x150, 0x100 };
...
#ifdef PTS_LOGGING
static inline void vpts_perform_pulldown(struct vframe_s *next_vf, bool* expired) {
	int pattern_range, expected_curr_interval = -1, expected_prev_interval = -1;
	...
	if (*expired) {
		if (pts_trace < expected_curr_interval) {
			...
			if (((int)(nextPts + expected_curr_interval * vsync_pts_inc -
				next_vf->next_vf_pts) < 0) &&
				((int)(nextPts + expected_prev_interval * vsync_pts_inc -
				next_vf->next_vf_pts) >= 0)) {
					*expired = false;
					pr_info("hold frame for pattern: %d", pts_pattern_detected);
			}
		}
	} else {
		if (pts_trace == expected_curr_interval) {
			...
			int nextPts = timestamp_pcrscr_get() + vsync_pts_align;
			if (((int)(nextPts + vsync_pts_inc - next_vf->pts) >= 0) &&
				((int)(nextPts + vsync_pts_inc - next_vf->next_vf_pts) < 0) &&
				((int)(nextPts + expected_curr_interval * vsync_pts_inc - next_vf->next_vf_pts) >= 0)) {
					*expired = true;
					pr_info("pull frame for pattern: %d", pts_pattern_detected);
			}
		}
	}
}
			       ...
			       int toggled_cnt)
{
	...
	if (NULL != next_vf && next_vf->flag & VFRAME_FLAG_GAME_MODE)
		return true;
	...
	/* check video PTS discontinuity */
		...
		*/
		...
		if (cur_vf) {
			last_vpts = cur_vf->pts;
			if (cur_vf->pts == 0)
				last_vpts = timestamp_vpts_get() +
				    DUR2PTS(cur_vf->duration);
		} else {
			last_vpts = timestamp_vpts_get();
		}

		if ((int)(systime - pts) >= 0) {
			if (abs(last_vpts - next_vf->pts)
...
int hdmi_in_start_check(struct vframe_s *vf)
{
	...
	expire = jiffies_to_msecs(jiffies) - hdmin_delay_start_time;
...
#endif
{
	...
	/* before omx_run, app may drop many frames,
	these frames also should be	dropped through
	set_omx_pts, but sometimes set_omx_pts couldn't
	get all the frames from prev module timely, it
	will lead to some frames always hold by prev
	module and video block.
	Here we try to drop again, to fix this issue.*/
	if (!omx_run && !omx_drop_done && omx_secret_mode) {
		struct vframe_s *vf = NULL;
		unsigned int try_cnt = OMX_MAX_CHECK_DROP_COUNT;
		unsigned long flags;
		spin_lock_irqsave(&omx_sync_lock, flags);
		while (try_cnt-- && !omx_run) {
			vf = vf_peek(RECEIVER_NAME);
			if (vf) {
				if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
					pr_info("vsync get frame %d\n", vf->omx_index);
				}
				if (omx_current_frame_num > 0 &&
					omx_current_frame_num >= vf->omx_index) {
					vf = vf_get(RECEIVER_NAME);
					if (vf) {
						if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
							pr_info("vsync drop frame %d\n", vf->omx_index);
						}
						vf_put(vf, RECEIVER_NAME);
					}
				} else
		...
		spin_unlock_irqrestore(&omx_sync_lock, flags);
	}
	...
	if (omx_secret_mode == true) {
		...
		if ((diff - omx_pts_interval_upper) > 0
			...
			|| (display_frame_count <
				OMX_TV_AVSYN_MAX_RESET_COUNT
				&& (diff < 0 || abs(diff) > OMX_TV_AVSYNC_GAP))
			...
			if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
				pr_info("sys_time=%d, omx_pts=%d, diff=%d display:%d\n",
				system_time, omx_pts, diff, display_frame_count);
			}
	...
	if (vf) {
		...
		if (ret > 0) {
			if (vf->early_process_fun &&
				...
				vf->early_process_fun(vf->private_data, vf);
			goto exit;
		}
	}
	...
	if (vf != NULL && frame_in_count == 1 && first_frame_peeked == false) {
		if (debug_flag & DEBUG_FLAG_BLACKOUT)
			pr_info("amvideo first frame peeked, vf = %p\n", vf);
		first_frame_peeked = true;
	}
	while (vf) {
		if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
			pr_info("next pts= %d,index %d,pcr = %d,vpts = %d\n",
				vf->pts, vf->omx_index, timestamp_pcrscr_get(),
			timestamp_vpts_get());
		}
		if ((tunnel_drop_enable) &&
			(timestamp_apts_started() > 0) &&
			timestamp_apts_get() > 0) {
			if ((vf->pts < timestamp_apts_get()) &&
				(timestamp_apts_get() - vf->pts >
					TUNNEL_MODE_DROP_THRESHOLD)) {
				pr_info("drop video frame:0x%x, apts:0x%x\n",
					vf->pts, timestamp_apts_get());
				vf = vf_get(RECEIVER_NAME);
				if (vf)
					vf_put(vf, RECEIVER_NAME);
				vf = video_vf_peek();
			} else {
				tunnel_drop_enable = 0;
				pr_info("clear tunnel_drop_enable\n");
			}

		} else if ((omx_continuous_drop_flag) &&
			...
			if (vpts_expire(cur_dispbuf, vf, toggle_cnt) || show_nosync) {
				...
				if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
					pr_info("render omx_index %d, pts %d\n",
							vf->omx_index, vf->pts);
				}
#if defined(CONFIG_AM_VECM)
				refresh_on_vs(vf);
				...
				if (debug_flag & DEBUG_FLAG_TOGGLE_FRAME_PER_VSYNC)
					break;
				video_get_vf_cnt++;
				if (video_get_vf_cnt >= 2 && vf) {
					video_drop_vf_cnt++;
					if ((debug_flag &
						DEBUG_FLAG_OMX_DEBUG_DROP_FRAME)
						&& (omx_secret_mode == false)) {
						pr_info("tunnel mode drop frame. pts:%x\n",
							vf->pts);
					}
				}
			} else {
...
cur_dev->vpp_off,0,VPP_VD2_ALPHA_BIT,9);//vd2 alpha must set
			...
			if (reverse) {
				if ((cur_dispbuf->type & VIDTYPE_TYPEMASK)
					...
					vin_type = VIDTYPE_INTERLACE_BOTTOM;
				else if ((cur_dispbuf->type & VIDTYPE_TYPEMASK)
					== VIDTYPE_INTERLACE_BOTTOM)
					vin_type = VIDTYPE_INTERLACE_TOP;
			}
#endif
			vphase =
 ...
 exit:
	...
	if (video1_off_req) {
		...
		*/
		...
		/* don't clear buffer in pause mode. */
		/* Just move the buffer clear in try_free_keep_video
		brian.zhu 20191029.*/
#if 0
		/*clear cur_dispbuf on video disable commit for zhouzhi1221.*/
		if (cur_dispbuf && (cur_dispbuf != &vf_local))
			video_vf_put(cur_dispbuf);
		cur_dispbuf = NULL;
		di_force_free_mem();
		pr_info("force freee di");
#else
		if (cur_dispbuf && (cur_dispbuf == &vf_local))
			cur_dispbuf = NULL;
#endif
...
static int video_receiver_event_fun(int type, void *data, void *private_data)
{
	...
	if (type == VFRAME_EVENT_PROVIDER_UNREG) {
		...
		omx_current_frame_num = 0;
		omx_drop_done = false;
		omx_check_previous_session = true;
		tunnel_drop_enable = 0;
		pr_info("PROVIDER_REG\n");
		omx_continuous_drop_count = 0;
		omx_continuous_drop_flag = false;
		cur_disp_omx_index = 0;
		mutex_unlock(&omx_mutex);
...
int _video_set_disable(u32 val)
{
	...
	disable_video = val;
	if ((disable_video == VIDEO_DISABLE_FORNEXT)
		&& cur_dispbuf && (cur_dispbuf != &vf_local))
		disable_video = VIDEO_DISABLE_NONE;
	...
	if (disable_video != VIDEO_DISABLE_NONE) {
		...
		if (cur_dispbuf && (cur_dispbuf != &vf_local))
			EnableVideoLayer();
		/*ensue vpp config is setting*/
		video_property_changed = true;
	}
	pr_info("set disable video to :%u", val);
	return 0;
}
...
static void set_omx_pts(u32 *p)
{
	...
	if (frame_num > omx_current_frame_num)
		omx_current_frame_num = frame_num;

	if (debug_flag & DEBUG_FLAG_OMX_DEBUG_DROP_FRAME) {
		pr_info("check %d,session 0x%x,"
				"omx_cur_session 0x%x,"
				"omx_index %d-%d,pts %d,need_drop %d\n",
			omx_check_previous_session,
			session, omx_cur_session,
			frame_num, omx_current_frame_num,
			tmp_pts, !set_from_hwc);
	}

	if (omx_check_previous_session) {
		if (session != omx_cur_session) {
			pr_info("update session 0x%x->0x%x\n", omx_cur_session, session);
			omx_cur_session = session;
			omx_check_previous_session = false;
		} else {
			pr_info("ignore old session\n");
			mutex_unlock(&omx_mutex);
			return;
		}
	}
...
static long amvideo_ioctl(struct file *file, unsigned int cmd, ulong arg)
{
	...
	*****************************************************************/
	...
	case AMSTREAM_IOC_GET_VIDEO_INFO: {
		unsigned long flags;
		struct am_videoinfo info;
		...
		if (cur_dispbuf) {
			...
			info.prog_mode = vframe_type;
		}

		spin_unlock_irqrestore(&lock, flags);
		pr_info("AMSTREAM_IOC_GET_VIDEO_INFO:%d", info.prog_mode);
		if (copy_to_user(argp, &info,
			sizeof(struct am_videoinfo)) != 0)
			ret = -EFAULT;
		break;
	}
	default:
			...
			struct class_attribute *attr, char *buf)
{
	...
	di_free = di_free_mirror_mem();

	if ((video_inuse == 0) && (di_free == true)) {
		r = sprintf(buf, "%d\n", video_inuse);
		video_inuse = 1;
...
static int __init video_early_init(void)
{
	...
	WRITE_VCBUS_REG(VPU_WRARB_MODE_L2C1, 0x80000);
...
static struct mconfig video_configs[] = {
	...
	MC_PI32("omx_current_frame_num", &omx_current_frame_num),
...
void update_vframe_type_uevent(void)
{
	if (cur_sys_vframe_type != vframe_type) {
		cur_sys_vframe_type = vframe_type;
		pr_debug("update vframe_type %d\n", cur_sys_vframe_type);
		switch_set_state(&video2_state_sdev, cur_sys_vframe_type);
		pr_debug("update vframe_type %d done\n", cur_sys_vframe_type);
	}
}
...
static int __init video_init(void)
{
	...
	switch_dev_register(&video2_state_sdev);
	switch_set_state(&video2_state_sdev, -1);
...
MODULE_PARM_DESC(tunnel_drop_enable, "\n tunnel_drop_enable\n");
module_param(tunnel_drop_enable, uint, 0664);
...
MODULE_PARM_DESC(omx_current_frame_num, "\n omx_current_frame_num\n");
module_param(omx_current_frame_num, int, 0664);
...
#ifdef PTS_LOGGING
/* 3:2 , 2:2 and 4:1 pattern entries*/
module_param_array(pts_pattern_enter_cnt, int, &n_patterns, 0644);
MODULE_PARM_DESC(pts_pattern_enter_cnt, "pts_pattern_enter_cnt detection (3:2) (2:2) (4:1)");

module_param_array(pts_pattern_exit_cnt, int, &n_patterns, 0644);
MODULE_PARM_DESC(pts_pattern_exit_cnt, "pts_pattern_exit_cnt broken (3:2) (2:2) (4:1)");

module_param_array(pts_log_enable, int, &n_patterns, 0644);
MODULE_PARM_DESC(pts_log_enable, "pts_pattern_log_enable (3:2) (2:2) (4:1)");
