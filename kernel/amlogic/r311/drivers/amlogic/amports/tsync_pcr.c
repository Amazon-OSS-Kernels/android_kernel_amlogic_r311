...
#define MAX_SYNC_VGAP_TIME   (90000*3)
#define MIN_SYNC_VCHACH_TIME   45000

#define MAX_SYNC_AGAP_TIME   45000
#define MIN_SYNC_ACHACH_TIME   27000

#define PLAY_MODE_THRESHOLD 500
#define DEMUX_PCR_CORRECTION_THRESHOLD 20
...
/* reset control flag */
static u32 tsync_pcr_reset_flag;
...
void tsync_pcr_pcrscr_set(void)
{
	...
	if (first_apts && !(tsync_pcr_inited_flag & complete_init_flag)
		...
		init_check_first_demuxpcr = tsdemux_pcrscr_get();
...
int tsync_pcr_reset(int flag)
{
	tsync_pcr_reset_flag = flag;
	return 0;
}
...
static u8 tsync_process_checkspeed(void)
{
	...
	/* speed correction use pcr */
	/* need to correct pcr_scr in time */
	jiffies_diff = cur_jiffiestime - tsync_pcr_first_jiffes;
	demuxpcr_diff = (cur_demuxpcr - init_check_first_demuxpcr) / 90;

	delta = demuxpcr_diff - jiffies_diff;
	if (abs(delta) >= DEMUX_PCR_CORRECTION_THRESHOLD) {
		timestamp_pcrscr_set(timestamp_pcrscr_get() +
			delta * 90);
		tsync_pcr_first_jiffes = jiffies_to_msecs(jiffies);
		tsync_pcr_first_systime = tsdemux_pcrscr_get();
		if (tsync_pcr_debug & 0x1)
			pr_info("correction - speed diff too much :%d ms\n", delta);
	}

	/* speed correction use cache level */
	audio_discontinue = tsync_pcr_tsdemuxpcr_discontinue
				& AUDIO_DISCONTINUE;
	video_discontinue = tsync_pcr_tsdemuxpcr_discontinue
				& VIDEO_DISCONTINUE;

	if (audio_discontinue || video_discontinue)
		return PLAY_MODE_NORMAL;
	...
	audio_cache_ms = (checkin_apts - checkout_apts) / 90;
	video_cache_ms = (checkin_vpts - checkout_vpts) / 90;

	if (audio_cache_ms >= PLAY_MODE_THRESHOLD
		&& video_cache_ms >= PLAY_MODE_THRESHOLD) {
		timestamp_pcrscr_set(timestamp_pcrscr_get() +
			tsync_pcr_recovery_span);
		if (tsync_pcr_debug & 0x1)
			pr_info("correction - av cache too much data. a:%dms v:%dms\n",
				audio_cache_ms, video_cache_ms);
	}
...
static void tsync_process_discontinue(void)
{
	...
	u32 diff = (cur_apts > cur_vpts)?(cur_apts-cur_vpts)
			: (cur_vpts-cur_apts);

	int audio_discontinue = tsync_pcr_tsdemuxpcr_discontinue
				& AUDIO_DISCONTINUE;
	int video_discontinue = tsync_pcr_tsdemuxpcr_discontinue
				& VIDEO_DISCONTINUE;

	if (tsdemux_pcraudio_valid() == false
		|| tsdemux_pcrvideo_valid() == false) {
		return;
	}

	if (diff <= MAX_SYNC_VGAP_TIME) {
		if (audio_discontinue) {
			tsync_pcr_tsdemuxpcr_discontinue &= ~AUDIO_DISCONTINUE;
			timestamp_pcrscr_set(cur_apts);
			pr_info("av diff:%d less than:%d ms, remove audio discontinue flag\n"
				, diff, MAX_SYNC_VGAP_TIME);
		}
		...
		if (tsync_pcr_debug & 0x1)
			pr_info("[debug] av diff:%d too much. maybe discontinue. [a-%d v-%d]. cur_apts:%u cur_vpts:%u\n"
				, diff, audio_discontinue, video_discontinue,
				cur_apts, cur_vpts);
	}
}
...
void tsync_pcr_avevent_locked(enum avevent_e event, u32 param)
{
	...
	case VIDEO_TSTAMP_DISCONTINUITY: {
		...
		pr_info("[%s]video discontinue happen.discontinue=%d param=%x\n",
			__func__, tsync_pcr_tsdemuxpcr_discontinue, param);
		tsync_pcr_tsdemuxpcr_discontinue |= VIDEO_DISCONTINUE;
	...
	case AUDIO_START:
		...
		if (get_audio_reset() == 0) {
			...
			init_check_first_systemtime = cur_pcr;
			init_check_first_demuxpcr = tsdemux_pcrscr_get();
		}
		else
			pr_info("audio reset\n");
...
static unsigned long tsync_pcr_check(void)
{
	...
	if (tsync_pcr_first_jiffes != 0) {
			...
			tysnc_pcr_systime_diff = tsdemux_pcrscr_get()
			- tsync_pcr_first_systime;
	}
	...
	if (tsync_pcr_usepcr == 1) {
		...
		if (tsync_pcr_last_tsdemuxpcr != 0 && tsdemux_pcr != 0
			...
			need_recovery = 0;
			/* pcr discontinue - need recalc */
			tsync_pcr_first_jiffes = jiffies_to_msecs(jiffies);
			init_check_first_demuxpcr = tsdemux_pcr;
	...
	struct class_attribute *attr, char *buf)
{
		return sprintf(buf, "%d %d %d %s\n", tsync_pcr_jiffes_diff*90,
			...
			tsync_pcr_jiffes_diff-tysnc_pcr_systime_diff/90, "ms");
}
		...
		struct class_attribute *attr, char *buf)
{
	...
	if (tsync_pcr_reset_flag == 1)
		tsync_pcr_reset_flag = 0;
	 return size;
}

static ssize_t store_reset_flag(struct class *cla,
		struct class_attribute *attr,
		const char *buf, size_t count)
{
	size_t r;

	r = sscanf(buf, "%d", &tsync_pcr_reset_flag);
	pr_info("%s(%d)\n", __func__, tsync_pcr_reset_flag);
	if (r != 1)
		return -EINVAL;
	return count;
}
...
static struct class_attribute tsync_pcr_class_attrs[] = {
	...
	show_reset_flag, store_reset_flag),
	...
	__ATTR(tsync_pcr_debug, S_IRUGO | S_IWUSR,
