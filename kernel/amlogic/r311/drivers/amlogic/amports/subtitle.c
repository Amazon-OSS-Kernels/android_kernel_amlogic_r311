/*
 * drivers/amlogic/amports/subtitle.c
 *
...
static struct class_attribute subtitle_class_attrs[] = {
	...
	__ATTR(startpts, S_IRUGO | S_IWUSR | S_IWGRP, show_startpts,
	store_startpts),
	__ATTR(fps, S_IRUGO | S_IWUSR | S_IWGRP, show_fps, store_fps),
	__ATTR(subtype, S_IRUGO | S_IWUSR | S_IWGRP, show_subtype,
	store_subtype),
...
static long amsubtitle_ioctl(struct file *file, unsigned int cmd, ulong arg)
{
	...
	case AMSTREAM_IOC_SET_SUBTITLE_INFO: {
		...
		case SUB_INDEX:
			...
			break;
		case SUB_WRITE_POS:
			if (states->subtitle_info >= MAX_SUBTITLE_PACKET ||
				states->subtitle_info < 0)
				return -EINVAL;
			mutex_lock(&amsubtitle_mutex);
...
static int __init subtitle_init(void)
{
	...
	ret = alloc_chrdev_region(&amsub_devno, 0, AMSUBTITLE_DEVICE_COUNT,
							  DEVICE_NAME);
	if (ret < 0) {
		pr_info("amsub: faild to alloc major number\n");
		ret = -ENODEV;
		return ret;
	}
	...
	if (IS_ERR(amsubtitle_dev)) {
		amlog_level(LOG_LEVEL_ERROR,
			"## Can't create amsubtitle device\n");
		goto err4;
	}
