...
#define DEFAULT_VIDEO_BUFFER_SIZE_TVP       (1024 * 1024 * 6)
	...
	struct stream_buf_s *pvbuf)
{
	...
	if (pvbuf->for_4k) {
		...
		/* if less than 96M, use no more than 8M */
		if (codec_mm_get_total_size() < 96 * SZ_1M)
			pvbuf->buf_size = 8 * SZ_1M;
	} else {
...
static int amstream_port_release(struct port_priv_s *priv)
{
	...
	if (port->type & PORT_TYPE_MPTS) {
		tsync_pcr_stop();
		pr_info("amstream:  tsdemux_releasen\n");
		tsdemux_release();
	}
...
static long amstream_ioctl_get_ex(struct port_priv_s *priv, ulong arg)
{
	...
	case AMSTREAM_GET_EX_UD_POC:
		if (this->type & PORT_TYPE_USERDATA) {
			mutex_lock(&userdata_mutex);
			userdata_poc = userdata_poc_info[userdata_poc_ri];
	...
	case AMSTREAM_SET_PTR_AUDIO_INFO:
		if ((this->type & PORT_TYPE_VIDEO)
			...
			if ((parm.pdata_audio_info != NULL) &&
				access_ok(VERIFY_READ, parm.pdata_audio_info,
	...
	unsigned int cmd, ulong arg)
{
	...
	case AMSTREAM_IOC_VDECINFO:
			...
			return -EINVAL;
		{
			...
			memset(&para, 0, sizeof(struct am_io_info));
		...
		unsigned int cmd, struct am_ioctl_parm_ptr32 __user *arg)
{
	...
	if (!access_ok(VERIFY_WRITE, data, sizeof(*data)))
		return -EFAULT;
	if (!access_ok(VERIFY_WRITE, data32, sizeof(*data32)))
		return -EFAULT;

	if (get_user(value, &data32->cmd) ||
		put_user(value, &data->cmd) ||
		get_user(value, &data32->pointer) ||
		put_user(compat_ptr(value), &data->pointer) ||
		get_user(value, &data32->len) ||
		put_user(value, &data->len))
		return -EFAULT;
		...
		struct dec_sysinfo32 __user *arg)
{
	...
	if (get_user(param, &data32->param) ||
		put_user(compat_ptr(param), &data->param))
		return -EFAULT;
