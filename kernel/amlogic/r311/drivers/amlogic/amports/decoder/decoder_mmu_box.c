		...
		int start_release_index)
{
	...
	struct codec_mm_scatter *sc;
	if (box == NULL) {
		pr_err("box is null, can't free tail mmu box,idx:%d\n",
				idx);
		return -1;
	}
	if (idx < 0 || idx >= box->max_sc_num) {
...
int decoder_mmu_box_free_idx(void *handle, int idx)
{
	...
	struct codec_mm_scatter *sc;
	if (box == NULL) {
		pr_err("box is null, can't free idx of box,idx:%d\n",
				idx);
		return -1;
	}
	if (idx < 0 || idx >= box->max_sc_num) {
...
static int decoder_mmu_box_dump_all(void *buf, int size)
{
	...
	while (list != head) {
		...
		if (buf) {
			...
			pr_info("%s", sbuf);
			pbuf = sbuf;
			decoder_mmu_box_dump(box, NULL, 0);
			tsize = 0;
		}
		list = list->next;
		i++;
