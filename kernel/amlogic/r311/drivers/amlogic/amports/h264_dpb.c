	...
	if (!flush_flag) {
		for (i = 0; i < p_Dpb->used_size; i++) {
			if ((!p_Dpb->fs[i]->is_output) &&
				...
				(p_Dpb->fs[i]->is_used == 3)) {
				none_displayed_num++;
				if ((p_H264_Dpb->first_insert_frame
					== FirstInsertFrm_IDLE)
					&& (p_Dpb->fs[i]->is_used == 3)	&&
					(p_Dpb->last_output_poc == INT_MIN)) {
					p_H264_Dpb->first_insert_frame
							= FirstInsertFrm_OUT;
					p_H264_Dpb->first_output_poc
							= p_Dpb->fs[i]->poc;
					fast_output_flag = 1;
					dpb_print(p_H264_Dpb->decoder_index,
						PRINT_FLAG_DPB_DETAIL,
						"%s first insert frame i %d  poc %d frame_num %x\n",
						__func__, i, p_Dpb->fs[i]->poc,
