...
static unsigned int force_less_fragment_mode;
...
#endif
	int drm_mode;
	int less_fragment_buf_mode;
	wait_queue_head_t wait_q;
} /*hevc_stru_t */;

#ifdef AGAIN_HAS_THRESHOLD
u32 again_threshold = 0x0;
#endif
...
static int get_mv_buf(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (pic->width > 1920 || pic->height > 1088)
		new_size = MPRED_MV_BUF_SIZE;/*0x120000*/
	else
	...
	struct buf_stru_s *buf_stru)
{
	...
	if (get_double_write_mode(hevc)) {
		...
		if ((hevc->less_fragment_buf_mode > 0) &&/*no buffers < 1.5M.*/
			!hevc->mmu_enable &&
			(get_double_write_mode(hevc)
== 0x10)) {/*dw write only*/
			int min_buffer;
			if (hevc->less_fragment_buf_mode == 1)
				min_buffer = 3 * SZ_1M / 2;/*720p*/
			else if (hevc->less_fragment_buf_mode == 2)
				min_buffer = 3 * SZ_1M;/*1080p*/
			else
				min_buffer = 3 * SZ_1M  / 2;/*720p*/
...
{
	int used_buf_num = 0;
	/* in less fragment,used fixed max_buf_num
	 * so max_buf_num must bigger than refs.
	 * 265's spec max refs is 12.
	 * so max_buf_num must >= 13
	 */
	if (!hevc->less_fragment_buf_mode &&
		get_dynamic_buf_num_margin(hevc) > 0) {
		...
		*/
		...
		if ((hevc->sps_num_reorder_pics_0 > 0) &&
			hevc->sps_num_reorder_pics_0 < 6)
			used_buf_num = hevc->sps_num_reorder_pics_0 + 8;
		else
...
#ifdef RESET_BUFMGR_SUPPORT
static void h265_reset_bufmgr(struct hevc_state_s *hevc)
{
	int i;
	struct PIC_s *pic;
	for (i = 0; i < MAX_REF_PIC_NUM; i++) {
...
static inline void hevc_pre_pic(struct hevc_state_s *hevc,
			struct PIC_s *pic, u8 reset_bufmgr)
#else
static inline void hevc_pre_pic(struct hevc_state_s *hevc,
			struct PIC_s *pic)
#endif
			...
			struct PIC_s *pic)
{
	...
	vf->video_format = VFRAME_FORMAT_HEVC;
	vf->flag = 0;
...
static int prepare_display_buf(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (display_frame_count[hevc->index] == 0)
		pr_info("hevc decoder first frame ready\n");
...
static void vh265_check_timer_func(unsigned long arg)
{
	...
	if (dbg_cmd != 0) {
		if (dbg_cmd == 1) {
			...
			hevc_print(hevc, 0,
			"Test, reset bufmgr\n");
			hevc_pre_pic(hevc, NULL, 1);
			hevc->search_next_i = 1;
		}
#endif
		dbg_cmd = 0;
	}
...
static int vh265_local_init(struct hevc_state_s *hevc)
{
	...
	if (i_only_flag)
		...
		& 0x08)
		hevc->lowlatency_flag = 1;
	else
		hevc->lowlatency_flag = 0;
	hevc->error_watchdog_count = 0;
	hevc->sei_present_flag = 0;
	pts_unstable = ((unsigned long)hevc->vh265_amstream_dec_info.param
		& 0x40) >> 6;
	hevc_print(hevc, 0,
		"h265:pts_unstable=%d  aa lowlatency_flag= %d\n", pts_unstable,
							hevc->lowlatency_flag);
/*
TODO:FOR VERSION
*/
	hevc_print(hevc, 0,
...
static s32 vh265_init(struct hevc_state_s *hevc)
{
	...
	init_waitqueue_head(&hevc->wait_q);
#ifdef MULTI_INSTANCE_SUPPORT
	if (hevc->m_ins_flag) {
...
#else
		while (hevc->uninit_list != 2)	/* wait uninit complete */
			msleep(20);
#endif
...
#ifdef RESET_BUFMGR_SUPPORT
static unsigned char is_pic_in_displaying(struct hevc_state_s *hevc)
{
	struct PIC_s *pic;
	int i;
	/*return 1 if pic_list is not initialized yet*/
...
static void vh265_work(struct work_struct *work)
{
	...
	struct vdec_s *vdec = hw_to_vdec(hevc);
	int dec_result = hevc->dec_result;
	...
	wake_up_interruptible(&hevc->wait_q);
#ifdef CONFIG_AM_VDEC_DV
	...
	if (dec_result == DEC_RESULT_AGAIN)
		hevc->ready_resched = true;
...
static bool run_ready(struct vdec_s *vdec)
{
...
#endif
	if (hevc->eos || hevc->fatal_error) {
		not_run_ready[hevc->index]++;
		return 0;
	}

	if (hevc->ready_resched) {
		hevc->ready_resched = false;
		not_run_ready[hevc->index]++;
		return 0;
	}
	...
	if (!ret) {
		...
		__func__, ret);
#ifdef RESET_BUFMGR_SUPPORT
		if (!is_pic_in_displaying(hevc)) {
			hevc_print(hevc, 0,
			"Error, buf not available any more, reset bufmgr\n");
			hevc_pre_pic(hevc, NULL, 1);
			hevc->search_next_i = 1;
			not_run_ready[hevc->index]++;
			return 0;
...
#endif
	hevc->drm_mode = vdec_secure(hw_to_vdec(hevc));
	hevc->less_fragment_buf_mode = hevc->drm_mode ? 2 : 0;
	if (force_less_fragment_mode > 0)
		hevc->less_fragment_buf_mode = force_less_fragment_mode;
	if (hevc->less_fragment_buf_mode) {
		hevc_print(hevc, 0,
			"265 used less fragment buffer mode:%d\n",
			hevc->less_fragment_buf_mode);
	}
	if (pdata == NULL) {
		hevc_print(hevc, 0,
...
static void vh265_dump_state(struct vdec_s *vdec)
{
	...
	hevc_print(hevc, 0,
		"width/height (%d/%d), reorder_pic_num %d buf count(bufspec size) %d next_again_flag %x\n",
		hevc->frame_width,
		hevc->frame_height,
		hevc->sps_num_reorder_pics_0,
		get_work_pic_num(hevc),
		hevc->next_again_flag
		);

	hevc_print(hevc, 0,
		"is_framebase(%d), eos %d, fatal_error 0x%x dec_result 0x%x dec_frm %d disp_frm %d run %d not_run_ready %d input_empty %d\n",
		input_frame_based(vdec),
		hevc->eos,
		hevc->fatal_error,
		hevc->dec_result,
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	hevc->index = pdev->id;
	if (hevc->index < MAX_DECODE_INSTANCE_NUM)
		display_frame_count[hevc->index] = 0;
...
static int ammvdec_h265_remove(struct platform_device *pdev)
{
	...
	if (get_dbg_flag(hevc))
		hevc_print(hevc, 0, "%s\r\n", __func__);
	amhevc_stop();
	pr_info("%s, vdec->next_status %d,vdec->status %d\n",
			__func__, vdec->next_status, vdec->status);
	if (vdec->next_status == VDEC_STATUS_DISCONNECTED
				&& (vdec->status == VDEC_STATUS_ACTIVE)) {
			hevc->dec_result = DEC_RESULT_FORCE_EXIT;
			vdec_schedule_work(&hevc->work);
			wait_event_interruptible_timeout(hevc->wait_q,
...
module_param(force_less_fragment_mode, int, 0664);
MODULE_PARM_DESC(force_less_fragment_mode,
"\n amvdec_h265 force_less_fragment_mode\n");
