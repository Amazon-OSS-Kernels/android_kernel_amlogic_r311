...
static atomic_t decoder_reset = ATOMIC_INIT(0);
...
static void set_frame_info(struct vframe_s *vf)
{
	...
	if (first_frame_ready)
		temp = vmpeg12_amstream_dec_info.width;
	else
	...
	if (first_frame_ready)
		temp = vmpeg12_amstream_dec_info.height;
	else {
...
static irqreturn_t vmpeg12_isr(int irq, void *dev_id)
{
	...
	if (READ_VREG(AV_SCRATCH_H) != 0) {
		if (debug_enable & 0x2)
			pr_info("dbg %x: %x,\n",
		READ_VREG(AV_SCRATCH_H), READ_VREG(AV_SCRATCH_L));
	...
	if (reg) {
		...
		if (debug_enable & 0x1)
			pr_info("type:%d,offset %d\n",
			(info & PICINFO_TYPE_MASK), offset);
		...
		if (frame_prog & PICINFO_PROG) {
			...
			vf->canvas0Addr = vf->canvas1Addr =
						index2canvas(index);
			vf->video_format = VFRAME_FORMAT_MPEG12;
			vf->orientation = 0;
			...
			vfbuf_use[index] = 1;
			vf->error_flag = ((info & PICINFO_ERROR) > 0)?(1):(0);
			if ((error_skip(info, vf)) ||
				...
				vf->mem_handle =
					decoder_bmmu_box_get_mem_handle(
						...
						index);
				if (debug_enable & 0x1)
					pr_info("type:%d:w:h %d:%d, pts=%d:%lld, num=%d\n",
					(info & PICINFO_TYPE_MASK), vf->width,
					vf->height, vf->pts,
					vf->pts_us64, frame_num);
				frame_num++;
				kfifo_put(&display_q,
						  (const struct vframe_s *)vf);
				vf_notify_receiver(PROVIDER_NAME,
	...
	if (READ_VREG(AV_SCRATCH_L) == 0xf0) {
		WRITE_VREG(AV_SCRATCH_L, 0);
		if (debug_enable & 0x1)
			pr_info("res change reg =%d:%d:%d\n", reg,
		READ_VREG(MREG_PIC_WIDTH), READ_VREG(MREG_PIC_HEIGHT));
		vmpeg12_amstream_dec_info.height = READ_VREG(MREG_PIC_HEIGHT);
		vmpeg12_amstream_dec_info.width  = READ_VREG(MREG_PIC_WIDTH);
	}
	reg = READ_VREG(AV_SCRATCH_M);
	if (reg & (1<<16))
...
static void vmpeg12_local_init(void)
{
	...
	for (i = 0; i < DECODE_BUFFER_NUM_MAX; i++)
		vfbuf_use[i] = 0;
	if (atomic_read(&decoder_reset) == 0) {
		if (mm_blk_handle) {
...
{
	struct vdec_s *vdec = ((struct vdec_s *)(platform_get_drvdata(pdev)));
