...
static int lcd_info_print(char *buf, int offset)
{
	...
	len += snprintf((buf+len), n,
		...
		"fr_auto_policy: %d, vmode_vsync_en: %d\n",
		lcd_drv->version,
		pconf->lcd_propname, lcd_drv->chip_type,
...
/*tst_mode,    Y,       Cb,     Cr,     tst_en,  vfifo_en  rgbin*/
	...
	{0,    0x2cc,   0x2cc,  0x2cc,   1,      0,        3},  /* 9 */
	{0,    0x3ff,   0x3ff,  0x3ff,   1,      0,        3},  /* 10 */
};
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	case 't': /* test */
		...
		break;
	case 'i': /* info */
		print_buf = kzalloc(PR_BUF_MAX * sizeof(char), GFP_KERNEL);
		if (print_buf == NULL) {
	...
	case 'r':
		if (buf[2] == 'g') { /* reg */
			print_buf = kzalloc(PR_BUF_MAX * sizeof(char),
				GFP_KERNEL);
			if (print_buf == NULL) {
				LCDERR("%s: buf malloc error\n", __func__);
				return -EINVAL;
		...
		struct class_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "lcd resume flag: 0x%x(%s,%s)\n",
		lcd_drv->lcd_resume_type,
		(lcd_drv->lcd_resume_type & 0x1) ? "workqueue" : "directly",
		(lcd_drv->lcd_resume_type & 0x2) ? "resume" : "late_resume");
}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%x", &temp);
	if (ret == 1) {
		lcd_drv->lcd_resume_type = (unsigned char)temp;
		LCDPR("set lcd resume flag: 0x%x\n", lcd_drv->lcd_resume_type);
	} else {
		LCDERR("invalid data\n");
		return -EINVAL;
	}
...
static struct class_attribute lcd_debug_class_attrs[] = {
	...
	__ATTR(vlock,      S_IRUGO | S_IWUSR, lcd_debug_vlock_show, NULL),
	__ATTR(dump,      S_IRUGO | S_IWUSR,
