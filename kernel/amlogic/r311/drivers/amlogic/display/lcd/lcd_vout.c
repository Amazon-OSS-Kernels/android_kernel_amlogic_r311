	...
	char *metricsmsg)
{
	...
	snprintf(buf, sizeof(buf),
		...
		KERNEL_METRICS_GROUP_ID, KERNEL_DISPOUT_SCREEN_DRAIN_SCHEMA_ID,
		metricsmsg,
...
static void dummy_light_set(unsigned int action)
{
	switch (action) {
	case 0:
		LCDPR("%s: DISPLAY is OFF", __func__);
		bq_log_metrics("Screen on drainage", "screen_on_drain");
		break;
	case 1:
		LCDPR("%s: DISPLAY is ON", __func__);
...
static int lcd_config_probe(void)
{
	...
	/* default workqueue, resume */
	lcd_driver->lcd_resume_type = (1 << 0) | (0 << 1);
...
{
	if ((lcd_driver->lcd_resume_type & 0x2) == 0)
		return 0;

	/*queue_work(lcd_driver->workqueue, &(lcd_driver->lcd_resume_work));*/
	if (lcd_driver->lcd_resume_type & 0x1) {
		lcd_resume_flag = 1;
		if (lcd_driver->workqueue) {
			queue_delayed_work(lcd_driver->workqueue,
