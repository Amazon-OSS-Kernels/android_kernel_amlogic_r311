/*
 * drivers/amlogic/display/lcd/lcd_clk_config.c
 *
...
static void lcd_clk_config_chip_init(void)
{
	...
	case LCD_CHIP_M8B:
		...
		break;
	case LCD_CHIP_G9TV:
		cConf->od_fb = PLL_FRAC_OD_FB_G9TV;
		...
		break;
	case LCD_CHIP_G9BB:
		cConf->od_fb = PLL_FRAC_OD_FB_G9BB;
...
static void lcd_set_pll_ss_txlx(struct lcd_clk_config_s *cConf)
{
	...
	/* direct write register */
	if (cConf->ss_level >= 10) { /* custom */
	...
	for (od3_sel = cConf->pll_od_sel_max; od3_sel > 0; od3_sel--) {
		...
		for (od2_sel = od3_sel; od2_sel > 0; od2_sel--) {
			...
			for (od1_sel = od2_sel; od1_sel > 0; od1_sel--) {
				...
				cConf->pll_fvco = pll_fvco;
				n = 1;
				if (lcd_drv->chip_type == LCD_CHIP_TXL) {
					if (pll_fvco < 3700000)
						od_fb = 0;
					else
						od_fb = 1;
...
char *lcd_get_spread_spectrum(void)
{
	...
	case LCD_CHIP_M8M2:
		...
		sprintf(lcd_ss_str, "%s", lcd_pll_ss_table_m8[ss_level]);
		break;
	case LCD_CHIP_M8B:
		ss_level = (ss_level >= clk_conf.ss_level_max) ? 0 : ss_level;
		sprintf(lcd_ss_str, "%s", lcd_pll_ss_table_m8b[ss_level]);
		break;
	case LCD_CHIP_G9TV:
		ss_level = (ss_level >= clk_conf.ss_level_max) ? 0 : ss_level;
		sprintf(lcd_ss_str, "%s", lcd_pll_ss_table_g9tv[ss_level]);
		break;
	case LCD_CHIP_G9BB:
		ss_level = (ss_level >= clk_conf.ss_level_max) ? 0 : ss_level;
		sprintf(lcd_ss_str, "%s", lcd_pll_ss_table_g9bb[ss_level]);
		break;
	case LCD_CHIP_GXTVBB:
		ss_level = (ss_level >= clk_conf.ss_level_max) ? 0 : ss_level;
	...
	case LCD_CHIP_TXLX:
		if (ss_level >= 10) {
			...
			ss_level = (ss_level >= clk_conf.ss_level_max) ?
				0 : ss_level;
			sprintf(lcd_ss_str, "%s",
...
void lcd_set_spread_spectrum(void)
{
	...
	case LCD_CHIP_M8:
	case LCD_CHIP_M8M2:
		clk_conf.ss_level = (ss_level >= clk_conf.ss_level_max) ?
			0 : ss_level;
