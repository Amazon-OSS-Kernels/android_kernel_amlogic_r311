...
struct vinfo_s *get_current_vinfo(void)
{
	...
	if (!IS_ERR_OR_NULL(vout_module.curr_vout_server)) {
...
int set_vframe_rate_hint(int duration)
{
	...
	if (!IS_ERR_OR_NULL(p_server)) {
		if (p_server->op.set_vframe_rate_hint) {
			r = p_server->op.set_vframe_rate_hint(duration);
			/* mutex_unlock(&vout_mutex); */
			return r;
		}
	}
...
int set_vframe_rate_end_hint(void)
{
	...
	if (!IS_ERR_OR_NULL(p_server)) {
		if (p_server->op.set_vframe_rate_end_hint) {
			r = p_server->op.set_vframe_rate_end_hint();
			/* mutex_unlock(&vout_mutex); */
			return r;
		}
	}
...
int set_vframe_rate_policy(int policy)
{
	...
	if (!IS_ERR_OR_NULL(p_server)) {
		if (p_server->op.set_vframe_rate_policy) {
			r = p_server->op.set_vframe_rate_policy(policy);
			/* mutex_unlock(&vout_mutex); */
			return r;
		}
	}
...
int vout_suspend(void)
{
	...
	if (!IS_ERR_OR_NULL(vout_module.curr_vout_server)) {
		if (vout_module.curr_vout_server->op.vout_suspend)
			ret = vout_module.curr_vout_server->op.vout_suspend();
	}
...
int vout_resume(void)
{
	...
	if (!IS_ERR_OR_NULL(vout_module.curr_vout_server)) {
		if (vout_module.curr_vout_server->op.vout_resume) {
			/* ignore error when resume. */
			vout_module.curr_vout_server->op.vout_resume();
		}
	}
	...
	list_for_each_entry(p_server, &vout_module.vout_server_list, list) {
		...
		if (true == p_server->op.vmode_is_supported(mode)) {
			...
			if (p_server->op.get_vinfo)
				update_vout_mode_attr(p_server->op.get_vinfo());
