...
static struct switch_dev free_scale_state = {
	.name = "free_scale_state",
};

static struct switch_dev window_axis_state = {
	.name = "window_axis_state",
};
	...
	case FBIOPUT_OSD_SYNC_RENDER_ADD:
		{
			...
			if (sync_request_render.shared_fd == -1) {
				addr = get_fb_rmem_paddr(DEV_OSD0);
				ret = 0;
			} else {
				...
				const char *buf, size_t count)
{
	...
	if (fb_info->node == DEV_OSD0) {
		if (free_scale_enable == 0)
			switch_set_state(&free_scale_state, 0);
		else
			switch_set_state(&free_scale_state, 1);
	}
	return count;
}
				 ...
				 const char *buf, size_t count)
{
	...
	if (fb_info->node == DEV_OSD0) {
        switch_set_state(&window_axis_state, 1);
	}
	return count;
}
...
static ssize_t show_osd_reg(struct device *device,
				struct device_attribute *attr,
				char *buf)
{
	struct fb_info *fb_info = dev_get_drvdata(device);
	char reg_buf[PAGE_SIZE/2];
	int n = 0;

	n += snprintf(buf, PAGE_SIZE, "OSD<%d> register dump\n", fb_info->node);

	memset(reg_buf, 0, PAGE_SIZE/2);
	osd_dump_regs(reg_buf);
...
static struct device_attribute osd_attrs[] = {
	...
	__ATTR(ver_update_pan, S_IWUSR | S_IWGRP,
	...
	__ATTR(osd_regdump, S_IRUGO | S_IRUSR,
			show_osd_reg, NULL),
};
...
static void mem_free_work(struct work_struct *work)
{
	...
	if (b_cma) {
		...
		if (fb_rmem.base) {
#endif
			if (fb_rmem.size >= (fb_memsize[0] +
				fb_memsize[1] + fb_memsize[2])) {
				/* logo memory before fb0/fb1 memory */
...
static int osd_probe(struct platform_device *pdev)
{
	...
	iret = of_property_read_u32_array(pdev->dev.of_node,
	...
	iret = of_reserved_mem_device_init(&pdev->dev);
	if (iret != 0)
	...
	iret = of_property_read_string(pdev->dev.of_node,
	...
	switch_dev_register(&free_scale_state);
	switch_set_state(&free_scale_state, 0);

	switch_dev_register(&window_axis_state);
	switch_set_state(&window_axis_state, 0);
...
static int osd_remove(struct platform_device *pdev)
{
	...
	switch_dev_unregister(&free_scale_state);
	switch_dev_unregister(&window_axis_state);
	return 0;
}
...
{
	if (of_flat_dt_is_compatible(rmem->fdt_node, "shared-dma-pool"))
		b_cma = true;
	else
		b_cma = false;
