...
static atomic_t cur_fen_timeline_val = ATOMIC_INIT(0);
/* thread control part */
...
static int out_fence_create(int *release_fence_fd, u32 *val, u32 buf_num)
{
	...
	if (out_fence_fd < 0) {
		osd_log_err("failed to get fd for out fence\n");
		return -1;
	}
...
int osd_sync_request(u32 index, u32 yres, struct fb_sync_request_s *request)
{
	...
	buf_num = find_buf_num(yres, request->yoffset);
	if (!fence_map) {
	...
	fence_map->yoffset = request->yoffset;
	fence_map->xoffset = request->xoffset;
	fence_map->yres = yres;
	fence_map->format = request->format;
	fence_map->in_fence = sync_fence_fdget(request->in_fen_fd);

	if (out_fence_create(&out_fence_fd, &fence_map->out_pt_val, buf_num)
		< 0) {
		fence_map->out_pt_val = 0;
		out_fence_fd = -1;
	}
...
{
	s32 ret = -1;
	struct sync_fence *buf_ready_fence = fence_map->in_fence;
...
static inline void wait_vsync_wakeup(void)
{
	...
	if (timeline_created) {
		u32 cur_val = atomic_xchg(&cur_fen_timeline_val, 0);
		if (cur_val > timeline->value+1) {
			sw_sync_timeline_inc(timeline,
				cur_val-timeline->value-1);
		}
	}
...
s64 osd_wait_vsync_event(void)
{
	...
	if (pxp_mode)
		...
		timeout = msecs_to_jiffies(1000); /* waiting for 1000ms. */
		  ...
		  const struct color_bit_define_s *color)
{
	...
	memset(&pan_data, 0x0, sizeof(struct pandata_s));
	memset(&disp_data, 0x0, sizeof(struct pandata_s));
	pan_data.x_start = xoffset;
	pan_data.y_start = yoffset;
...
{
	osd_log_info("osd_set_free_scale_enable_hw\n");
...
void osd_set_free_scale_mode_hw(u32 index, u32 freescale_mode)
{
	...
	osd_log_info("osd_set_free_scale_mode_hw\n");
}
...
void osd_get_window_axis_hw(u32 index, s32 *x0, s32 *y0, s32 *x1, s32 *y1)
{
	...
	if (vinfo) {
		...
		case VMODE_1080I_50HZ:
			...
			height *= 2;
			*y0 = osd_hw.free_dst_data_backup[index].y_start * 2;
			*y1 = height + *y0 - 1;
			break;
		default:
...
void osd_wait_vsync_hw_fence(struct osd_fence_map_s *fence_map)
{
	unsigned long timeout;
	...
	atomic_set(&cur_fen_timeline_val, fence_map->out_pt_val);
	wait_event_interruptible_timeout(osd_vsync_wq,
		atomic_read(&cur_fen_timeline_val) == 0, timeout);
}
...
static void osd_pan_display_fence(struct osd_fence_map_s *fence_map)
{
	...
	if (ret) {
		...
		if (fence_map->ext_addr && fence_map->width
			...
			spin_unlock_irqrestore(&osd_lock, lock_flags);
			osd_wait_vsync_hw_fence(fence_map);
			...
			/* switch from Direct render to GPU render
			 * need update color format
			 */
...
static void osd1_basic_update_disp_geometry(void)
{
	...
	if (osd_hw.scale[OSD1].h_enable || osd_hw.scale[OSD1].v_enable) {
		...
		data32 = VSYNCOSD_RD_MPEG_REG(VIU_OSD1_BLK0_CFG_W1);
		buffer_w = ((data32 >> 16) & 0x1fff) - (data32 & 0x1fff) + 1;
		data32 = VSYNCOSD_RD_MPEG_REG(VIU_OSD1_BLK0_CFG_W2);
...
void osd_init_hw(u32 logo_loaded)
{
	...
	osd_hw.free_scale_enable_backup[OSD1] =
		osd_hw.free_scale_enable_backup[OSD2] = 0;
	...
	osd_hw.free_scale_backup[OSD1].h_enable = 0;
	osd_hw.free_scale_backup[OSD1].h_enable = 0;
	osd_hw.free_scale_backup[OSD2].v_enable = 0;
	osd_hw.free_scale_backup[OSD2].v_enable = 0;
	...
	*/
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_M8) {
		...
		osd_hw.free_src_data[OSD2].y_end = 0;
		osd_hw.free_src_data_backup[OSD1].x_start = 0;
		osd_hw.free_src_data_backup[OSD1].x_end = 0;
		osd_hw.free_src_data_backup[OSD1].y_start = 0;
		osd_hw.free_src_data_backup[OSD1].y_end = 0;
		osd_hw.free_src_data_backup[OSD2].x_start = 0;
		osd_hw.free_src_data_backup[OSD2].x_end = 0;
		osd_hw.free_src_data_backup[OSD2].y_start = 0;
