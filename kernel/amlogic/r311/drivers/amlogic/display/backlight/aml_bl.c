...
static unsigned int bypass_poweron;
...
static atomic_t bl_init = ATOMIC_INIT(0);
...
static int aml_bl_check_driver(void)
{
	...
	if ((bl_drv == NULL) || !atomic_read(&bl_init)) {
		BLERR("no bl driver\n");
		return -1;
	}
 ...
 * lcd notify
 * **************************************** */
static atomic_t zygote_restart_times = ATOMIC_INIT(0);

int get_backlight_status(void)
{
	if ((bl_drv->state & BL_STATE_BL_ON) == 1)
		return 1;
	else
		return 0;
}

void add_zygote_restart_times(void)
{
	atomic_set(&zygote_restart_times, 1);
}

static int clear_zygote_restart_times(void)
{
	int restart_times;

	restart_times = atomic_read(&zygote_restart_times);
	if (restart_times > 0) {
		BLPR("restart_times: %d\n", restart_times);
		atomic_set(&zygote_restart_times, 0);
		return -1;
	}
	return 0;
}
...
{
	unsigned int level_brightness = bl_drv->bldev->props.brightness;
	unsigned int level_default = bl_drv->bconf->level_default;
	int ret;

	ret = clear_zygote_restart_times();
	if (ret)
		return;
	...
	if (brightness_bypass) {
		...
		if (bl_step_on_flag && (level_default > level_brightness))
			aml_bl_step_on(level_default);
...
void update_silent_ota_flag(void)
{
	if ((bl_drv->state & BL_STATE_BL_ON) == 1) {
		BLPR("Backlight is ON so clear the silent_ota flag.\n");
		amlogic_clear_silent_ota_flag();
	} else {
		BLPR("Backlight is OFF so set the silent_ota flag.\n");
		amlogic_set_silent_ota_flag();
	}
}
...
static void set_backlight(unsigned char on)
{
	if (on) {
		/* Check the bl driver */
		if (bl_drv == NULL) {
			BLPR("BL driver don't init\n");
			return;
		}

		if ((bl_drv->state & BL_STATE_LCD_ON) == 0) {
			BLPR("LCD IS OFF, so can't turn on backligh.\n");
			return;
		}
		bypass_poweron = 0;
		bl_power_on();
		BLPR("Turn on backlight\n");
	} else {
		bl_power_off();
		BLPR("Turn off backlight\n");
	}
}
...
unsigned int toggle_backlight(unsigned int value)
{
	...
	if (boot_complete == 1) {
		BLPR("Device Booted, Skip backlight toggle\n");
		return ret;
	}
...
ssize_t backlight_status_proc_write(struct file *file, const char __user *buf,
		size_t count, loff_t *ppos)
{
	toggle_backlight(OTA_TOGGLE_BACKLIGHT);
	return count;
}
...
ssize_t boot_complete_proc_write(struct file *file, const char __user *buf,
		size_t count, loff_t *ppos)
{
	int n = 0;
	char buffer[2];
	...
	buffer[1] = '\0';
	pr_warn("write boot_completed  %s %d\n", buffer, buffer[0]);
	bypass_poweron = 0;
	n = sscanf(buffer, "%d", &boot_complete);
	if (n != 1)
		return -EINVAL;
...
static int aml_bl_probe(struct platform_device *pdev)
{
	...
	/* Silent OTA */
	pr_warn("Inital backlight_status is %d\n", backlight_status);
	if (!backlight_status) {
		bl_drv->state &= ~BL_STATE_BL_ON;
		bypass_poweron = 1;
	}
	proc_create("boot_completed", 0664, NULL, &boot_complete_fops);
	proc_create("backlight_status", 0664, NULL,
			&backlight_status_proc_fops);
	atomic_set(&bl_init, 1);
	BLPR("probe OK\n");
	return 0;
err:
	kfree(bl_drv);
...
static int __init bl_status_setup(char *str)
{
	int ret = 0;

	if (str != NULL)
		ret = kstrtouint(str, 10, &backlight_status);

	BLPR("backlight_status: %d\n", backlight_status);

	return ret;
}
__setup("bl_status=", bl_status_setup);
