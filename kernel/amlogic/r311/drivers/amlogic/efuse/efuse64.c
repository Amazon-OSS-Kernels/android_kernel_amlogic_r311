...
#include <linux/arm-smccc-linaro.h>

#include <linux/amlogic/secmon.h>
#include "efuse.h"
#if ((defined CONFIG_ARM64) || (defined CONFIG_ARM64_32))
...
static ssize_t store_usb_boot(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtou32(buf, 16, &states)) {
		pr_err("write value is valid.\n");
		return -EINVAL;
	}

	if (states == 1) {
		arm_smccc_smc(EFUSE_OBJ_READ,
			EFUSE_OBJ_LICENSE_DISABLE_USB_BOOT,
			32, 0, 0, 0, 0, 0, &res);
		rc = res.a0;

		if (rc == EFUSE_OBJ_SUCCESS &&
			((unsigned char *)sharemem_output_base)[0] == 0) {
			arm_smccc_smc(EFUSE_OBJ_WRITE,
				EFUSE_OBJ_LICENSE_DISABLE_USB_BOOT,
				0, 0, 0, 0, 0, 0, &res);
			rc = res.a0;
			if (rc != EFUSE_OBJ_SUCCESS) {
				pr_err("error setting efuse object. %d\n", rc);
				return -EINVAL;
			}
		} else {
			pr_info("read efuse failed or the efuse is enabled\n");
			return -EINVAL;
		}
	} else {
		pr_info("only write 1 is valid.\n");
		return -EINVAL;
	}
...
static ssize_t store_usb_password_protect(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	struct arm_smccc_res res;
	u32 rc = 0;
...
static ssize_t show_usb_boot(struct class *class,
	struct class_attribute *attr, char *buf)
{
	struct arm_smccc_res res;
	u32 rc = 0;
	...
	arm_smccc_smc(EFUSE_OBJ_READ, EFUSE_OBJ_LICENSE_DISABLE_USB_BOOT,
		      32, 0, 0, 0, 0, 0, &res);
	rc = res.a0;

	if (rc == EFUSE_OBJ_SUCCESS) {
		len = sprintf(buf, "%02x\n",
			((unsigned char *)sharemem_output_base)[0]);
	} else {
		pr_err("Error getting efuse object: %d\n", rc);
	}

	return len;
}

static ssize_t show_usb_password_protect(struct class *class,
	struct class_attribute *attr, char *buf)
{
	struct arm_smccc_res res;
	u32 rc = 0;
...
static struct class_attribute efuse_class_attrs[] = {
	...
	__ATTR(usb_boot_disable,  S_IWUSR | S_IRUSR, show_usb_boot, store_usb_boot),

	__ATTR(usb_password_protect, S_IWUSR | S_IRUSR, show_usb_password_protect, store_usb_password_protect),
...
int get_efusekey_info(struct device_node *np)
{
	...
	for (index = 0; index < efusekeynum; index++) {
		...
		memset(efusekey_infos[index].keyname, 0x00,
			sizeof(efusekey_infos[index].keyname));
		strncpy(efusekey_infos[index].keyname, uname,
			strlen(uname) >= sizeof(efusekey_infos[index].keyname) ?
			sizeof(efusekey_infos[index].keyname)-1:strlen(uname));
