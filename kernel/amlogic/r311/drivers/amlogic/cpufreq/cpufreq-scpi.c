...
static bool meson_cpufreq_boost;
...
int meson_cpufreq_enable_boost(bool enable)
{
	struct cpufreq_policy *policy;
	if (enable) {
		meson_cpufreq_boost = true;
		policy = cpufreq_cpu_get(0);
		if (!policy) {
			pr_err("no policy for cpu0\n");
			return 0;
		}
		cpufreq_driver_target(policy, policy->max, CPUFREQ_RELATION_L);
		cpufreq_cpu_put(policy);
		pr_info("cpufreq_boost enabled\n");
	} else {
		meson_cpufreq_boost = false;
		pr_info("cpufreq_boost disabled\n");
	}
	return 1;
}
EXPORT_SYMBOL(meson_cpufreq_enable_boost);
