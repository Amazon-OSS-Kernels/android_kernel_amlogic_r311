	...
	int size, int flags)
{
	...
	while (!codec_mm_is_lessmem() && !have_alloced) {
...
#define BUFPRINT(args...) \
	...
	{
		uint64_t temp = smgt->alloc_total_us;
		int average_timeus = 0;
		do_div(temp, smgt->alloc_cnt);
...
static int codec_mm_scatter_mgt_alloc_in(struct codec_mm_scatter_mgt **psmgt)
{
	...
	if (codec_mm_is_lessmem()) {
		smgt->reserved_block_mm_M = 10;
		smgt->keep_size_PAGE = 10 * SZ_1M >> PAGE_SHIFT;
		smgt->enable_slot_from_sys = 0;
	} else {
		smgt->reserved_block_mm_M = 40;
