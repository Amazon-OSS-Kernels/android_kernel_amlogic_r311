	...
	case KEYUNIFY_GET_INFO:
		{
			...
			key_item_info->name[KEY_UNIFY_NAME_LEN - 1] = 0x00;
	...
	size_t count)
{
	...
	if (count <= 0) {
		pr_err(" count=0x%zx is invalid in %s\n", count, __func__);
		return -EINVAL;
	}
...
static int aml_unifykeys_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		pr_err("unifykey: failed to parse device tree\n");
		goto out;
	}
