...
static int disable_alarm;
module_param_named(disable_alarm, disable_alarm, int, 0664);
...
static int aml_rtc_write_time(struct device *dev, struct rtc_time *tm)
{
	...
	rtc_tm_to_time(tm, &time1);
	pr_debug("aml_rtc : write the rtc time, time is %ld\n", time1);

	if (timere_low_vaddr && timere_high_vaddr)
		time = timere_read();
	vrtc_init_date = (unsigned int)time1 - time;
...
static int aml_vrtc_probe(struct platform_device *pdev)
{
	...
	debugfs_create_u32("rtc_str_time", 0644, NULL, &rtc_str_time);
	return 0;
}
