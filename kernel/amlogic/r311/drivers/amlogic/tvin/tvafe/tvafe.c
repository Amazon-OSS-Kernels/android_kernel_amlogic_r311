...
static int cutwindow_val_h_level4 = 20;/*48-->62 for ntsc-m*/
#else
...
/*only for codec_mem mode*/
static unsigned int tvafe_mem_release = 1;
module_param(tvafe_mem_release, uint, 0444);
MODULE_PARM_DESC(tvafe_mem_release, "tvafe_mem_release");
...
static void tvafe_state(struct tvafe_dev_s *devp)
{
	...
	pr_info("devp->mem.start:0x%x\n", devp->mem.start);
	pr_info("devp->mem.end:%d\n", devp->mem.start + devp->mem.size);
	pr_info("devp->mem.size:0x%x\n", devp->mem.size);
#endif
...
void tvafe_dec_start(struct tvin_frontend_s *fe, enum tvin_sig_fmt_e fmt)
{
	...
	if (fmt == TVIN_SIG_FMT_CVBS_NTSC_M &&
		...
		W_HIU_REG(HHI_CADC_CNTL2, 0xf);
		W_HIU_REG(HHI_DADC_CNTL2, 0x00000426);
	}
...
void tvafe_dec_stop(struct tvin_frontend_s *fe, enum tvin_port_e port)
{
	...
	pr_info("[tvafe..] %s stop port:0x%x in.\n", __func__, port);
	mutex_lock(&devp->afe_mutex);
	if (!(devp->flags & TVAFE_FLAG_DEV_STARTED)) {
