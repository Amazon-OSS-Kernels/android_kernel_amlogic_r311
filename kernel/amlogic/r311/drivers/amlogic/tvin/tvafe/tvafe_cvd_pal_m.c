...
#ifdef CONFIG_AM_TRY_FORMAT_MAX_AV
static unsigned int config_force_fmt;
#else
static unsigned int config_force_fmt = 3;
#endif
...
#ifdef CONFIG_AM_TRY_FORMAT_MAX_AV
static unsigned int try_format_max_atv = 26;
module_param(try_format_max_atv, uint, 0664);
MODULE_PARM_DESC(try_format_max_atv, "try_format_max_atv");

static unsigned int try_format_max_av = 6;
module_param(try_format_max_av, uint, 0664);
MODULE_PARM_DESC(try_format_max_av, "try_format_max_av");
#else
static unsigned int try_format_max = 6;
...
#ifdef CONFIG_AM_AV_CH_WHITE_PATTERN_EN
static unsigned int no_burst_adj;
module_param(no_burst_adj, uint, 0664);
MODULE_PARM_DESC(no_burst_adj, "no_burst_adj");
#endif
				...
				enum tvin_sig_fmt_e fmt)
{
	...
	if (vbi_mem_start != 0) {
		...
		pr_info("[tvafe..] %s:vbi mem:0x%x\n",
			__func__, vbi_start);
	} else {
		vbi_start = cvd2_addr + (vbi_offset >> 4);
		pr_info("[tvafe..] %s:vbimem fromafe:0x%x\n",
			__func__, vbi_start);
	}
#else
	vbi_start = cvd2_addr + (vbi_offset >> 4);
		...
		struct tvafe_cvd2_mem_s *mem)
{
	...
	/*cvd filter config, @20180420 new add for ntsc use level 1*/
	if (((cvd2->vd_port == TVIN_PORT_CVBS1) ||
		(cvd2->vd_port == TVIN_PORT_CVBS2)) &&
		(cvd2->config_fmt == TVIN_SIG_FMT_CVBS_NTSC_M)) {
		cvd2_filter_config_level = 1;
		W_APB_REG(ACD_REG_53, 0x50502040);
	} else
		cvd2_filter_config_level = 0;
...
#ifdef CONFIG_AM_AV_SECAM_CVD2
		else if (cvd2->config_fmt == TVIN_SIG_FMT_CVBS_SECAM) {
			W_APB_REG(CVD2_CHROMA_DTO_INCREMENT_23_16, 0xbf);
			W_APB_REG(CVD2_COMB_FILTER_THRESHOLD1, 0xc0);
			W_APB_REG(CVD2_CHROMA_EDGE_ENHANCEMENT, 0x82);
		}
#endif
	}

	/*@20180420 new add for ntsc color*/
	if (((cvd2->vd_port == TVIN_PORT_CVBS1) ||
		(cvd2->vd_port == TVIN_PORT_CVBS2)) &&
		(cvd2->config_fmt == TVIN_SIG_FMT_CVBS_NTSC_M))
		W_APB_REG(CVD2_CONTROL1, 0x5);
	...
	W_APB_REG(CVD2_VBI_FRAME_CODE_CTL, 0x00000011);
...
#ifdef CONFIG_AM_AV_CH_WHITE_PATTERN_EN
	/* this code for no color burst pattern occur offset */
	if ((cvd2->hw_data[cvd2->hw_data_cur].no_color_burst ||
	    (cvd2->hw.acc3xx_cnt < 40)) && !no_burst_adj) {
		if (R_APB_REG(CVD2_REG_87) != 0x8)
			W_APB_REG(CVD2_REG_87, 0x8);
	} else {
...
static bool tvafe_cvd2_condition_shift(struct tvafe_cvd2_s *cvd2)
{
...
#else
				W_APB_BIT(CVD2_PAL_DETECTION_THRESHOLD, 0x2,
					PAL_DET_TH_BIT, PAL_DET_TH_WID);
#endif
...
#ifdef CONFIG_AM_AV_ADD_CHROME_LOCK_DETECT
	/* fake chroma lock */
	if (((cvd2->vd_port == TVIN_PORT_CVBS1) ||
	    (cvd2->vd_port == TVIN_PORT_CVBS2)) &&
	    (cvd2->config_fmt == TVIN_SIG_FMT_CVBS_PAL_I)) {
		if (cvd2->hw.chroma_lock && cvd2->hw.h_lock &&
			...
			cvd2->hw.line625 && !cvd2->hw.no_color_burst) {
	...
	if (ret) {
		...
		if (cvd_dbg_en)
#ifdef CONFIG_AM_TRY_FORMAT_MAX_AV
			pr_info("[tvafe..] %s: pal is %d,secam flag is %d, 358:%d, 443:%d changed.\n",
			__func__, cvd2->hw.pal, cvd2->hw.secam,
			cvd2->hw.fsc_358, cvd2->hw.fsc_443);
#else
 ...
 */
static int cvd_manual_fmt_frint;
static int cvd_ntsc_retry;
			...
			struct tvafe_cvd2_mem_s *mem)
{
	...
	if ((cvd2->vd_port == TVIN_PORT_CVBS3) ||
	    (cvd2->vd_port == TVIN_PORT_CVBS0))
		try_format_max = try_format_max_atv;
	else
		try_format_max = try_format_max_av;
#endif
	...
	if (cvd2->info.state == TVAFE_CVD2_STATE_INIT) {
		...
		if (tvafe_cvd2_sig_unstable(cvd2)) {
			...
			if (!cvd_pr_flag && cvd_dbg_en) {
				cvd_pr_flag = true;
				pr_info("[tvafe..] %s: sig unstable,nosig:%d,h-lock:%d,v-lock:%d, config_fmt:%s(%d), manual_fmt:%s(%d)\n",
		...
		if (cvd2->info.state_cnt && cvd_dbg_en) {
			pr_info("[tvafe..] %s: sig stable, state_cnt:%d, config_fmt:%s(%d), manual_fmt:%s(%d)\n",
				__func__, cvd2->info.state_cnt,
				tvin_sig_fmt_str(cvd2->config_fmt),
				cvd2->config_fmt,
				tvin_sig_fmt_str(cvd2->manual_fmt),
				cvd2->manual_fmt);
		...
		if (cvd2->manual_fmt) {
			...
			if (cvd_manual_fmt_frint == 0) {
				pr_info("[tvafe..] %s: manual fmt is:%s,do not need try other format!!!\n",
				__func__, tvin_sig_fmt_str(cvd2->manual_fmt));
				cvd_manual_fmt_frint = 1;
			}
			return;
		}
		cvd_manual_fmt_frint = 0;
		/* auto mode */
		if (cvd_dbg_en)
			pr_info("[tvafe..] %s: current fmt:%s,hnon:%d,vnon:%d,c-lk:%d,pal:%d,secam:%d,h-lk:%d,v-lk:%d,fsc358:%d,fsc425:%d,fsc443:%d,secam detected %d,line625:%d\n",
			__func__, tvin_sig_fmt_str(cvd2->config_fmt),
		...
		if (++try_format_cnt == try_format_max) {
			if (cvd_dbg_en)
				pr_info("[tvafe..] %s: fmt %s try_format_cnt %d reached try_format_max %d, force_fmt:%d!!!\n",
					__func__,
					tvin_sig_fmt_str(cvd2->config_fmt),
					try_format_cnt, try_format_max,
					config_force_fmt);
			...
			force_fmt_flag = 1;
			if (cvd_dbg_en)
				pr_info("[tvafe..] %s: force to fmt %s!!!\n",
					__func__,
					tvin_sig_fmt_str(cvd2->config_fmt));
			return;
		}
 ...
 */
#ifdef CONFIG_AM_ATV_CHANGE_SCREEN_SIZE
inline bool tvafe_cvd2_no_sig(struct tvafe_cvd2_s *cvd2,
			struct tvafe_cvd2_mem_s *mem, bool adj_can_run)
#else
inline bool tvafe_cvd2_no_sig(struct tvafe_cvd2_s *cvd2,
			struct tvafe_cvd2_mem_s *mem)
#endif
...
#ifdef TVAFE_CVD2_AUTO_DE_ENABLE
#ifdef CONFIG_AM_ATV_CHANGE_SCREEN_SIZE
	if (adj_can_run && (auto_de_en || auto_vs_en)) {
#else
			...
			unsigned int hcnt64)
{
	...
	unsigned int diff, hcnt64_ave, i, hcnt64_standard = 0;
	...
	if (cvd2->info.hcnt64_cnt == 300) {
		...
		if (diff > hs_adj_th_level0) {
			...
			#ifdef CONFIG_AM_ATV_SYNC_SCAN_RANGE
...
#ifndef CONFIG_AM_AV_CH_WHITE_PATTERN_EN
			if (R_APB_REG(CVD2_REG_87) != 0x0)
				W_APB_REG(CVD2_REG_87, 0x0);
#endif
