...
unsigned char field_data_flag;

/******debug********/
/*bit0:debug info;bit1:warning info;bit3:erro info*/
...
static unsigned init_cc_data_flag;

static void vbi_hw_reset(struct vbi_dev_s *devp)
{
	/*reset in cvd2 reginit when open port*/
...
** 0x40000 ~ 0x40fff: temp data(for data compare)--- VBI_BUFF2_EA
** 0x41000 ~ 0x42000: vbi data which will be integration
**                    to vbi_data_s              --- VBI_BUFF3_EA
** 0x42000 ~ 0x42fff: temp data(for data compare)--- VBI_BUFF4_EA
**------------------------------------------------------------
**
 */
	...
	unsigned int *len)
{
	...
	if (!table_start_addr || !search_point || !table_end_addr ||
		...
		pr_info("search_table point error\n");
		return NULL;
	}
	if ((table_end_addr <= table_start_addr) ||
		(search_point > table_end_addr)) {
		pr_info("search_table add err: start=%p,search=%p,end=%p\n",
...
static void set_vbi_new_vcnt(struct vbi_dev_s *devp, unsigned char *rptr)
{
	...
	temp = vcnt;
	if (vbi_dbg_en & 0x2)
		pr_info("[vbi..] %s:vbi write addr:0x%x\n",
			__func__, R_VBI_APB_REG(ACD_REG_44));

	if ((rptr < devp->pac_addr_start) ||
		(rptr > devp->pac_addr_end)) {
		pr_info("[vbi..] %s:addr error, vbi cannot search vcnt\n",
			__func__);
		return;
	}
	...
	ret_addr = search_table(devp->pac_addr_start, rptr,
		...
		3, &len);
	if (!ret_addr) {
		/*no vbidata head,get vcnt from addr read from 0x143*/
		l_val = *rptr & 0xff;
		h_val = *(rptr+1) & 0xff;
		if (vbi_dbg_en & 0x2)
			pr_info("[vbi..] %s:no vbidata head,force addr vcnt\n",
				__func__);
	} else {
		/*find vbidata head,get vcnt*/
		if (ret_addr - devp->pac_addr_start >= 18) {
			...
			l_val = devp->pac_addr_start[devp->mem_size-18+tval] &
				0xff;
			h_val = devp->pac_addr_start[devp->mem_size-17+tval] &
				0xff;
		}
	}
	vcnt = (h_val << 8) | l_val;
	if (vbi_dbg_en & 0x2)
		pr_info("[vbi..] %s:pre vcnt=0x%x, current vcnt=0x%x\n",
			__func__, temp, vcnt);
	if (vcnt > 0xffff)
		vcnt = temp;
}
...
static unsigned char *search_vbi_new_addr(struct vbi_dev_s *devp)
{
	...
	if (vbi_dbg_en & 0x2)
		pr_info("[vbi..] vbi search new addr:0x%x\n", (paddr >> 4));
	return vaddr;
}
...
static int check_if_sync_cc_data(struct vbi_dev_s *devp, unsigned char *addr)
{
	...
	if (!addr) {
		...
		} else if (vbi_dbg_en & 0x2)
			pr_info("[vbi..] not find vbi data.\n");
		return -1;
	} else if (addr > devp->pac_addr_end) {
		pr_info("[vbi..] vbi ret_addr error.\n");
		return -1;
	}
...
static void vbi_slicer_task(unsigned long arg)
{
	...
	while (chlen > 0) {
		...
		vbi_addr = search_table(local_rptr, local_rptr,
			...
			vbi_head, 3 , &vbihlen);
		if (!vbi_addr) {
			/*pr_info("not find vbi buff data\n");*/
			goto err_exit;
		}
		...
		if (data_print_en) {
			pr_info("[vbi..]: field_id:%d, line_num:%4d;",
				vbi_data.field_id,
				vbi_data.line_num);
		  ...
		  struct vbi_slicer_s *vbi_slicer)
{
	...
	memset(vbi_slicer->buffer.data, 0, VBI_DATA_SIZE);
...
{
	rbuf->pread = rbuf->pwrite = 0;
	memset(rbuf->data, 0, VBI_DATA_SIZE);
		      ...
		      size_t count, loff_t *ppos)
{
	...
	if (tvafe_clk_status == false) {
		...
		vbi_ringbuffer_flush(src);
		if (vbi_dbg_en & 0x2)
			pr_info("[vbi..] %s: tvafe is closed.pread = %d;pwrite = %d\n",
...
static int vbi_release(struct inode *inode, struct file *file)
{
	...
	if (tvafe_clk_status) {
		...
		W_VBI_APB_REG(CVD2_VBI_FRAME_CODE_CTL, 0x00000010);
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case VBI_IOC_SET_TYPE:
		if (!tvafe_clk_status) {
			mutex_unlock(&vbi_dev->mutex);
			pr_info("[vbi..] %s: afe not open,SET_TYPE return\n",
				__func__);
			return -ERESTARTSYS;
		}
		pr_info("[vbi..] %s: VBI_IOC_SET_TYPE\n", __func__);
		if (mutex_lock_interruptible(&vbi_slicer->mutex)) {
			mutex_unlock(&vbi_dev->mutex);
			pr_info("[vbi..] %s: slicer mutex error\n", __func__);
			return -ERESTARTSYS;
		...
		if (copy_from_user(&vbi_slicer->type, argp, sizeof(int))) {
			pr_info("[vbi..] %s: copy_from_user err\n", __func__);
			ret = -EFAULT;
			mutex_unlock(&vbi_slicer->mutex);
			break;
		}
		if (tvafe_clk_status) {
			...
			ret = -EFAULT;
			pr_err("[vbi..] %s: tvafeclose, no vbi_slicer_set\n",
				__func__);
		}
		mutex_unlock(&vbi_slicer->mutex);
...
static int vbi_probe(struct platform_device *pdev)
{
	...
	memset(vbi_dev->slicer->buffer.data, 0, VBI_DATA_SIZE);
