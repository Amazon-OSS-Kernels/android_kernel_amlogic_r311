...
void sec_top_write(unsigned *addr, unsigned value)
{
	/*register unsigned int r0 asm("r0") = 0x8200001e;
	register unsigned int r1 asm("r1") = addr;
	register unsigned int r2 asm("r2") = value;
	asm volatile(
		__asmeq("%0", "r0")
		__asmeq("%1", "r1")
		__asmeq("%2", "r2")
	...
	fn_smc(0x8200001e, (unsigned int)addr, value, 0);
}

unsigned sec_top_read(unsigned *addr)
{
	/*register unsigned int r0 asm("r0") = 0x8200001d;
	register unsigned int r1 asm("r1") = addr;
	asm volatile(
		__asmeq("%0", "r0")
		__asmeq("%1", "r1")
	...
	return (unsigned)(r0&0xffffffff); */
	return fn_smc(0x8200001e, (unsigned int)addr, 0, 0);
}

void rx_sec_reg_write(unsigned *addr, unsigned value)
{
	/* register unsigned int r0 asm("r0") = 0x8200002f;
	register unsigned int r1 asm("r1") = addr;
	register unsigned int r2 asm("r2") = value;
	asm volatile(
		__asmeq("%0", "r0")
	...
	fn_smc(0x8200002f, (unsigned int)addr, value, 0);
}

unsigned rx_sec_reg_read(unsigned *addr)
{
	/* register unsigned int r0 asm("r0") = 0x8200001f;
	register unsigned int r1 asm("r1") = addr;
	asm volatile(
		__asmeq("%0", "r0")
		__asmeq("%1", "r1")
	...
	return (unsigned)(r0&0xffffffff); */
	return fn_smc(0x8200001f, (unsigned int)addr, 0, 0);
}

unsigned rx_sec_set_duk(void)
{
	/* register unsigned int r0 asm("r0") = 0x8200002e;
	asm volatile(
		__asmeq("%0", "r0")
		__SMC(0)
		: "+r"(r0)
	...
	return (unsigned)(r0&0xffffffff); */
	return fn_smc(0x8200002e, 0, 0, 0) & 0xffffffff;
}
#endif
	...
	if (is_meson_gxtvbb_cpu()) {
		if (!val) {
			hdmirx_wr_top(TOP_HPD_PWR5V,
				hdmirx_rd_top(TOP_HPD_PWR5V)&(~(1<<port_id)));
		} else {
			hdmirx_wr_top(TOP_HPD_PWR5V,
				hdmirx_rd_top(TOP_HPD_PWR5V)|(1<<port_id));
		}
...
void control_reset(unsigned char en)
{
	unsigned long data32;
...
void hdmirx_phy_switch(void)
{
	last_clk_rate = 0;
	/* clear clkrate cfg */
	hdmirx_wr_bits_phy(PHY_CDR_CTRL_CNT, CLK_RATE_BIT, 0);
	/* switch port */
	hdmirx_wr_bits_dwc(DWC_SNPS_PHYG3_CTRL, MSK(2, 2), rx.port);
}
...
bool rx_uboot_inited(void)
{
	if (hdmirx_rd_top(TOP_EDID_RAM_OVR7_DATA))
		return true;
	else
		return false;
}

void hdmirx_port_switch(void)
{
	rx_pr("%s port:%d\n", __func__, rx.port);
	control_reset(0);
...
unsigned char rx_get_hdcp14_encrypted_sts(void)
{
	return (unsigned char)((hdmirx_rd_dwc(DWC_HDCP_STS) >> 9) & 1);
}
...
void rx_get_phy_init(unsigned int *init)
{
	*init = phy_init;
	if (phy_init)
		phy_init = 0;
}
EXPORT_SYMBOL(rx_get_phy_init);
