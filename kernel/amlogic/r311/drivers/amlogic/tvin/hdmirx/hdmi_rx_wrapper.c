...
int need_full_reset_flag = 1;
MODULE_PARM_DESC(need_full_reset_flag, "\n need_full_reset_flag\n");
module_param(need_full_reset_flag, int, 0664);
...
static int clk_unstable_max = 60000;
...
static int hdcp_none_wait_max;
MODULE_PARM_DESC(hdcp_none_wait_max, "\n hdcp_none_wait_max\n");
module_param(hdcp_none_wait_max, int, 0664);
...
int pll_unlock_check_times_max = 3;
bool hdmi5v_lost_flag;
bool need_support_atmos_bit;
unsigned int hpd_delay_count;
bool hpd_delay_en;
unsigned int hpd_low;
...
struct freq_ref_s freq_ref[] = {
	...
	/* basic format*/
	{0, 0, 0, 0, 640,  480,  0, 0, HDMI_640x480p60},
	...
	{0, 0, 0, 0, 1152, 864, 0, 0, HDMI_1152X864P75},
...
/* func cec_set_full_reset_flag
* workaround for some high-end AVRs,
* like Sony STR-DH540, will crash by rxsense change.
* So if these devices were connected, avoid the rxsense changes
* during the source switch.
*/
int cec_set_full_reset_flag(unsigned char val)
{
	/*
	if (need_full_reset_flag > 1)
		return 0;
	else
		need_full_reset_flag = val;
	*/
	return 0;
}
EXPORT_SYMBOL(cec_set_full_reset_flag);
...
static int hdmi_rx_ctrl_irq_handler(struct hdmi_rx_ctrl *ctx)
{
	...
	if (intr_aud_fifo != 0) {
		if (get(intr_aud_fifo, OVERFL) != 0) {
			if (log_level & 0x100)
				rx_pr("[irq] OVERFL\n");
#ifdef CONFIG_AM_HDMI_AUDIO_FIFO
...
enum tvin_sig_fmt_e hdmirx_hw_get_fmt(void)
{
	...
	case HDMI_1152X864P75:
		fmt = TVIN_SIG_FMT_HDMI_1152X864P_75HZ;
		break;
		/* 4k2k mode */
...
void rx_plugout_handle(unsigned char port_id)
{
	rx_pr("%s port_id:%d\n", __func__, port_id);
...
void rx_start_hpd_delay(void)
{
	hpd_delay_en = true;
	hpd_delay_count = 0;
}

void rx_stop_hpd_delay(void)
{
	hpd_delay_en = false;
	hpd_delay_count = 0;
}

void rx_plugin_handle(unsigned char port_id)
{
	rx_pr("%s\n", __func__);
	if (rx.port != port_id)
...
void rx_hpd_delay(void)
{
	if (hpd_delay_en) {
		if (rx.open_fg == true)
			rx_stop_hpd_delay();
		else {
			if (hpd_delay_count++ == HPD_DELAY_COUNT) {
				rx_pr("%s EXIT\n", __func__);
				rx_stop_hpd_delay();
				rx_set_cur_hpd(1);
			}
...
unsigned char rx_plug_handle(uint32_t cur_sts, uint32_t pre_sts)
{
	int i;

	for (i = 0; i < E_PORT_NUM; i++) {
		if (((cur_sts >> i) & 1) != ((pre_sts >> i) & 1)) {
			if (((cur_sts >> i) & 1) == 0)
				rx_plugout_handle(i);
			else
				rx_plugin_handle(i);
...
void rx_5v_det(void)
{
	...
	if (check_cnt > pow5v_max_cnt) {
		check_cnt = 0;
		if (rx.edid_load_flag)
			rx_plug_handle(tmp_5v, pwr_sts);
		pwr_sts = tmp_5v;
		rx_pr("hotplg-%x\n", pwr_sts);
...
void rx_get_hpd_info(unsigned int *hpd_info)
{
	*hpd_info = hpd_low;
	if (hpd_low)
		hpd_low = 0;
}
EXPORT_SYMBOL(rx_get_hpd_info);

void rx_get_open_port(unsigned int *hpd_info)
{
	*hpd_info = open_port;
	if (open_port)
		open_port = 0;
}
EXPORT_SYMBOL(rx_get_open_port);
...
int hdmi_rx_ctrl_edid_update(void)
{
	...
	u_int checksum[E_PORT_NUM] = {0, 0, 0, 0};
...
void hdmirx_hw_init(enum tvin_port_e port)
{
	...
	if ((pre_port != rx.port) ||
		...
		if ((rx.boot_flag) ||
			...
			hdmirx_port_switch();
		if (hpd_delay_en  == true) {
			rx_stop_hpd_delay();
			if (pre_port != rx.port)
				rx_set_port_hpd(pre_port, 1);
		}
		open_port |= (1 << rx.port);
		pre_port = rx.port;
		rx_set_eq_run_state(E_EQ_START);
...
void hdmirx_hw_uninit(void)
{
	...
	if (!get_cur_hpd_sts() && rx.cur_5v_sts)
		rx_start_hpd_delay();
}
