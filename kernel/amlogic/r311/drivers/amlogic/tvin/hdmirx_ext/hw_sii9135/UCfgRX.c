...
static void PrintOutputColorDepth(BYTE bOutColorDepth)
{
	...
	case SiI_RX_CD_24BPP:
		HDMIIN_PD("24 bit per pixel\n");
		break;
	case SiI_RX_CD_30BPP:
		HDMIIN_PD("30 bit per pixel\n");
		break;
	case SiI_RX_CD_36BPP:
		HDMIIN_PD("36 bit per pixel\n");
		break;
	}
}
/*------------------------------------------------------------------------------
...
{
	HDMIIN_PD("RX API intance intilized with:\n");
	HDMIIN_PD("PCM Master Clock output:\n");
	PrintMasterClock(pbInitData[0] & 0x03);
	HDMIIN_PD("DSD Master Clock output:\n");
	PrintMasterClock((pbInitData[0] >> 2) & 0x03);
	HDMIIN_PD("HBR Master Clock output:\n");
	PrintMasterClock((pbInitData[0] >> 4) & 0x03);
	HDMIIN_PD("FPGA support:\n");
	if (pbInitData[0] & SiI_RX_FPGA)
		HDMIIN_PD("yes\n");
	else
		HDMIIN_PD("no\n");
	HDMIIN_PD("Video Output clock:\n");
	if (pbInitData[0] & SiI_RX_InvertOutputVidClock)
		HDMIIN_PD("yes\n");
	else
		HDMIIN_PD("no\n");
	HDMIIN_PD("Output Color Depth:\n");
...
BYTE siiRXConfig(void)
{
	...
	if (!bError) {
		...
		bError = TRUE;
		HDMIIN_PE("No Ack from configuration EEPROM\n");
		HDMIIN_PE("use API to init RX\n");
	}
#endif
	return bError;
