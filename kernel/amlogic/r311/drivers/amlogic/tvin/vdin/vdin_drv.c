...
static unsigned int tvin_sig_format;
module_param(tvin_sig_format, uint, 0664);
MODULE_PARM_DESC(tvin_sig_format, "tvin signal format");
...
int vdin_open_fe(enum tvin_port_e port, int index,  struct vdin_dev_s *devp)
{
	...
	if ((port == TVIN_PORT_CVBS3) &&
		...
		devp->parm.info.fmt = TVIN_SIG_FMT_NULL;
	tvin_sig_format = devp->parm.info.fmt;
...
void vdin_close_fe(struct vdin_dev_s *devp)
{
	...
	devp->parm.info.fmt  = TVIN_SIG_FMT_NULL;
	devp->parm.info.status = TVIN_SIG_STATUS_NULL;
	tvin_sig_format = TVIN_SIG_FMT_NULL;
	pr_info("%s ok\n", __func__);
}
		...
		struct vframe_s *vf)
{
	...
	if ((devp->parm.port >= TVIN_PORT_HDMI0) &&
		...
		vf->pic_mode.hs = devp->prop.bar_end_left;
		vf->pic_mode.he = devp->prop.bar_start_right;
		vf->pic_mode.vs = devp->prop.bar_end_top;
		vf->pic_mode.ve = devp->prop.bar_start_bottom;
	}
	...
	if (devp->prop.aspect_ratio != TVIN_ASPECT_NULL)
...
irqreturn_t vdin_isr(int irq, void *dev_id)
{
	...
	unsigned long vlock_delay_jiffies;
	...
	if (devp->duration) {
		vlock_delay_jiffies = 96000/devp->duration;
		vlock_delay_jiffies = HZ/vlock_delay_jiffies;
		vlock_delay_jiffies = vlock_delay_jiffies/4;
	} else
