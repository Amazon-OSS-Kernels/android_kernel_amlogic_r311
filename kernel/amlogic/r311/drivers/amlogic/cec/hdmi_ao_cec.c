...
static int hpd_force_add;
static int poll_en = 1;
...
static struct cec_device_info_s port_info_list[5];
...
unsigned char cec_phy_addr_to_port(unsigned int phy_addr)
{
	int i;
	unsigned char phy_value = 0xFF;

	/*get the UI port*/
	phy_value = (unsigned char)((phy_addr >> 12) & 0xF);
	if (phy_value == 0)
		return 0xFF;
	/*find the port id*/
	for (i = 0; i < 4; i++) {
			if (((cec_dev->port_seq >> i*4) & 0xF) == phy_value)
				break;
	}
	if (i != 4)
		return i;
...
unsigned char cec_logic_addr_to_port(unsigned char logic_addr)
{
	int i;

	for (i = 0; i < 4; i++) {
		if (port_info_list[i].logic_addr == logic_addr)
			break;
	}
	return i;
}
...
void cec_set_poll_enable(int port_id, bool enable)/* phy port */
{
	if (port_id < 4) {
		if (enable) {
			if (!port_info_list[port_id].plug) {
				port_info_list[port_id].skip = true;
				port_info_list[port_id].view = 0;
				protect_count = 1;
				port_info_list[port_id].hpd_timeout =
						 CEC_HPD_VIEW_DELAY;
			}
		} else {
			port_info_list[port_id].skip = false;
			port_info_list[port_id].view = 0;
			port_info_list[port_id].hpd = 0;
			port_info_list[port_id].hpd_count = 0;
			port_info_list[port_id].hpd_timeout =
						 CEC_HPD_VIEW_DELAY;
		}
		port_info_list[port_id].bypass_count = 0;
	}
	CEC_INFO("%s port:%d skip:%d count:%d\n",  __func__, port_id,
		port_info_list[port_id].skip,
		port_info_list[port_id].bypass_count);
}

void cec_set_plug(unsigned char port_id, bool enable)/* phy port */
{
	if (port_id < 4) {
		port_info_list[port_id].plug = enable;
		port_info_list[port_id].plug_count = 0;
		if (enable == true)
			cec_set_poll_enable(port_id, false);
		CEC_INFO("%s port:%d plug:%d plug_count:%d\n",
			__func__, port_id, port_info_list[port_id].plug,
			port_info_list[port_id].plug_count);
	}
}

void cec_request_enable(bool enable)
{
	request_active = enable;
	request_count = 0;
}

void cec_check_plug(unsigned int pre_value, unsigned int cur_value)
{
	int i;
	bool rxsen = 0;
	for (i = 0; i < 4; i++) {
		if (((cur_value >> i) & 1)  > ((pre_value >> i) & 1))
			cec_set_plug(i, true);
		else if (((cur_value >> i) & 1)  < ((pre_value >> i) & 1)) {
			port_info_list[i].logic_addr = 0;
			port_info_list[i].plug = 0;
			port_info_list[i].phy_addr = 0;
			if (port_info_list[i].rxsen == 1) {
				port_info_list[i].rxsen = 0;
				CEC_INFO("RXSENSE DEV OUT\n");
			}
		}
		if (port_info_list[i].rxsen == 1)
			rxsen = 1;
	...
	if (rxsen == 0)
		cec_set_full_reset_flag(1);
}

void cec_poll_info(void)
{
	int i;
	for (i = 0; i < 4 ; i++) {
		if (port_info_list[i].plug) {
			if (port_info_list[i].plug_count++ >= CEC_PLUG_DELAY)
				cec_set_plug(i, false);
		}
		if (port_info_list[i].skip) {
			if (port_info_list[i].bypass_count++ >=
						port_info_list[i].hpd_timeout)
				cec_set_poll_enable(i, false);
		}
		if (port_info_list[i].hpd)
			port_info_list[i].hpd_count++;
	}
	if (protect_count) {
		if (protect_count++ == CEC_PROTECT_DELAY) {
			cec_request_enable(false);
			protect_count = 0;
			CEC_INFO("%s protect end\n", __func__);
		}
		if (request_active == true)
			if (request_count++ == CEC_REQUEST_DELAY)
				cec_request_enable(false);
	} else
		cec_request_enable(false);
}

bool cec_get_poll_info(unsigned int phy_addr)/* UI port */
{
	int ret = 0;
	unsigned char port_id = cec_phy_addr_to_port(phy_addr);

	CEC_INFO("%s protect_count:%d\n", __func__, protect_count);
	if (protect_count == 0)
		return false;
	if (port_id < 4) {
		ret = port_info_list[port_id].skip;
		CEC_INFO("%s port:%d skip:%d count:%d request_active:%d\n",
			__func__, port_id, port_info_list[port_id].skip,
			port_info_list[port_id].bypass_count, request_active);
	} else
		ret = false;
...
void cec_add_device(unsigned char logic_addr, unsigned int phy_addr)
{
	unsigned char port_id = cec_phy_addr_to_port(phy_addr);
	int i;

	if (port_id < 4) {
		CEC_INFO("%s add logic addr:%d,port_id:%d\n", __func__,
			logic_addr, port_id);
		...
		for (i = 0; i < 4; i++) {
			...
			CEC_INFO("logic addr:%#x,port id:%d\n",
					port_info_list[i].logic_addr, i);
		}
		port_info_list[port_id].logic_addr = logic_addr;
	}
}

void cec_view_on_add(unsigned char logic_addr)
{
	int i;
	for (i = 0; i < 4; i++) {
		if (port_info_list[i].logic_addr == logic_addr)
	...
	if ((i != 4) && port_info_list[i].skip) {
		CEC_INFO("%s add logic addr:%d,port_id:%d,active:%d,hpd:%d\n",
			__func__, logic_addr, i, port_info_list[i].view,
			port_info_list[i].hpd);
		if (hpd_force_add) {
			if (port_info_list[i].view == port_info_list[i].hpd)
				cec_set_poll_enable(i, false);
			else {
				port_info_list[i].view += 1;
				port_info_list[i].hpd_timeout =
							CEC_HPD_ACTIVE_DELAY;
			}
		} else {
			if (port_info_list[i].hpd >= 2) {
				if (port_info_list[i].view == 2)
					cec_set_poll_enable(i, false);
				else {
					port_info_list[i].view += 1;
					port_info_list[i].hpd_timeout =
				...
				port_info_list[i].hpd = 0;
			}
		}
	}
}

void cec_hpd_add(unsigned char port_id)
{
	CEC_INFO("%s add port_id:%d,count:%d,hpd:%d,skip:%d\n",
		__func__, port_id, port_info_list[port_id].hpd_count,
		port_info_list[port_id].hpd, port_info_list[port_id].skip);

	if (port_id < 4) {
		if (port_info_list[port_id].skip) {
			if ((port_info_list[port_id].hpd_count < CEC_HPD_MAX)
				|| hpd_force_add)
				port_info_list[port_id].hpd++;
			else
				port_info_list[port_id].hpd = 1;
		}
		port_info_list[port_id].hpd_count = 0;
	}
}
EXPORT_SYMBOL(cec_hpd_add);
...
{
	unsigned char port_addr = cec_logic_addr_to_port(msg[0]>>4);
	...
	if (msg[1] == 0x82) {
		if (len >= 4) {
			CEC_INFO("++++++++++msg82:%#x,%#x,%#x,%#x,skip:%d\n",
				msg[0], msg[1], msg[2], msg[3],
				cec_get_poll_info((msg[2]<<8) + msg[3]));
			cec_add_device(msg[0] >> 4, (msg[2]<<8) + msg[3]);
			if (cec_get_poll_info((msg[2]<<8) + msg[3])) {
				CEC_INFO("--no skip active source---\n");
			}
		}
	} else if (msg[1] == 0x84) {
		if (len >= 4) {
			CEC_INFO("-----------msg84:%#x,%#x,%#x,%#x\n",
				msg[0], msg[1], msg[2], msg[3]);
			cec_add_device(msg[0] >> 4, (msg[2] << 8) + msg[3]);
		}
	} else if ((msg[1] == 0x0d) || (msg[1] == 0x04)) {
		if (len >= 2) {
			CEC_INFO("-----------msg0d:%#x,%#x\n", msg[0], msg[1]);
			cec_view_on_add(msg[0] >> 4);
		}
	} else if (msg[1] == 0x85) {
		if (len >= 2) {
			CEC_INFO("-----------msg85:%#x,%#x\n", msg[0], msg[1]);
			cec_request_enable(true);
		}
	} else if (msg[1] == 0x87) {
		CEC_INFO("-----------msg87:%#x,%#x,%#x,%#x,%#x\n",
			msg[0], msg[1], msg[2], msg[3], msg[4]);
		if ((msg[2] == 0x00) &&	(msg[3] == 0x07) &&
			(msg[4] == 0xaa))
			cec_set_plug(port_addr, true);
		if (((msg[0]>>4) == 5) && (((msg[2] == 0x41) && (msg[3] == 0x56)
			&& (msg[4] == 0x20)) || ((msg[2] == 0x08) &&
			(msg[3] == 0x00) &&	(msg[4] == 0x46)))) {
			port_info_list[port_addr].rxsen = 1;
			cec_set_full_reset_flag(0);
			CEC_INFO("GET THE RXSENSE DEV ID\n");
		}
	} else if (msg[1] == 0x47) {
		if ((msg[2] == 0x54) && (msg[3] == 0x65) && (msg[4] == 0x73)
			&& (msg[5] == 0x74) && (msg[6] == 0x20) &&
			(msg[7] == 0x44) && (msg[8] == 0x65) &&
			 (msg[9] == 0x76))
			cec_set_plug(port_addr, true);
		if ((msg[2] == 0x41) && (msg[3] == 0x56) && (msg[4] == 0x20)
			&& (msg[5] == 0x41) && (msg[6] == 0x4d) &&
			(msg[7] == 0x50) && ((msg[0]>>4) == 5)) {
			port_info_list[port_addr].rxsen = 1;
			cec_set_full_reset_flag(0);
			CEC_INFO("GET THE RXSENSE DEV OSD\n");
		}
	}
	return true;
}
...
#if 0
	cec_set_reg_bits(AO_RTI_PWR_CNTL_REG0, 0, 6, 1);
#endif
...
int cec_ll_tx(const unsigned char *msg, unsigned char len)
{
	...
	if (!ee_cec && need_nack_repeat_msg(msg, len, t)) {
		if (!memcmp(msg, last_cec_msg->msg, len)) {
			CEC_INFO("NACK repeat message:%x %x\n", len, msg[0]);
...
void ao_cec_init(void)
{
	...
	if (chiptype >= MESON_CPU_MAJOR_ID_GXBB) {
		if (chiptype >= MESON_CPU_MAJOR_ID_TXLX) {
			reg = (0 << 31) |
				(0 << 30) |
				(1 << 28) | /* clk_div0/clk_div1 in turn */
				((732-1) << 12) |/* Div_tcnt1 */
...
{
	input_event(cec_dev->cec_info.remote_cec_dev, EV_KEY, KEY_POWER_CEC, 1);
	input_sync(cec_dev->cec_info.remote_cec_dev);
	input_event(cec_dev->cec_info.remote_cec_dev, EV_KEY, KEY_POWER_CEC, 0);
	input_sync(cec_dev->cec_info.remote_cec_dev);
	if (!suspend)
...
static void cec_rx_process(void)
{
	...
	int dest_phy_addr;
	unsigned int cec_cfg = cec_config(0, 0);
	...
	case CEC_OC_TEXT_VIEW_ON:
		...
		if (((cec_cfg >> CEC_FUNC_MASK) & 0x1) &&
			((cec_cfg >> AUTO_POWER_ON_MASK) & 0x1)) {
			CEC_INFO("weak up by otp\n");
			cec_key_report(0);
		} else {
			CEC_INFO("otp disabled\n");
		}
		break;
...
void cec_timer_handler(unsigned long arg)
{
	int i;
	unsigned int hpd_info;
	unsigned int phy_init;
	unsigned int open_port;

	rx_get_hpd_info(&hpd_info);
	rx_get_phy_init(&phy_init);
	rx_get_open_port(&open_port);
	for (i = 0; i < 4; i++) {
		if ((hpd_info >> i) & 1)
			cec_set_poll_enable(i, true);
		if (phy_init)
			cec_set_poll_enable(i, true);
		if ((open_port >> i) & 1)
			cec_hpd_add(i);
	}
	cec_check_plug(plug_info, rx_get_plug_info());
	plug_info = rx_get_plug_info();
	cec_poll_info();
	if (poll_en) {
		cec_dev->timer.expires = jiffies + CEC_TIMER_STATE_CHECK;
		add_timer(&cec_dev->timer);
	}
	/*CEC_INFO("time handler:\n");*/
}
	...
	struct class_attribute *attr, char *buf)
{
	...
	if ((cec_dev->tx_dev) && (cec_dev->dev_type == DEV_TYPE_PLAYBACK)) {
		tmp = tx_hpd;
		return sprintf(buf, "%x\n", tmp);
	} else {
...
static ssize_t hpd_add_store(struct class *cla, struct class_attribute *attr,
	const char *bu, size_t count)
{
	int cnt, val;
...
static ssize_t hpd_add_show(struct class *cla,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", hpd_force_add);
}

static ssize_t poll_en_store(struct class *cla, struct class_attribute *attr,
	const char *bu, size_t count)
{
	int cnt, val;
...
static ssize_t poll_en_show(struct class *cla,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", poll_en);
}

static ssize_t boot_addr_store(struct class *cla,
	struct class_attribute *attr, const char *bu, size_t count)
{
	return count;
}

static ssize_t boot_addr_show(struct class *cla,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n",
		readl(cec_dev->cec_reg + AO_RTI_STATUS_REG1));
}

static ssize_t boot_reason_store(struct class *cla,
	struct class_attribute *attr, const char *bu, size_t count)
{
	return count;
}

static ssize_t boot_reason_show(struct class *cla,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n",
		readl(cec_dev->cec_reg + AO_RTI_STATUS_REG0));
}
...
static struct class_attribute aocec_class_attr[] = {
	...
	__ATTR(hpd_add, 0664, hpd_add_show, hpd_add_store),
	__ATTR(poll_en, 0664, poll_en_show, poll_en_store),
	__ATTR(boot_addr, 0664, boot_addr_show, boot_addr_store),
	__ATTR(boot_reason, 0664, boot_reason_show, boot_reason_store),
...
void cec_dump_info(void)
{
	...
	CEC_ERR("wake_ok:0x%x\n", wake_ok);
	CEC_ERR("cec_suspend:0x%x\n", cec_dev->cec_suspend);
	CEC_ERR("new_msg:0x%x\n", new_msg);
			     ...
			     unsigned int cmd, unsigned long arg)
{
	...
	case CEC_IOC_GET_PHYSICAL_ADDR:
		...
		if (cec_dev->dev_type ==  DEV_TYPE_PLAYBACK && !phy_addr_test) {
			...
			if ((cec_dev->tx_dev == NULL)
			|| (cec_dev->tx_dev->
			hdmi_info.vsdb_phy_addr.valid == 0))
				return -EINVAL;
	...
	case CEC_IOC_GET_BOOT_ADDR:
		tmp = readl(cec_dev->cec_reg + AO_RTI_STATUS_REG1);
		CEC_INFO("Boot addr:%#x\n", (unsigned int)tmp);
		if (copy_to_user(argp, &tmp, _IOC_SIZE(cmd)))
			return -EINVAL;
		break;

	case CEC_IOC_GET_BOOT_REASON:
		tmp = readl(cec_dev->cec_reg + AO_RTI_STATUS_REG0);
		CEC_INFO("Boot reason:%#x\n", (unsigned int)tmp);
		if (copy_to_user(argp, &tmp, _IOC_SIZE(cmd)))
			return -EINVAL;
		break;
...
static int aml_cec_probe(struct platform_device *pdev)
{
	...
	if (cec_dev->cec_info.remote_cec_dev == NULL) {
		CEC_ERR("No enough memory\n");
	} else {
		...
		/* Support Amazon's customized CEC power keys. */
		input_set_capability(cec_dev->cec_info.remote_cec_dev,
				EV_KEY, KEY_POWER_CEC);
	}
	...
	if (of_property_read_u32(node, "port_num",
			&(cec_dev->port_num))) {
		CEC_ERR("not find 'port_num'\n");
		cec_dev->port_num = 1;
	}

	if (of_property_read_u32(node, "arc_port_mask",
			&(cec_dev->arc_port))) {
	...
	if (of_property_read_u32(node, "vendor_id",
			&(vend->vendor_id)))
		CEC_ERR("not find vendor id\n");
	...
	if (of_property_read_u32(node, "cec_version",
			&(cec_dev->cec_info.cec_version))) {
		/* default set to 2.0 */
		CEC_ERR("not find cec_version\n");
	...
	if (of_get_property(node, "interrupt-names", NULL)) {
		...
		if (!r && !ee_cec) {
			...
			if (r < 0)
				CEC_ERR("aocec irq request fail\n");
		}
		if (!r && ee_cec) {
			r = request_irq(irq_idx, &cecrx_isr, IRQF_SHARED,
					irq_name, (void *)cec_dev);
			if (r < 0)
				CEC_ERR("cecb irq request fail\n");
		}
	}
#endif
	...
	plug_info = rx_get_plug_info();
	if (poll_en) {
		init_timer(&cec_dev->timer);
		cec_dev->timer.data = (ulong)&port_info_list;
		cec_dev->timer.function = cec_timer_handler;
		cec_dev->timer.expires = jiffies + CEC_TIMER_STATE_CHECK;
		add_timer(&cec_dev->timer);
	}
	/* for init */
...
static int aml_cec_resume_noirq(struct device *dev)
{
	...
	CEC_ERR("pre GEN CNTL 0x%x",
		readl(cec_dev->cec_reg + AO_CEC_GEN_CNTL));
	cec_hw_reset();
	CEC_ERR("after GEN CNTL 0x%x",
		readl(cec_dev->cec_reg + AO_CEC_GEN_CNTL));

	/* reselect pin mux if resume */
	pin = devm_pinctrl_get(cec_dev->dbg_dev);
	if (IS_ERR(pin))
		return -EINVAL;

	pinstate = pinctrl_lookup_state(pin, "dummy");
	r = pinctrl_select_state(pin, pinstate);
	if (r < 0)
		devm_pinctrl_put(pin);

	if (of_get_property(cec_dev->node, "pinctrl-names", NULL)) {
		if (ee_cec)
			r = of_property_read_string_index(cec_dev->node,
							"pinctrl-names", 0,
							&cec_dev->pin_name);
		else
			r = of_property_read_string_index(cec_dev->node,
							"pinctrl-names", 1,
							&cec_dev->pin_name);
		CEC_INFO("pin mux name: %s\n", cec_dev->pin_name);
		if (!r) {
			pinstate = pinctrl_lookup_state(pin, cec_dev->pin_name);
			r = pinctrl_select_state(pin, pinstate);
			if (r < 0)
				CEC_ERR("pinctrl_select_state %d\n", r);
		}
