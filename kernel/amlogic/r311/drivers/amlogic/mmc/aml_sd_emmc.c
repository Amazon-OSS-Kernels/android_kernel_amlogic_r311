...
int aml_sd_emmc_suspend(struct device *dev)
{
	int ret = 0;
	u32 *reg = NULL;
	u32 value;
	...
	sd_emmc_regs = (struct sd_emmc_regs_v3 *)host->sd_emmc_regs;

	pr_err("***Entered %s\n", __func__);
	pr_err("clock 0x%x adjust 0x%x cfg 0x%x\n", sd_emmc_regs->gclock,
		sd_emmc_regs->gadjust, sd_emmc_regs->gcfg);
	/* close source gating */
	value = readl(host->clksrc_base);
	writel((value & ~(1 << 7)), host->clksrc_base);
	pr_err("source Gating 0x%x\n", readl(host->clksrc_base));
	/* close clk81 gating */
	reg = ioremap(0xFF63C140, 0x10);
	if (!reg)
		goto _out;
	value = readl(reg);
	writel(value&(~(1 << 26)), reg);
	pr_err("clk81 Gating 0x%x\n", readl(reg));
	iounmap(reg);
...
int aml_sd_emmc_resume(struct device *dev)
{
	int ret = 0;
	struct platform_device *pdev;
	struct amlsd_host *host;
