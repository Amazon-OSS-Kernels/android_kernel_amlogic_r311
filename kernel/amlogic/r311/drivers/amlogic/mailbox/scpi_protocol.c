...
int scpi_set_led_pattern(u32 pattern)
{
	struct scpi_data_buf sdata;
	struct mhu_data_buf mdata;
	int state;

	SCPI_SETUP_DBUF(sdata, mdata, SCPI_CL_LED_PATTERN,
			SCPI_CMD_SET_LED_PATTERN, pattern, state);
	if (scpi_execute_cmd(&sdata))
		return -EPERM;

	return 0;
}
EXPORT_SYMBOL_GPL(scpi_set_led_pattern);

#ifdef CONFIG_MESON_REMOTE_LEARN
int scpi_get_ir_data(u32 stop, u8 *val, u32 len, u32 *size)
{
	struct scpi_data_buf sdata;
	struct mhu_data_buf mdata;
	struct __packed {
	...
	SCPI_SETUP_DBUF(sdata, mdata, 0,
			SCPI_CMD_GET_IR_DATA, txbuf, rxbuf);
	ret = scpi_execute_cmd(&sdata);
	if (IS_ERR_VALUE(ret))
		return ret;
	*size = rxbuf.size;
...
EXPORT_SYMBOL_GPL(scpi_get_ir_data);

int scpi_set_ir_data(u32 ir_id, uint32_t data_size)
{
	struct scpi_data_buf sdata;
	struct mhu_data_buf mdata;
	struct __packed {
	...
	SCPI_SETUP_DBUF(sdata, mdata, ir_id,
			SCPI_CMD_SET_IR_DATA, buf, state);
	return scpi_execute_cmd(&sdata);
}
EXPORT_SYMBOL_GPL(scpi_set_ir_data);
#endif

#ifdef CONFIG_WOL_POWER
int scpi_set_wol_power(u32 enable)
{
	return scpi_send_usr_data(SCPI_CL_WOL_POWER, &enable, sizeof(u32));
}
EXPORT_SYMBOL_GPL(scpi_set_wol_power);
