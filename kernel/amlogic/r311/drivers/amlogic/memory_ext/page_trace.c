...
unsigned int cma_alloc_trace = 1;
...
static struct fun_symbol common_func[] __initdata = {
	...
	{"codec_mm_alloc_in",		1},
	{"codec_mm_alloc",		1},
	{"codec_mm_alloc_for_dma",	1},
	{"codec_mm_release",		1},
	{"codec_mm_free_for_dma",	1},
	{"codec_mm_free_in",		1},
	{"codec_mm_release_with_check",	1},
	{"cma_release",			1},
	{"free_alloced_keep_buffer",	1},
#ifdef CONFIG_ARM64
	{"__dma_free_coherent",		1},
#endif
...
static void __init set_init_page_trace(struct page *page, int order, gfp_t flag)
{
	...
	if (page && trace_buffer) {
		...
		trace.migrate_type = allocflags_to_migratetype(flag);
		trace.order = order;
