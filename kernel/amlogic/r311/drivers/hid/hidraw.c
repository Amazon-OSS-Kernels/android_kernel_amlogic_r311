		...
		case HIDIOCGRAWINFO:
			{
				...
				/* Temporarily for Huitong BLE remote. */
				if (!strcmp(dev->hid->name, "SR2_Demo")) {
					dinfo.vendor = 6421;
					dinfo.product = 1;
					pr_debug("Modify vendor id and product id for SR2_Demo\n");
				}
...
#if 0
	/* Temporarily for Huitong BLE remote. */
	if (strcmp(hid->name, "SR2_Demo")) {
		pr_debug("Only SR2_Demo can create hidraw device now\n");
		return -EINVAL;
	}
#endif
