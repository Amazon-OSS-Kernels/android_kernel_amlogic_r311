...
#include <linux/version.h>
	...
	IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
...
struct ftv_remote_device {
	...
	unsigned int voice_data_received;
#endif
};
...
static int bleremote_audio_dev_close(struct inode *inode, struct file *file)
{
	...
	IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
	{
		...
		pressed_time = (int)ktime_to_ms(ktime_sub(
			bleremote_dev.voice_key_up_time,
			bleremote_dev.voice_key_down_time));
		record_time = (int)ktime_to_ms(ktime_sub(
			bleremote_dev.voice_stop_time,
			bleremote_dev.voice_start_time));
		start_delay = (int)ktime_to_ms(ktime_sub(
			bleremote_dev.voice_start_time,
			bleremote_dev.voice_key_down_time));
		stop_delay = (int)ktime_to_ms(ktime_sub(
			bleremote_dev.voice_stop_time,
			bleremote_dev.voice_key_up_time));
#if IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
...
static const __u32 amazon_ble_game_controllers[] =
{
...
static int is_amazon_ble_game_controller(struct hid_device *hdev) {
	...
	if (hdev->vendor == BT_VENDOR_ID_LAB126) {
		...
		int i = 0;
		int list_size = sizeof(amazon_ble_game_controllers) / sizeof(__u32);
		for (i = 0; i < list_size; i++) {
			if (product == amazon_ble_game_controllers[i]) {
				found = 1;
				break;
			}
		}
	...
	u8 *data, int size)
{
	...
	pr_err("%s: report->id = 0x%x, size = %d\n",
	...
	IS_ENABLED(CONFIG_AMAZON_METRICS_LOG)
				...
				if (!bleremote_dev.audio_state_started) {
					char dimensions[METRICS_META_DATA_MAX_LEN];
					int pressed_time = (int)ktime_to_ms(ktime_sub(
						bleremote_dev.voice_key_up_time,
						bleremote_dev.voice_key_down_time));
#if IS_ENABLED(CONFIG_AMAZON_MINERVA_METRICS_LOG)
					snprintf(dimensions, METRICS_META_DATA_MAX_LEN,
	...
	case LAB126_REPORT_ID:
		...
		case VOICE_VIEW_KEY:
			input_report_key(remote_drvdata->input,
					 VOICE_VIEW_APP, 1);
			input_sync(remote_drvdata->input);
			input_report_key(remote_drvdata->input,
					 VOICE_VIEW_APP, 0);
...
static int ftv_remote_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	 ...
	 * have to filter by name.
	 */
	pr_err("%s: hdev->name = %s, vendor_id = %d, product_id = %d\n",
		__func__, hdev->name, hdev->vendor, hdev->product);

	remote_drvdata = kmalloc(sizeof(*remote_drvdata),
...
static const struct hid_device_id ftv_remote_devices[] = {
	...
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_EARHART)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_EARHART)},
	...
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn123)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn123)},
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn1232)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn1232)},
	...
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn123_abc123)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abn123_abc123)},
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abl123_abc123)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_abl123_abc123)},
