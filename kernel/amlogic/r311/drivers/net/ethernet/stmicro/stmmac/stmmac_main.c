...
static struct moniter_tx_t stmmac_mon;
...
static void moniter_tx_check(struct stmmac_priv *priv)
{
	struct moniter_tx_t *mon = &stmmac_mon;

	if (mon->check_tx_cnt == 5 &&
		(mon->last_dirty_tx == priv->dirty_tx))
		pr_err("tx stop triggered\n");
}
