...
static irqreturn_t audio_isr_handler(int irq, void *data)
{
	...
	return elapsed ? IRQ_WAKE_THREAD : IRQ_HANDLED;
}

static irqreturn_t audio_threaded_isr_handler(int irq, void *data)
{
	struct aml_runtime_data *prtd = data;
	struct snd_pcm_substream *substream = prtd->substream;
...
static int snd_request_hw_timer(void *data)
{
	...
	ret = request_threaded_irq(INT_TIMER_D, audio_isr_handler,
			audio_threaded_isr_handler, IRQF_SHARED,
			"timerd_irq", data);
		if (ret < 0) {
