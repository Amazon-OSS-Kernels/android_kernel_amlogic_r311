...
void aml_spdif_play(int samesrc)
{
	if (!is_meson_tv_chipset()) {
		...
		if (IEC958_mode_codec == TYPE_TRUE_HD
			|| IEC958_mode_codec == TYPE_DTS_HD) {
...
void aml_hw_iec958_init(struct snd_pcm_substream *substream, int samesrc)
{
	...
	if (IEC958_mode_codec > TYPE_PCM
		&& IEC958_mode_codec != TYPE_PCM_HIGH_SR) {
		if (IEC958_mode_codec == TYPE_RAW_DTS) {
	...
	if (iec958_mode == AIU_958_MODE_PCM16
		...
		if (IEC958_mode_codec == TYPE_EAC3) {
			/* DD+ */
			if (runtime->rate == 176400) {
				set.chan_stat->chstat1_l = 0xc00;
				set.chan_stat->chstat1_r = 0xc00;
	...
	if (IEC958_mode_codec == TYPE_AC3) {
		aout_notifier_call_chain(AOUT_EVENT_RAWDATA_AC_3, substream);
	} else if (IEC958_mode_codec == TYPE_DTS) {
		aout_notifier_call_chain(AOUT_EVENT_RAWDATA_DTS, substream);
	} else if (IEC958_mode_codec == TYPE_EAC3) {
		aout_notifier_call_chain(AOUT_EVENT_RAWDATA_DOBLY_DIGITAL_PLUS,
					 substream);
	} else if (IEC958_mode_codec == TYPE_TRUE_HD
		|| IEC958_mode_codec == TYPE_DTS_HD) {
		/*
		 FIXME.we need configure 192K DTS-HD/TrueHD channel status para.
		 need add other sample rates.
		 */
		...
		if (IEC958_mode_codec == TYPE_DTS_HD)
				   ...
				   struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (IEC958_mode_codec == TYPE_MULTI_PCM)
			pr_info("8ch PCM output:disable aml_spdif_play\n");
	}
