	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	/*bit2: 0 means PCM, 1 means others*/
	int valid_bit = aml_audin_read(AUDIN_SPDIF_CHNL_STS_A)&2;
	pc = pc&0xfff;
	...
	/*if spdif audio type is PAUSE and Format is PCM, clean the PAUSE*/
	if (audio_type == 6 && valid_bit == 0)
		audio_type = 0;
...
static int aml_audio_hal_format_get_enum(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
...
static int aml_audio_hal_format_set_enum(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	...
	p_aml_audio = snd_soc_card_get_drvdata(card);
	if (p_aml_audio->hal_fmt != ucontrol->value.integer.value[0]) {
		switch_set_state(&p_aml_audio->hal_fmt_sdev,
			p_aml_audio->hal_fmt);
	p_aml_audio->hal_fmt = ucontrol->value.integer.value[0];
	pr_info("%s()\n", __func__);
	}
	return 0;
}
...
static int db_on_det_adc_value(struct aml_audio_private_data *p_aml_audio)
{
	int ret, db_on_value;
	int db_on_val_sum = 0;
	int loop_num = 0;
	...
	while (loop_num < 8) {
		db_on_value = gpiod_get_value(p_aml_audio->db_on_det_desc);
		if (db_on_value < 0) {
			pr_info("db_on detect get error adc value!\n");
			return -1;	/* continue; */
		}
		db_on_val_sum += db_on_value;
		loop_num++;
		msleep_interruptible(15);
	}
	db_on_val_sum = db_on_val_sum >> 3;
...
static int aml_db_on_detect(struct aml_audio_private_data *p_aml_audio)
{
	int loop_num = 0;
	int ret;

	while (loop_num < 3) {
		ret = db_on_det_adc_value(p_aml_audio);
		if (p_aml_audio->db_on_last_status != ret) {
			msleep_interruptible(50);
			if (ret < 0)
				ret = p_aml_audio->db_on_last_status;
			else
				p_aml_audio->db_on_last_status = ret;
		} else
			msleep_interruptible(50);
...
#endif
static int audio_hp_db_on_det_adc_value(struct aml_audio_private_data *p_aml_audio, int *hp_flag, int *db_on_flag)
{
	int ret = 0, db_on_value;
	int db_on_val_sum = 0;
	int loop_num = 0;
	int hp_value;
	int hp_val_sum = 0;
	...
	while (loop_num < 8) {
		db_on_value = gpiod_get_value(p_aml_audio->db_on_det_desc);
		hp_value = gpiod_get_value(p_aml_audio->hp_det_desc);
		if (db_on_value < 0 || hp_value < 0) {
			pr_info(
				"hp or db_on detect get error adc value!db_on[%d]hp_value[%d]\n",
				db_on_value, hp_value);
			return -1;	/* continue; */
		}
		db_on_val_sum += db_on_value;
		hp_val_sum += hp_value;
...
static int aml_audio_hp_db_on_detect(struct aml_audio_private_data *p_aml_audio, int *hp_flag, int *db_on_flag)
{
	int loop_num = 0;
	int ret = 0;

	while (loop_num < 3) {
		ret = audio_hp_db_on_det_adc_value(p_aml_audio, hp_flag, db_on_flag);
		if (p_aml_audio->db_on_last_status != *db_on_flag
			|| p_aml_audio->hp_last_state != *hp_flag) {
			msleep_interruptible(50);
			if (*db_on_flag < 0)
				*db_on_flag = p_aml_audio->db_on_last_status;
			else
				p_aml_audio->db_on_last_status = *db_on_flag;

			if (*hp_flag < 0)
				*hp_flag = p_aml_audio->hp_last_state;
			else
				p_aml_audio->hp_last_state = *hp_flag;
...
static void aml_asoc_work_func(struct work_struct *work)
{
	...
	db_on_flag = aml_db_on_detect(p_aml_audio);
#endif
	/*get hp and db_on stat at same time*/
	aml_audio_hp_db_on_detect(p_aml_audio, &flag, &db_on_flag);
	if (p_aml_audio->detect_flag != flag || ((flag & 0x1) &&
			(p_aml_audio->db_on_det_flag != db_on_flag))) {
		...
		if (flag & 0x1) {
			...
			pr_info("aml UART tool plug status = %d\n", db_on_flag);

#ifdef ANDROID_HP_SWITCH
			if (!db_on_flag)
				switch_set_state(&p_aml_audio->sdev,
						STATE_HEADPHONE);
			else
				switch_set_state(&p_aml_audio->sdev,
						STATE_NONE);
			...
			audio_hp_status = 0;
#ifdef ANDROID_HP_SWITCH
			switch_set_state(&p_aml_audio->sdev, STATE_NONE);
#else
...
static int aml_asoc_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	/* for report audio format to android */
	p_aml_audio->hal_fmt_sdev.name = "audio_hal_fmt";
	ret = switch_dev_register(&p_aml_audio->hal_fmt_sdev);
	if (ret < 0) {
		pr_err("ASoC: register audio hal fmt switch dev failed\n");
		return ret;
	}
	...
	if (p_aml_audio->av_hs_switch) {
		...
		/*UART detect gpio*/
		p_aml_audio->db_on_det_desc = gpiod_get(card->dev, "db_on_det");
		if (!IS_ERR(p_aml_audio->db_on_det_desc))
			gpiod_direction_input(p_aml_audio->db_on_det_desc);
		else
			pr_err("ASoC: db_on_det-gpio failed\n");
...
{
	struct gpio_desc *pdn_desc;

	pdn_desc = of_get_named_gpiod_flags(p_node, "pdn_pin", 0, NULL);
	if (IS_ERR(pdn_desc)) {
		pr_err("%s fail to get pdn pin from dts!\n", __func__);
	} else {
		int pdn_pin = desc_to_gpio(pdn_desc);
		gpio_request(pdn_pin, NULL);
...
static int of_get_mclk_pdata(struct tas57xx_platform_data *pdata,
				 struct device_node *p_node)
{
	struct clk *mclk = devm_clk_get(pdata->dev, "mclk");
	if (IS_ERR(mclk))
		dev_info(pdata->dev, "Can't retrieve mclk clock\n");
	else
		pdata->clk_mclk = mclk;
			      ...
			      struct device_node *p_node)
{
	...
	ret = of_get_mclk_pdata(pdata, p_node);
	if (ret)
		pr_info("codec mclk not found in dts\n");
...
#ifdef CONFIG_SND_SOC_AD82584F
static bool aml_aux_dev_check_ad82584f(struct snd_soc_card *card)
{
#define AD82584F_NAME "ad82584f"
	struct device_node *audio_codec_node = card->dev->of_node;
	...
	if (!strcmp(AD82584F_NAME, codec_name))
		return true;
	else
		return false;
}
...
static void aml_pinmux_work_func(struct work_struct *pinmux_work)
{
	...
	if (is_meson_txl_cpu()) {
		...
		aml_eqdrc_update_bits(AED_EQ_VOLUME, 3 << 30, 0 << 30);
		aml_eqdrc_write(AED_EQ_VOLUME_SLEW_CNT, 0x5);
	}
...
#ifdef CONFIG_SND_SOC_AD82584F
	if (aml_aux_dev_check_ad82584f(card))
...
#endif
#ifdef CONFIG_TVIN_HDMI
	p_aml_audio->atmos_edid_enable = 0;
#endif
	p_aml_audio->data = (void *)card;
