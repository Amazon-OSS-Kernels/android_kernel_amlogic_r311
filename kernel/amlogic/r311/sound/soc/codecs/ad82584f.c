...
static int ad82584f_set_DRC_enum(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol);
static int ad82584f_get_DRC_enum(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol);
...
static const struct snd_kcontrol_new ad82584f_snd_controls[] = {
	...
	SOC_SINGLE("Set EQ Enable", CTRL4, 1, 1, 1),
	SOC_SINGLE_BOOL_EXT("Set DRC Enable", 0,
			ad82584f_get_DRC_enum, ad82584f_set_DRC_enum),
};

/*reg 1byte*/
static unsigned ad82584f_drc1_table_length = AD82584F_REGISTER_COUNT;
static unsigned ad82584f_drc1_table[AD82584F_REGISTER_COUNT] = {0};

/*ram1+ram2 3byte*/
static unsigned ad82584f_drc1_tko_table_length = AD82584F_RAM_TABLE_COUNT*3*2;
static unsigned ad82584f_drc1_tko_table[AD82584F_RAM_TABLE_COUNT*3*2] = {0};

int debug_en = 0;
module_param(debug_en, int, 0644);
MODULE_PARM_DESC(debug_en, "debug_en");

module_param_array(ad82584f_drc1_table,
		uint, &ad82584f_drc1_table_length, 0664);
MODULE_PARM_DESC(ad82584f_drc1_table,
		"An array of ad82584 DRC reg table param");

module_param_array(ad82584f_drc1_tko_table,
		uint, &ad82584f_drc1_tko_table_length, 0664);
MODULE_PARM_DESC(ad82584f_drc1_tko_table,
		"An array of ad82584 DRC ram1 ram2 table param");
...
static int m_ram1_tab[AD82584F_RAM_TABLE_COUNT][4] = {
	...
	{0x57, 0x13, 0x0f, 0xaa},//##CH1.2_DRC1_Release_threshold
	...
	{0x64, 0x11, 0xa1, 0x34},//DRC1_Power_Meter
...
static int m_ram2_tab[AD82584F_RAM_TABLE_COUNT][4] = {
	...
	{0x64, 0x11, 0xce, 0x9b},//DRC2_Power_Meter
...
static int ad82584f_ram_check(struct snd_soc_codec *codec)
{
	int i = 0;
	int H8_data = 0, M8_data = 0, L8_data = 0;

	pr_info("ad82584f_ram_check\n");
	...
	for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
		...
		pr_info("0x%02x, 0x%02x, 0x%02x, 0x%02x\n", m_ram1_tab[i][0],
			H8_data, M8_data, L8_data);
	};

	pr_info("ram2:\n");
	for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
		snd_soc_write(codec, CFADDR, m_ram2_tab[i][0]);
		snd_soc_write(codec, CFUD, 0x44);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	DRC_enable = ucontrol->value.integer.value[0];
	if (DRC_enable == 0) {
		/*drc bypass*/
		for (i = 0; i < 8; i++) {
			m_reg_tab[CH1_CONFIG+i][1] |= (1<<2);
			snd_soc_write(codec, m_reg_tab[CH1_CONFIG+i][0],
				m_reg_tab[CH1_CONFIG+i][1]);
		}
	} else {
		/*drc enable*/
		/*for (i = 0; i < 8; i++) {
		...
		/*reg*/
		for (i = 0; i < AD82584F_REGISTER_COUNT; i++)
			m_reg_tab[i][1] = ad82584f_drc1_table[i];
		/* mute */
		snd_soc_write(codec, MUTE, 0x7f);
		...
		for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
			for (j = 0; j < 3; j++)
				m_ram1_tab[i][j+1] =
					ad82584f_drc1_tko_table[i * 3 + j];
		}
		/*ram2*/
		for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
			for (j = 0; j < 3; j++)
				m_ram2_tab[i][j+1] =
			ad82584f_drc1_tko_table
			[(i+AD82584F_RAM_TABLE_COUNT)*3+j];
		}
...
static int ad82584f_get_DRC_enum(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = DRC_enable;
	return 0;
}
...
static int ad82584f_init(struct snd_soc_codec *codec)
{
	...
	mdelay(25);
	snd_soc_write(codec, 0x1a, 0x32);//--Normal operation
	mdelay(20);
...
static int ad82584f_probe(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_codec_set_cache_io(codec, 8, 8, ad82584f->control_type);
	ad82584f_init(codec);
...
{
	//struct ad82584f_priv *ad82584f = snd_soc_codec_get_drvdata(codec);
	struct snd_soc_codec *codec = h->param;
	dev_info(codec->dev, "ad82584f_early_suspend!\n");

	ad82584f_suspend(codec);
	/* save volume */
	/*ad82584f->Ch1_vol = snd_soc_read(codec, C1VOL);
	ad82584f->Ch2_vol = snd_soc_read(codec, C2VOL);
	ad82584f->master_vol = snd_soc_read(codec, MVOL);
...
static void ad82584f_late_resume(struct early_suspend *h)
{
	//struct ad82584f_priv *ad82584f = snd_soc_codec_get_drvdata(codec);
	struct snd_soc_codec *codec = h->param;
	dev_info(codec->dev, "ad82584f_late_resume!\n");

	ad82584f_resume(codec);
	/*ad82584f_init(codec);
			     ...
			     const struct i2c_device_id *id)
{
	...
	ad82584f->control_type = SND_SOC_I2C;
	...
	/*reg*/
	for (i = 0; i < AD82584F_REGISTER_COUNT; i++)
		ad82584f_drc1_table[i] = m_reg_tab[i][1];
	/*ram1*/
	for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
		for (j = 0; j < 3; j++)
			ad82584f_drc1_tko_table[i * 3 + j] = m_ram1_tab[i][j+1];
	}
	/*ram2*/
	for (i = 0; i < AD82584F_RAM_TABLE_COUNT; i++) {
		for (j = 0; j < 3; j++)
