...
static int txlx_acodec_save_user_configs(struct snd_soc_codec *codec)
{
	struct txlx_acodec_priv *aml_acodec = snd_soc_codec_get_drvdata(codec);

	aml_acodec->dac_reg = snd_soc_read(codec, DAC_VOL_CTR_DAC_SOFT_MUTE);
	aml_acodec->dac2_reg = snd_soc_read(codec, ACODEC_DAC2_CONFIG);
	aml_acodec->adc_reg = snd_soc_read(codec, ADC_VOL_CTR_PGA_IN_CONFIG);
	return 0;
}

static int txlx_acodec_restore_user_configs(struct snd_soc_codec *codec)
{
	struct txlx_acodec_priv *aml_acodec = snd_soc_codec_get_drvdata(codec);

	snd_soc_write(codec, DAC_VOL_CTR_DAC_SOFT_MUTE, aml_acodec->dac_reg);
	snd_soc_write(codec, ACODEC_DAC2_CONFIG, aml_acodec->dac2_reg);
	snd_soc_write(codec, ADC_VOL_CTR_PGA_IN_CONFIG, aml_acodec->adc_reg);
	return 0;
}
...
{
	pr_info("%s!\n", __func__);
	txlx_acodec_save_user_configs(codec);
	txlx_acodec_dai_set_bias_level(codec, SND_SOC_BIAS_OFF);
...
static int txlx_acodec_resume(struct snd_soc_codec *codec)
{
	...
	txlx_acodec_dai_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	txlx_acodec_restore_user_configs(codec);
