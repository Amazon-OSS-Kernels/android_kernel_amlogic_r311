...
int aml_sd_voltage_switch(struct mmc_host *mmc, char signal_voltage)
{
	...
	if (pdata->vol_switch) {
		...
		if (signal_voltage == MMC_SIGNAL_VOLTAGE_180) {
			ret = gpio_direction_output(pdata->vol_switch_def, 1);
			ret |= gpio_direction_input(pdata->vol_switch);
			ret |= gpiod_set_pull(gpio_to_desc(pdata->vol_switch), GPIOD_PULL_DIS);
		} else {
			ret = gpio_direction_output(pdata->vol_switch_def, 1);
			ret |= gpio_direction_output(pdata->vol_switch, 0);
		}
		CHECK_RET(ret);
		if (!ret)
			pdata->signal_voltage = signal_voltage;
...
int of_amlsd_init(struct amlsd_platform *pdata)
{
	...
	if (pdata->gpio_cd) {
		...
		gpiod_set_pull(gpio_to_desc(pdata->gpio_cd), GPIOD_PULL_UP);
		CHECK_RET(ret);
	}
#if 0
	if (pdata->gpio_ro) {
	...
	if (pdata->vol_switch_def) {
		pr_info("vol_switch_def = %x\n", pdata->vol_switch_def);
		ret = gpio_request_one(pdata->vol_switch_def,
			GPIOF_OUT_INIT_HIGH, MODULE_NAME);
		CHECK_RET(ret);
	}
	if (pdata->vol_switch) {
		pr_info("vol_switch = %x\n", pdata->vol_switch);
		ret = gpio_request_one(pdata->vol_switch,
			GPIOF_OUT_INIT_LOW, MODULE_NAME);
		CHECK_RET(ret);
	}
...
int sd_power_up(struct amlsd_platform *pdata)
{
	int ret;
...
int sd_power_down(struct amlsd_platform *pdata)
{
	int ret;
	...
	if (pdata->vol_switch_def) {
		//ret = gpio_direction_output(pdata->vol_switch_def, 0);
		//CHECK_RET(ret);
	}
	if (pdata->vol_switch) {
		ret = gpio_direction_output(pdata->vol_switch, 0);
