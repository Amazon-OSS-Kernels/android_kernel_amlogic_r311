...
int meson_set_mode(struct thermal_zone_device *tzd,
		   enum thermal_device_mode mode)
{
	int i, ret = 0;
	struct thermal_cooling_device *cdev = NULL;
	struct meson_cooldev *meson_cooldev = get_meson_gcooldev();
	/*
	 * each cooling device should return to max state if thermal is disabled
	 */
	...
	for (i = 0; i < meson_cooldev->cool_dev_num; i++) {
		cdev = meson_cooldev->cool_devs[i].cooling_dev;
		if (cdev)
