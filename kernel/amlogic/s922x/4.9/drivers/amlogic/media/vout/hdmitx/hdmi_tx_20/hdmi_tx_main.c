...
struct extcon_dev *hdmitx_extcon_hdcp_log;
struct extcon_dev *hdmitx_extcon_hdmiocp;
...
static int load_edid_data(unsigned int type, char *path)
{
	...
	memcpy(hdmitx_device.EDID_buf1, buf, length);
...
static void hdmitx_set_drm_pkt(struct master_display_info_s *data)
{
	...
	if (hdev->hdr_transfer_feature == 0x30) {
		if ((hdev->RXCap.hdr10plus_info.ieeeoui
			...
			pr_info("hdmitx: HDR10+ not supported, treat as hdr10\n");
		}
	}
...
/**/
static ssize_t print_aud_cap(char *buf, struct rx_audiocap *cap,
				char *forced_name)
{
	int j, pos = 0;
	...
	pos += snprintf(buf + pos, PAGE_SIZE, "%s, %d ch, ",
		!forced_name ? aud_coding_type[cap->audio_format_code] :
		forced_name, cap->channel_num_max + 1);
	...
	pos += snprintf(buf + pos, PAGE_SIZE, " kHz, ");
	...
	pos += snprintf(buf + pos, PAGE_SIZE, " bit\n");
			...
			struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < pRXCap->AUD_count; i++) {
		...
		/* Detect Dolby Atmos over DD+ */
		if (pRXCap->RxAudioCap[i].audio_format_code == 10) {
			if (pRXCap->RxAudioCap[i].cc3 & 0x3) {
				pos += print_aud_cap(&buf[pos],
					&pRXCap->RxAudioCap[i], "DolbyAtmos");
			}
		}
	}
...
/**/
static ssize_t show_lipsync_cap(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int pos = 0;
	...
	pos += snprintf(buf + pos, PAGE_SIZE,
		"ProgressiveLipSyncPresent Video Audio\n");
	pos += snprintf(buf+pos, PAGE_SIZE, "%d, %d, %d\n",
		hdmitx_device.RXCap.Latency_Present,
		hdmitx_device.RXCap.Video_Latency,
		hdmitx_device.RXCap.Audio_Latency);
...
static ssize_t show_hpd_ctrl(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d", hdmitx_extcon_audio->state);
}

static ssize_t store_hpd_ctrl(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	if (strncmp(buf, "1", 1) == 0)
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	extcon_set_state_sync(hdmitx_extcon_hdmi,
						EXTCON_DISP_HDMI,
						hdev->hpd_state);
...
static DEVICE_ATTR(lipsync_cap, 0444, show_lipsync_cap, NULL);
...
static DEVICE_ATTR(hpd_ctrl, 0664, show_hpd_ctrl, store_hpd_ctrl);
...
static void clear_hdr_info(struct hdmitx_dev *hdev)
{
	...
	if (info) {
		...
		memset(info->hdr_info.rawdata, 0, 7);
		for (i = 0; i < 4; i++)
			memset(&(info->hdr_info.dynamic_info[i]),
				0, sizeof(struct hdr_dynamic));
...
/* When init hdmi, clear the hdmitx module edid ram and edid buffer. */
	...
	cod_init(hdmitx_device);
	cod_test(hdmitx_device);
...
void hdmitx_set_hdcplog(int cnt)
{
	extcon_set_state_sync(hdmitx_extcon_hdcp_log, EXTCON_DISP_HDMI, cnt);
}
...
void hdmitx_extcon_register(struct platform_device *pdev, struct device *dev)
{
	...
	edev->dev.parent = dev;
	edev->name = "hdcp_log";
	dev_set_name(&edev->dev, "hdcp_log");
	ret = extcon_dev_register(edev);
	if (ret < 0) {
		pr_info(SYS "failed to register extcon hdcp\n");
		return;
	}
	hdmitx_extcon_hdcp_log = edev;

	/*hdmitx extcon hdmicod*/
	edev = extcon_dev_allocate(hdmi_cable);
	if (IS_ERR(edev)) {
		pr_info(SYS "failed to allocate extcon hdcp\n");
		return;
	...
	edev->dev.parent = dev;
	edev->name = "hdmi_cod";
	dev_set_name(&edev->dev, "hdmi_cod");
	ret = extcon_dev_register(edev);
	if (ret < 0) {
		pr_info(SYS "failed to register extcon hdmi_cod\n");
		return;
	}
	hdmitx_extcon_hdmiocp = edev;
}
...
static int amhdmitx_get_dt_info(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		hdmitx_device.hdmiocp = gpiod_get(&(pdev->dev), "hdmiocp", GPIOD_IN);
		if (IS_ERR(hdmitx_device.hdmiocp)) {
			pr_err("fail to claim hdmiocp gpio\n");
		}
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(dev, &dev_attr_hpd_ctrl);
...
static int amhdmitx_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev, &dev_attr_hpd_ctrl);
