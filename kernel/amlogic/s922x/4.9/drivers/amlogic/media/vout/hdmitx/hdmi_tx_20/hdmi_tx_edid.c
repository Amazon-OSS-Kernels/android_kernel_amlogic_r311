	...
	for (offset = 4 ; offset < End ; ) {
		...
		case HDMI_EDID_BLOCK_TYPE_VENDER:
			...
			if ((BlockBuf[offset] == 0x03) &&
				...
				/* Lip Sync */
				if ((count >= 8) && (BlockBuf[offset+7] & 0x80))
					pRXCap->Latency_Present = 1;

				if (count >= 9 && (BlockBuf[offset+7] & 0x80) &&
					(BlockBuf[offset+8] != 0x00) &&
					(BlockBuf[offset+8] != 0xFF))
					pRXCap->Video_Latency =
					    (BlockBuf[offset+8] - 1) * 2;

				if (count >= 10 && (BlockBuf[offset+7] & 0x80)
					&& (BlockBuf[offset+9] != 0x00)
					&& (BlockBuf[offset+9] != 0xFF))
					pRXCap->Audio_Latency =
					    (BlockBuf[offset+9] - 1) * 2;
	...
	int buffer_len)
{
	...
	pos += snprintf(buffer+pos, buffer_len-pos,
		"Latency present: %d\n", pRXCap->Latency_Present);

	pos += snprintf(buffer+pos, buffer_len-pos,
		"Video Latency (P), ms: %d\n", pRXCap->Video_Latency);

	pos += snprintf(buffer+pos, buffer_len-pos,
		"Audio Latency (P), ms: %d\n", pRXCap->Audio_Latency);
