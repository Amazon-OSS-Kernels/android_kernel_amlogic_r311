			    ...
			    bool force)
{
	...
	u32 val, mask, bit;
	unsigned long flags;
	struct cpumask bigcluster_mask;

	if (!force) {
		/* set big cluster affinity */
		cpumask_and(&bigcluster_mask, mask_val, cpu_online_mask);
		cpumask_clear_cpu(0, &bigcluster_mask);
		cpumask_clear_cpu(1, &bigcluster_mask);
		cpu = cpumask_any(&bigcluster_mask); //cpumask_any()
