...
void *aml_sync_create_timeline(const char *tname)
{
	...
	timeline = (void *)sw_sync_timeline_create(tname);
	return timeline;
}
...
{
	struct sw_sync_timeline *tl = (struct sw_sync_timeline *)timeline;
	int fd;
	int err;
	struct sync_pt *pt;
	...
	pt = sw_sync_pt_create(tl, value);
	if (!pt) {
		err = -ENOMEM;
		goto err;
	}

	sync_fence = sync_fence_create(tl->obj.name, pt);
	if (!sync_fence) {
		sync_pt_free(pt);
		err = -ENOMEM;
		goto err;
...
void aml_sync_inc_timeline(void *timeline, unsigned int value)
{
	...
	sw_sync_timeline_inc(tl, value);
}
EXPORT_SYMBOL(aml_sync_inc_timeline);

void *aml_sync_get_fence(int syncfile_fd)
{
	return (void *)sync_fence_fdget(syncfile_fd);
}
EXPORT_SYMBOL(aml_sync_get_fence);

int aml_sync_wait_fence(void *wait, long timeout)
{
	struct sync_fence *fence = (struct sync_fence *)wait;

	return sync_fence_wait(fence, timeout);
}
EXPORT_SYMBOL(aml_sync_wait_fence);

void aml_sync_put_fence(void *put)
{
	struct sync_fence *fence = put;
