...
struct aml_chipset_info {
	/* axg could not reset each module sepecailly. */
	bool reset_all;
				      ...
				      struct snd_pcm_hw_params *params)
{
	...
	if (loopback_is_enable((char *)dai_props->suffix_name) && mclk)
...
int aml_card_prepare(struct snd_pcm_substream *substream)
{
	...
	if (loopback_is_enable((char *)dai_props->suffix_name))
				     ...
				     struct aml_card_data *priv)
{
	...
	if (lb_link) {
		...
		else if (priv->chipinfo) {
			priv->lb_cfg.is_reset_all = priv->chipinfo->reset_all;
			if (priv->lb_cfg.is_reset_all)
				pr_info("this chip needs to reset all loopback related modules\n");
		}
	}
