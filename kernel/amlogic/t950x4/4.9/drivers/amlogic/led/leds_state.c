...
static void meson_ledled_set_twenty_percent(struct work_struct *work)
{
	...
	if (strstr(oem_data, "ABC-tm") != NULL) {
		pr_info("%s set brightness to 50%%\n", DRIVER_NAME);
...
static ssize_t amazon_led_pattern_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	/* TODO: */
	return 0;
			   ...
			   const char *buf, size_t size)
{
	...
	struct led_state_data *data = container_of(led_cdev,
			struct led_state_data, cdev);
	...
	if (res != 0) {
		pr_err("%s Can't parse! amazon led pattern\n",
		       DRIVER_NAME);
		return -EINVAL;
	}
...
static DEVICE_ATTR_RW(amazon_led_pattern);
...
// FOR AMAZON, set the scpi led state to default
int meson_led_state_set_default(u32 led_id)
{
    // for set LED to default
	uint32_t data[2];
	int ret, count;

	data[0] = 0;
	data[1] = LED_STATE_DEFAULT;
...
EXPORT_SYMBOL_GPL(meson_led_state_set_default);
...
void meson_led_device_create(struct led_classdev *led_cdev)
{
	...
	rc = device_create_file(led_cdev->dev, &dev_attr_amazon_led_pattern);
	if (rc)
...
err_out_led_state:
	...
	device_remove_file(led_cdev->dev, &dev_attr_amazon_led_pattern);
}
...
static int meson_led_state_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&data->led_work, meson_ledled_set_twenty_percent);
