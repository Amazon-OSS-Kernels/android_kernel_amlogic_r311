...
static void nl80211_ch_switch_notify_cnt(struct cfg80211_registered_device *rdev,
				     struct net_device *netdev,
				     struct cfg80211_chan_def *chandef,
				     gfp_t gfp,
                                     uint16_t csacnt)
{
	...
	hdr = nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_CH_SWITCH_NOTIFY_CNT);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
	...
	if (nl80211_send_chandef(msg, chandef))
        ...
        if (nla_put_u16(msg, NL80211_ATTR_VENDOR_DATA, csacnt))
                goto nla_put_failure;
...
static void nl80211_quiet_stop(struct cfg80211_registered_device *rdev,
				     struct net_device *netdev,
				     gfp_t gfp,
				     int timeNs)
{
	...
	hdr = nl80211hdr_put(msg, 0, 0, 0, NL80211_CMD_QUIET_STOP);
	if (!hdr) {
		nlmsg_free(msg);
		return;
	}
    if (nla_put_u32(msg, NL80211_ATTR_VENDOR_DATA, timeNs))
            goto nla_put_failure;
...
void cfg80211_ch_switch_notify_cnt(struct net_device *dev,
			       struct cfg80211_chan_def *chandef,
                               uint16_t csacnt)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	...
	trace_cfg80211_ch_switch_notify_cnt(dev, chandef, csacnt);

	wdev->channel = chandef->chan;
	nl80211_ch_switch_notify_cnt(rdev, dev, chandef, GFP_KERNEL, csacnt);
}
EXPORT_SYMBOL(cfg80211_ch_switch_notify_cnt);


void cfg80211_quiet_stop(struct net_device *dev, int timeNs)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct wiphy *wiphy = wdev->wiphy;
	struct cfg80211_registered_device *rdev = wiphy_to_dev(wiphy);
	...
	trace_cfg80211_quiet_stop(dev, timeNs);

	nl80211_quiet_stop(rdev, dev, GFP_KERNEL, timeNs);
}
EXPORT_SYMBOL(cfg80211_quiet_stop);
