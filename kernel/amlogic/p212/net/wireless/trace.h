...
TRACE_EVENT(cfg80211_quiet_stop,
	TP_PROTO(struct net_device *netdev, int timeNs),
	TP_ARGS(netdev, timeNs),
	TP_STRUCT__entry(
		NETDEV_ENTRY
		__field(int, timeNs)
	),
	TP_fast_assign(
		NETDEV_ASSIGN;
		__entry->timeNs = timeNs;
	),
	TP_printk(NETDEV_PR_FMT ", time %d ", NETDEV_PR_ARG, __entry->timeNs)
);

TRACE_EVENT(cfg80211_ch_switch_notify_cnt,
	TP_PROTO(struct net_device *netdev,
		 struct cfg80211_chan_def *chandef,
                 u8 csacnt),
	TP_ARGS(netdev, chandef, csacnt),
	TP_STRUCT__entry(
		NETDEV_ENTRY
		CHAN_DEF_ENTRY
                __field(u8, csacnt)
	...
	TP_fast_assign(
		NETDEV_ASSIGN;
		CHAN_DEF_ASSIGN(chandef);
                __entry->csacnt = csacnt;
	),
	TP_printk(NETDEV_PR_FMT ", " CHAN_DEF_PR_FMT ", csacnt: %02X",
		  NETDEV_PR_ARG, CHAN_DEF_PR_ARG, __entry->csacnt)
);
