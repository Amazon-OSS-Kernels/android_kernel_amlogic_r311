	...
	do {
		...
		case VENDOR_TAG:
			if ((Addr + (Data&0x1f)) < D) {
				...
				if ((buff[Addr + 8]) & 0x80) {
					info->p_latency_present = 1;
					info->p_audio_latency = buff[Addr + 10] ? ((buff[Addr + 10] - 1) << 1) : 0;
					info->p_video_latency = buff[Addr + 9] ? ((buff[Addr + 9] - 1) << 1) : 0;
				}
			}
			break;
...
/* return -1; */
	...
	hdmi_print(0, "\n");
#endif
	if (hdmitx_device->hdmi_info.p_latency_present)
		pr_info("hdmi lipsync video %d audio %d\n", hdmitx_device->hdmi_info.p_video_latency, hdmitx_device->hdmi_info.p_audio_latency);
	else
		pr_info("hdmi lipsync not present\n");
...
void hdmitx_edid_clear(struct hdmitx_dev *hdmitx_device)
{
	...
	hdmitx_device->hdmi_info.p_latency_present = 0;
	hdmitx_device->hdmi_info.p_video_latency = 0;
	hdmitx_device->hdmi_info.p_audio_latency = 0;
