	...
	struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf + pos, PAGE_SIZE, "ProgressiveLipSyncPresent Video Audio\n");
	pos += snprintf(buf+pos, PAGE_SIZE, "%d, %d, %d \n",
		hdmitx_device.hdmi_info.p_latency_present, hdmitx_device.hdmi_info.p_video_latency,
		hdmitx_device.hdmi_info.p_audio_latency);
	return pos;
}
...
static DEVICE_ATTR(lipsync_cap, S_IRUGO, show_lipsync_cap, NULL);
...
/* extern struct switch_dev lang_dev; */
	...
	ret = device_create_file(hdmitx_dev, &dev_attr_lipsync_cap);
