...
static int tmp103_get_temp(struct device *dev, int data, unsigned long *temp)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct tmp103_temp_sensor *tmp103 = platform_get_drvdata(pdev);
	long current_temp;
	static long saved_temp;
	int count = 1;
	...
	pr_debug("TMP103 reads temperature %d", current_temp);
...
static struct virtual_thermal_dev_ops tmp103_fops = {
