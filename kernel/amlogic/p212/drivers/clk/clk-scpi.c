...
#ifdef CONFIG_AVS
static int osc_ring_val;

static int __init init_osc_ring(char *arg)
{
	int ret;

	ret = kstrtouint(arg, 10, &osc_ring_val);
	if (ret)
		return ret;

	pr_info("osc_ring val=%d\n",
		osc_ring_val);

	return 1;
}
__setup("osc_ring=", init_osc_ring);
#endif
...
#ifdef CONFIG_AVS
	pr_info("osc_ring setting======0x%x\n",
		scpi_osc_ring_val_set(osc_ring_val));
#endif
