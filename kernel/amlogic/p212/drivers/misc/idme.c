...
unsigned int idme_get_board_long_rev(void)
{
	struct device_node *ap = NULL;
	char board_rev[10] = { 0 };
	const char *board_id = NULL;
	...
	strlcpy(board_rev, (board_id + 8), sizeof(board_rev));
	if (unlikely(kstrtouint(board_rev, 10, &rs)))
		pr_err("idme_get_board_rev kstrtouint failed!\v");

	return rs;
}
EXPORT_SYMBOL(idme_get_board_long_rev);
...
static int __init idme_init(void)
{
	...
	if (!proc_idme) {
		of_node_put(root);
		pr_err("can't create proc entry for idme");
		return -ENOMEM;
	}
