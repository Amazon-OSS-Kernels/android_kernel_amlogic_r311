...
KERNEL_OUT_DIR="${WORKSPACE_OUT_DIR}/kernel"

for d in "${PLATFORM_EXTRACT_DIR}" "$KERNEL_OUT_DIR"
do
    mkdir -p "${d}"
done
...
function usage {
    echo "Usage: ${BASH_SOURCE[0]} path_to_platform_tar|path_to_platform_dir output_folder" 1>&2
    exit 1
}
...
function display_config {
    ...
    echo "DEFINITION CONFIG: ${KERNEL_DEFCONFIG_NAME}"
...
function exec_build_kernel {
    ...
    # Step 1: defconfig
    echo "Make defconfig: make ${MAKE_ARGS} ${KERNEL_DEFCONFIG_NAME}"
    make ${MAKE_ARGS} ${KERNEL_DEFCONFIG_NAME}
    ...
    if [[ -n "${ZIMAGE_TARGET}" ]]
    then
        make ${MAKE_ARGS} ${PARALLEL_EXECUTION} zImage
    else
